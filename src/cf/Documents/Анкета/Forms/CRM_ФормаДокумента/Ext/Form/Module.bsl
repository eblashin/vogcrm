...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	// +CRM
	// Открыта анкета без документа "Назначение опросов".
	Если ЗначениеЗаполнено(Объект.CRM_ШаблонАнкеты) Тогда
		ВозможностьПредварительногоСохранения = Истина;
		ШаблонАнкеты          				  = Объект.CRM_ШаблонАнкеты;
		Вступление                            = ШаблонАнкеты.Вступление;
		Заключение                            = ШаблонАнкеты.Заключение;
		Если Параметры.Свойство("ТолькоФормаЗаполнения") Тогда
			АвтоЗаголовок = Ложь;
			Заголовок                            = ШаблонАнкеты.Заголовок;
			Элементы.НадписьВступление.Заголовок = ШаблонАнкеты.Вступление;
			Вступление                           = ШаблонАнкеты.Вступление;
		КонецЕсли;
		Анкетирование.УстановитьЭлементДереваРазделовАнкетыВступлениеЗаключение(ДеревоРазделов,"Вступление");
		Анкетирование.ЗаполнитьДеревоРазделов(ЭтотОбъект,ДеревоРазделов);
	...
	Если Параметры.Свойство("ОткрытиеИзТелемаркетинга") Тогда
		CRM_ОткрытиеИзТелемаркетинга = Параметры.ОткрытиеИзТелемаркетинга;
	КонецЕсли;		
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	КонецЕсли;
	Если CRM_ОткрытиеИзТелемаркетинга Тогда 
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			CRM_ОткрытиеИзТелемаркетинга	= Ложь;
			Отказ							= Истина;
...
Процедура УстановитьЗначенияРеквизитовФормыЗаполненияРаздела()
	...
	Для Каждого СтрокаВопросРаздела Из ТаблицаВопросовРаздела Цикл
		Если СтрокаВопросРаздела.ТипСтроки = "Вопрос" Тогда
			...
			Если НЕ (ТекущийВопросТипОтвета = Перечисления.ТипыОтветовНаВопрос.НесколькоВариантовИз) 
				И НЕ (ТекущийВопросТипОтвета = Перечисления.ТипыОтветовНаВопрос.ОдинВариантИз) Тогда
				Если НЕ ПустаяСтрока(СтрокаВопросРаздела.ЭлементарныйВопрос.CRM_ИмяСправочника) Тогда
					Если НЕ (СтрокаВопросРаздела.ТипВопроса = Перечисления.ТипыВопросовШаблонаАнкеты.Табличный) Тогда
						...
						Если (НЕ ЗначениеЗаполнено(ЭтотОбъект[ИмяВопроса]))
							ИЛИ (ТипЗнч(ЭтотОбъект[ИмяВопроса]) = Тип("Булево") И ЭтотОбъект[ИмяВопроса] = Ложь) Тогда							
							ЗначениеИзСправочника = CRM_АнкетированиеПривилегированный.ПолучитьРеквизитИзСправочника(Объект.Респондент, СтрокаВопросРаздела.ЭлементарныйВопрос);
							Если НЕ (ЗначениеИзСправочника = Неопределено) Тогда
								ЭтотОбъект[ИмяВопроса] = ЗначениеИзСправочника;
							КонецЕсли;	
						КонецЕсли; 
					КонецЕсли;
				КонецЕсли;				
...
Функция ОкончаниеРедактированиеФормыЗаполнения(РежимЗаписи)
	...
	// +CRM
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		CRM_АнкетированиеПривилегированный.ЗаписатьРеквизитыОбъектаАнкетирования(ДокументОбъект);
	КонецЕсли;
	// -CRM
	Возврат Ложь;
...
Процедура ПостроениеФормыСогласноРаздела()
	...
	CRM_Анкетирование.ПостроениеФормыЗаполненияПоРазделу(ЭтотОбъект,ТекущиеДанныеДеревоРазделов);
	// -CRM
