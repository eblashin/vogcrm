...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбменаПриЗагрузке = "") 
	...
	ИначеЕсли НаправлениеОбмена = "Отправка" И ЗначениеЗаполнено(ПутьКМенеджеруОбменаВыгрузки) Тогда
		Попытка
			КомпонентыОбмена.МенеджерОбмена = ВнешниеОбработки.Создать(ПутьКМенеджеруОбменаВыгрузки);
		Исключение
			ПутьКМенеджеруОбменаВыгрузки = "";
			МенеджерОбменаВнутренний = Истина;
		КонецПопытки;
	ИначеЕсли НаправлениеОбмена = "Получение" И ЗначениеЗаполнено(ПутьКМенеджеруОбменаЗагрузки) Тогда
		Попытка
			КомпонентыОбмена.МенеджерОбмена = ВнешниеОбработки.Создать(ПутьКМенеджеруОбменаЗагрузки);
		Исключение
			ПутьКМенеджеруОбменаЗагрузки = "";
			МенеджерОбменаВнутренний = Истина;
		КонецПопытки;
	ИначеЕсли ЗначениеЗаполнено(УзелОбмена) Тогда
		...
		Если ЗначениеЗаполнено(ПутьКМенеджеруОбмена) Тогда
			КомпонентыОбмена.МенеджерОбмена = ВнешниеОбработки.Создать(ПутьКМенеджеруОбмена);
		Иначе
			МенеджерОбменаВнутренний = Истина;
		КонецЕсли;
	Иначе
...
Функция РезультатВыгрузкиВXML(СтруктураПараметров, АдресДляРазмещенияРезультата = Неопределено) Экспорт
	...
	Если ПараметрыКонвертации.Свойство("ВыгружатьАналитикуПоСкладам") Тогда
	...
	Если СписокДополнениеКВыгрузке.Количество() > 0 Тогда
		Для Каждого ЭлементСписка Из СписокДополнениеКВыгрузке Цикл
			...
			ПравилоОбработки = КомпонентыОбмена.ПравилаОбработкиДанных.Найти(СсылкаВыгрузки.Метаданные(), "ОбъектВыборкиМетаданные");
			Если ПравилоОбработки = Неопределено Тогда
				Продолжить;
			КонецЕсли;
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		ТекстСообщения = НСтр("ru='В ходе выполнения операции возникли ошибки'") + ": " + Символы.ПС + КомпонентыОбмена.СтрокаСообщенияОбОшибке;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	ИначеЕсли НЕ ЕстьВыгруженныеОбъекты Тогда
		ТекстСообщения = НСтр("ru='Не найдено ни одного объекта к выгрузке.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	РезультатВыгрузки = КомпонентыОбмена.ФайлОбмена.Закрыть();
	РезультатВыгрузки = РезультатВыгрузки + Символы.ПС + "</Body>" + Символы.ПС + "</Message>";
	Если МестоВыгрузки = 0 Тогда
		// в файл
		ТХ = Новый ТекстовыйДокумент;
		ТХ.УстановитьТекст(РезультатВыгрузки);
	...
	Иначе
		// в текст
		Если АдресДляРазмещенияРезультата = Неопределено Тогда
			АдресХранения = ПоместитьВоВременноеХранилище(РезультатВыгрузки);
		Иначе
			ПоместитьВоВременноеХранилище(РезультатВыгрузки, АдресДляРазмещенияРезультата);
			АдресХранения = АдресДляРазмещенияРезультата;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузкаСообщения(СтруктураПараметров, АдресРезультата) Экспорт
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Ошибка чтения сообщения загрузки. Неверный формат сообщения.'"));
