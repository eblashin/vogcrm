...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не (Метаданные.Обработки.Найти(МетаОбработкаИмя) <> Неопределено
	...
	Если ЭтоВнешняяОбработка Тогда
		...
		АдресХранилища = ПоместитьВоВременноеХранилище(ДанныеОбработки, УникальныйИдентификатор);
		БазовоеИмяДляФормы = "ВнешняяОбработка." + ВнешниеОбработки.Подключить(АдресХранилища,,Ложь);
		ИмяОбработки = ОбработкаОбъект.ИспользуемоеИмяФайла;
	Иначе
...
Процедура ВыполнитьОперацию(Команда)
	...
	Если ВидОперации = "Загрузка" Тогда
		ПодключитьОбработчикОжидания("ЗагрузитьСообщение",1,Истина);
	Иначе
...
Процедура ЗагрузитьСообщение()
	...
	Если ИсточникЗагрузки = 1 Тогда
		Если НЕ ЗначениеЗаполнено(ДанныеДляЗагрузкиXDTO) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Текстовое поле с данными для загрузки не заполнено.'"));
			Возврат;
		КонецЕсли;
...
Процедура ВыбратьФайлДляЗагрузкиНаКлиенте(ЗагружатьПослеВыбора = Ложь)
	...
	ОповещениеВыбора = Новый ОписаниеОповещения("ВыбранФайл", ЭтотОбъект, Новый Структура("ДляЗагрузки, ЗагружатьПослеВыбора", Истина, ЗагружатьПослеВыбора));
...
Функция ВыгрузитьДанныеНаСервере()
	...
	Если ЭтоВнешняяОбработка Тогда
		Возврат Новый Структура("Статус, АдресРезультата, ИдентификаторЗадания", "Выполнено", ОбработкаОбъект.РезультатВыгрузкиВXML(СтруктураПараметров));
	КонецЕсли;
...
Функция ЗагрузитьДанныеНаСервере()
	...
	Если ЭтоВнешняяОбработка Тогда
		...
		Возврат Новый Структура("Статус, АдресРезультата, ИдентификаторЗадания", "Выполнено", "");
	КонецЕсли;
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(НСтр("ru = 'Общий модуль ""%1"" не найден.'"), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
...
Процедура УстановитьВидимостьДоступностьКнопок(ФлагДоступность)
	...
	Элементы.ФормаПрервать.Видимость = НЕ ФлагДоступность И НЕ ЭтоВнешняяОбработка;
	ЭтотОбъект.ОбновитьОтображениеДанных();
КонецПроцедуры
