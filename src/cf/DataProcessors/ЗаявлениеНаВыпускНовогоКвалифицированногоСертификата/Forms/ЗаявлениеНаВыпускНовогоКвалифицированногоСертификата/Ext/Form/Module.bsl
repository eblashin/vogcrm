...
Процедура ПерейтиНаСтраницуОтправкаЗаявленияНаСервереПриЗагрузке(Загрузка = Истина)
	...
	ИнтернетПоддержкаПользователейПодключена = ИнтернетПоддержкаПользователейПодключена();
...
&НаСервере
Функция ИнтернетПоддержкаПользователейПодключена()
	...
	Иначе
		...
		Иначе
			ПоказатьПредупреждение(Новый ОписаниеОповещения(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось задействовать внешнюю компоненту по причине:
					           |%1'"),
					Результат.ОписаниеОшибки));
		КонецЕсли;
		Возврат;
