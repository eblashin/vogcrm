...
Процедура ОбновитьОписания()
	...
	ABCОписаниеЧастоИспользуемыеВозможности = НСтр("ru = 'ABC классификация по данным за период: '") + ПолучитьОписаниеНастройки(ПериодABCКлассификации, КоличествоПериодовABCКлассификации);
	XYZОписаниеЧастоИспользуемыеВозможности = НСтр("ru = 'XYZ классификация по данным за период: '") + ПолучитьОписаниеНастройки(ПериодXYZКлассификации, КоличествоПериодовXYZКлассификации, ПодпериодXYZКлассификации);
	ABCОписаниеПолныеВозможности = НСтр("ru = 'ABC классификация за все периоды по данным за период: '") + ПолучитьОписаниеНастройки(ПериодABCКлассификации, КоличествоПериодовABCКлассификации);
	XYZОписаниеПолныеВозможности = НСтр("ru = 'XYZ классификация за все периоды по данным за период: '") + ПолучитьОписаниеНастройки(ПериодXYZКлассификации, КоличествоПериодовXYZКлассификации, ПодпериодXYZКлассификации);
	
	// Часто используемые возможности.
	Элементы.ДекорацияABCXYZКлассификацияОписаниеЧастоИспользуемыеВозможности.Заголовок = ABCОписаниеЧастоИспользуемыеВозможности + " " + XYZОписаниеЧастоИспользуемыеВозможности;
...
Процедура НастройкаABCXYZКлассификацияПолныеВозможности(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОчиститьABCКлассификациюПолныеВозможностиЗавершение", ЭтотОбъект);
	ОткрытьФорму(ПолноеИмя + ".Форма.НастройкаПараметровКлассификацииКлиентов",,,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НастройкаABCXYZКлассификацияПолныеВозможностиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ПериодABCКлассификации = Результат.ПериодABCКлассификации;
		КоличествоПериодовABCКлассификации = Результат.КоличествоПериодовABCКлассификации;
		ПериодXYZКлассификации = Результат.ПериодXYZКлассификации;
		КоличествоПериодовXYZКлассификации = Результат.КоличествоПериодовXYZКлассификации;
		ПодпериодXYZКлассификации = Результат.ПодпериодXYZКлассификации;
...
Процедура ОчиститьABCКлассификациюПолныеВозможности(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОчиститьABCКлассификациюПолныеВозможностиЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Вся информация об ABC классификации клиентов будет очищена. Продолжить?'"), РежимДиалогаВопрос.ОКОтмена);
	
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьABCКлассификациюПолныеВозможностиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
...
Процедура ОчиститьXYZКлассификациюПолныеВозможности(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОчиститьXYZКлассификациюПолныеВозможностиЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Вся информация об XYZ классификации клиентов будет очищена. Продолжить?'"), РежимДиалогаВопрос.ОКОтмена);
	
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьXYZКлассификациюПолныеВозможностиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
...
Процедура ВыполнитьABCКлассификациюНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(ПериодABCКлассификации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнен период ABC-классификации. Необходимо произвести настройку.'"));
		Возврат;
	КонецЕсли;
	...
	ДатаКлассификации = CRM_ОбщегоНазначенияКлиентСервер.РасширенныйПериод(Выборка.Период, ПериодABCКлассификации, 0).ДатаОкончания + 1;
	МаксимальнаяДатаКлассификации = CRM_ОбщегоНазначенияКлиентСервер.РасширенныйПериод(CRM_ОбщегоНазначенияСервер.ПолучитьТекущуюДатуСеанса(), ПериодABCКлассификации, -1).ДатаОкончания + 1;
	...
	Пока ДатаКлассификации <= МаксимальнаяДатаКлассификации Цикл
		...
		ДатаКлассификации = CRM_ОбщегоНазначенияКлиентСервер.РассчитатьДатуОкончанияПериода(ДатаКлассификации, ПериодABCКлассификации, 1) + 1;
...
Процедура ВыполнитьXYZКлассификациюНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(ПериодXYZКлассификации) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнен период XYZ-классификации. Необходимо произвести настройку.'"));
		Возврат;
	КонецЕсли;
	...
	ДатаКлассификации = CRM_ОбщегоНазначенияКлиентСервер.РасширенныйПериод(Выборка.Период, ПериодXYZКлассификации, 0).ДатаОкончания + 1;
	МаксимальнаяДатаКлассификации = CRM_ОбщегоНазначенияКлиентСервер.РасширенныйПериод(CRM_ОбщегоНазначенияСервер.ПолучитьТекущуюДатуСеанса(), ПериодXYZКлассификации, -1).ДатаОкончания + 1;
	
	Пока ДатаКлассификации <= МаксимальнаяДатаКлассификации Цикл
		
		Справочники.Партнеры.ВыполнитьXYZКлассификацию(ДатаКлассификации);
		ДатаКлассификации = CRM_ОбщегоНазначенияКлиентСервер.РассчитатьДатуОкончанияПериода(ДатаКлассификации, ПериодXYZКлассификации, 1) + 1;
