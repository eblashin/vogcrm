			...
			Для каждого ТекущееХТТПСоединение Из ТаблицаХТТПСоединений Цикл
				...
				Если ВРег(ТекущееХТТПСоединение.Протокол) = ВРег("file") Тогда
					...
					Для каждого ТекущаяЛентаНовостей Из МассивОбрабатываемыхЛентНовостей Цикл
						...
						ТекстСообщения = НСтр("ru='Загрузка: загрузка файла новостей из ленты новостей %ЛентаНовостей% (протокол %Протокол%, сайт %Сайт%, имя файла %ИмяФайла%).
							|Успешно? %Успешно%'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Протокол%", ТекущаяЛентаНовостей.Протокол);
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сайт%", ТекущаяЛентаНовостей.Сайт);
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Успешно%", (ЕстьОшибки = Ложь));
				...
				ИначеЕсли (ВРег(ТекущееХТТПСоединение.Протокол) = ВРег("http"))
					...
					ТекстСообщения = НСтр("ru='Загрузка: установка http/https соединения (протокол %Протокол%).
						|Успешно? %СоединениеУстановлено%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Протокол%", ТекущееХТТПСоединение.Протокол);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%СоединениеУстановлено%", (ЕстьОшибки = Ложь));
					...
					Если ЕстьОшибки = Ложь Тогда
						...
						Для каждого ТекущаяЛентаНовостей Из МассивОбрабатываемыхЛентНовостей Цикл
							...
							ТекстСообщения = НСтр("ru='Загрузка: загрузка файла новостей из ленты новостей %ЛентаНовостей% (протокол %Протокол%, сайт %Сайт%, имя файла %ИмяФайла%, имя файла на сервере %лкИмяФайлаНаСервере%).
								|Успешно? %Успешно%'");
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяЛентаНовостей.ЛентаНовостей);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Протокол%", ТекущаяЛентаНовостей.Протокол);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Сайт%", ТекущаяЛентаНовостей.Сайт);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяФайла%", ТекущаяЛентаНовостей.ИмяФайла);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%лкИмяФайлаНаСервере%", лкИмяФайлаНаСервере);
							ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Успешно%", (ЕстьОшибки = Ложь));
...
#Область ОсновныеОбработки
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл
				...
				ТекстСообщения = НСтр("ru='Парсинг: лента новостей %ЛентаНовостей%.
					|Успешно? %Успешно%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЛентаНовостей%", ТекущаяСтрока.ЛентаНовостей);
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Успешно%", (ЕстьОшибки = Ложь));
...
//
Процедура ПарсингХМЛФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей, КонтекстВыполнения = Неопределено)
