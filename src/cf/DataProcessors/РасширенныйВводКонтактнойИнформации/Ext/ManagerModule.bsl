...
Функция ОшибкиЗаполненияАдресаXDTO(Знач Адрес, ВидИнформации) Экспорт
	...
	// ++ Харченко Д.И. №  - 04.10.2018 / 
	
	ВсеОшибки = Новый Массив;
	ПроверятьКлассификатор = Истина;
	
	// ++ Тищенко В.В. 31.01.2019
	// Добавлено условие НЕ ФлагиПроверки.вогВводитьАдресЯндексом
	Если ФлагиПроверки.вогПроверятьПоЯндексу И НЕ ФлагиПроверки.вогВводитьАдресЯндексом Тогда
		// -- Тищенко В.В.	
		Представление = Адрес.value;
		
		Если НЕ ЗначениеЗаполнено(Представление) Тогда
			ВидКонтактнойИнформации = УправлениеКонтактнойИнформациейСлужебный.СтруктураВидаКонтактнойИнформации(ВидИнформации);
			ВидКонтактнойИнформации.Вставить("ФорматАдреса", "");
			Представление = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(Адрес, ВидКонтактнойИнформации);
		КонецЕсли;
		
		РезультатЗапроса = вогГеокодированиеСервер.ПолучитьДанныеГеокодированияПоАдресу(Представление,, Истина);
		
		Если РезультатЗапроса.Успешно Тогда
			
			КоличествоТочныхСовпадений = 0;
			
			Для Каждого Стр из РезультатЗапроса.Результат Цикл 
				
				Если Стр.Комментарий = "Точное соответствие" Тогда
					КоличествоТочныхСовпадений = КоличествоТочныхСовпадений + 1;
				КонецЕсли;
				
			КонецЦикла;
			
			СтрокШаблонОшибки = "Точных соответствий: %1";
			
			Если НЕ КоличествоТочныхСовпадений = 1 Тогда
				СписокОшибок.Добавить("Дом", СтрШаблон(СтрокШаблонОшибки, Формат(КоличествоТочныхСовпадений,"ЧН=0")), Истина);
			КонецЕсли;
			
		Иначе 
			
			СписокОшибок.Добавить("Дом", "Ошибка соединения Яндекс: " + СокрЛП(РезультатЗапроса.Результат), Истина);
			
		КонецЕсли;
		
	ИначеЕсли ФлагиПроверки.вогПроверятьПоDaDaTa Тогда
		...
		РезультатЗапроса = вогГеокодированиеСервер.ПолучитьДанныеСервиса_dadata_ПоЗапросу(Представление, "Адрес");
		
		Если РезультатЗапроса.Успешно Тогда
			
			ДанныеАдреса = РезультатЗапроса.Результат;
			
			Если НЕ (ЗначениеЗаполнено(ДанныеАдреса.Улица)
				И ЗначениеЗаполнено(ДанныеАдреса.Город)
				И ЗначениеЗаполнено(ДанныеАдреса.Дом)) Тогда
				
				СписокОшибок.Добавить("Дом", "По сервису DaDaTa должен определяться город, улица, дом", Истина);
				
			КонецЕсли;
			
		Иначе 
			
			СписокОшибок.Добавить("Дом", "Ошибка соединения DaDaTa: " + СокрЛП(РезультатЗапроса.Результат), Истина);
			
		КонецЕсли;
		
		// ++ Тищенко В.В. 31.01.2019
		// Добавлено условие НЕ ФлагиПроверки.вогВводитьАдресЯндексом
	ИначеЕсли НЕ ФлагиПроверки.вогВводитьАдресЯндексом Тогда
		// -- Тищенко В.В. 
...
Процедура ЗаполнитьОшибкиАдресаПоКлассификатору(АдресРФ, ФлагиПроверки, Результат)
	...
	Если ФлагиПроверки.вогПроверятьПоЯндексу ИЛИ ФлагиПроверки.вогПроверятьПоDaDaTa Тогда
		// Заглушка
	ИначеЕсли ФлагиПроверки.ПроверятьПоФИАС 
		Или ФлагиПроверки.ПроверятьКорректность Тогда
