...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступноМуниципальноеДеление = вогОбщегоНазначенияПовтИсп.ДоступноМуниципальноеДеление();
	// -- VOG Солодов В.В. 06.03.2020
	...
	Если ПустаяСтрока(ЗначенияПолей) Тогда
		...
		// ++ VOG Солодов В.В. 06.03.2020 CRM-196
		Если ДоступноМуниципальноеДеление Тогда
	...
	Параметры.Свойство("Широта", 	Широта);
	Параметры.Свойство("Долгота", 	Долгота);
	//*** Блашин Евгений Игоревич VOG Окончание
	
	// ++ VOG Солодов В.В. 06.03.2020 CRM-196
	Если Не ДоступноМуниципальноеДеление Тогда
		Элементы.МуниципальноеДеление.Видимость = Ложь;
	КонецЕсли;
	// -- VOG Солодов В.В. 06.03.2020
...
Процедура ПриОткрытии(Отказ)
	...
	Если ВидКонтактнойИнформации.вогВводитьАдресЯндексом Тогда
		Отказ = Истина;
		ОткрытьВГеокодировании(Неопределено, Истина);
		Возврат;
	КонецЕсли;
	
	// +++ VOG Кулаков П.Л. 10.03.2020 CRM-223
	// Скроем кнопку выбора адреса на карте. 
	// Для ввода адреса по карте необходимо в виде контактной информации поставить галку "вогВводитьАдресЯндексом"
	Элементы.ОткрытьВГеокодировании.Видимость = Ложь;
	// --- VOG Кулаков П.Л.
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Модифицированность Тогда // При немодифицированности работает как "отмена".
		...
		|Модифицированность, ЕстьКлассификатор, ДополнительныйЭлемент, ПодчиненныйЭлемент, ЭтоВебСервис, АдресНаДату,Широта,Долгота
		|");
		...
		Если (ВидКонтактнойИнформации.ПроверятьКорректность	И Не РазрешитьВводАдресаВСвободнойФорме
			И (ВидКонтактнойИнформации.вогПроверятьПоDaDaTa ИЛИ ВидКонтактнойИнформации.вогПроверятьПоЯндексу)
...
#Иначе
		Если Не Открыта() И ВидКонтактнойИнформации.вогВводитьАдресЯндексом Тогда
			ВыполнитьОбработкуОповещения(ЭтаФорма.ОписаниеОповещенияОЗакрытии, Результат);
		Иначе 
			ОповеститьОВыборе(Результат);
		КонецЕсли;
#КонецЕсли
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	//*** 12 апреля 2019 г.
	//*** Блашин Евгений Игоревич VOG Начало
	Результат.ДанныеВыбора.Вставить("Широта", 	Контекст.Широта);
	Результат.ДанныеВыбора.Вставить("Долгота", 	Контекст.Долгота);
	//*** Блашин Евгений Игоревич VOG Окончание
...
// ++ Харченко Д.И. №  - 21.09.2018 / 
&НаКлиенте
Процедура ОткрытьВГеокодировании(Команда, СразуВыбирать = Ложь)
	
	СтрокаГеокода = "";
	//Проверим что обе не равны 0, надеемся что адреса с координатами 0,0 не будет)))
	Если Долгота <> 0  ИЛИ Широта <> 0 Тогда
		СтрокаГеокода = Формат(Широта, "ЧРД=.") + ", " + Формат(Долгота, "ЧРД=.");
		
	КонецЕсли;
			
	ПараметрыГеокода = Новый Структура;
	ПараметрыГеокода.Вставить("СкрытьПанельАдресов", Ложь);
	ПараметрыГеокода.Вставить("Адрес", ?(ПустаяСтрока(СтрокаГеокода),ПредставлениеАдреса,СтрокаГеокода));
	ПараметрыГеокода.Вставить("РежимВыбора", Истина);
	ПараметрыГеокода.Вставить("РежимПоискОбъектов", Истина);
	//*** Описание
	//*** 12 апреля 2019 г.
	//*** Блашин Евгений Игоревич VOG Начало
	...
	Описание = Новый ОписаниеОповещения("ОповещениеОбработчикВыборДанныхАдреса", ЭтаФорма, СразуВыбирать);
	
	ОткрытьФорму("Обработка.вогГеокодирование.Форма.Форма", ПараметрыГеокода, ЭтаФорма,,,, Описание);
	
КонецПроцедуры

&НаКлиенте
Процедура ОповещениеОбработчикВыборДанныхАдреса(Значение, СразуВыбирать) Экспорт 

	Если Значение = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Значение.Адрес) Тогда
		
		Если НЕ РазрешитьВводАдресаВСвободнойФорме Тогда
			ВвестиАдресВСвободнойФормеЗавершение(КодВозвратаДиалога.Да, Неопределено);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Значение.Индекс) Тогда
			Индекс = Значение.Индекс;
		КонецЕсли;
		
		ПредставлениеАдреса = Значение.Адрес;
		//*** Описание
		//*** 12 апреля 2019 г.
		//*** Блашин Евгений Игоревич VOG Начало
		Широта				= Значение.Широта;
		Долгота				= Значение.Долгота;
		//*** Блашин Евгений Игоревич VOG Окончание 
		...
		Если СразуВыбирать Тогда
			ПодтвердитьИЗакрыть();
		КонецЕсли;
		
	КонецЕсли;
	
КонецПроцедуры // ОповещениеОбработчикВыборДанныхАдреса()
...
Процедура ПредставлениеАдресаПриИзмененииСервер()
	...
	Если Страна = ОсновнаяСтрана Тогда
		...
		Адрес 					= УправлениеКонтактнойИнформацией.КонтактнаяИнформацияПоПредставлению(ПредставлениеАдреса, ВидКонтактнойИнформации);
		ДанныеАдреса 			= УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(Адрес, ВидКонтактнойИнформации.Тип);
		ДанныеАдреса.value 		= ПредставлениеАдреса;
		НаселенныйПунктДетально = РаботаСАдресами.ПодготовитьАдресДляВвода(ДанныеАдреса);
		// До изменения
		//XDTOКонтактная = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияПоПредставлению(ПредставлениеАдреса, ВидКонтактнойИнформации);
