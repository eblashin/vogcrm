...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.вогНастройкиУправленияКлассификацией") Тогда
		ТекущаяНастройка["СписокТорговыеТочки"] = ВыбранноеЗначение;	
...
Процедура ВыбратьНастройку(Команда)
	...
	Отбор.Вставить("ИмяСписка", "СписокТорговыеТочки");
...
Процедура СохранитьНастройку(Команда)
	...
	//Основные настройки
	ПараметрыНастройки.Вставить("ВариантСписка", ТекущийВариантСписка["СписокТорговыеТочки"]);
	...
	ОткрытьФорму("Справочник.вогНастройкиУправленияКлассификацией.Форма.СохранениеНастройки",
		Новый Структура("ИмяСписка, Настройки", "СписокТорговыеТочки", ПараметрыНастройки), ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОткрытьКорзину(Команда)
	...
	Для каждого НайденнаяСтрока Из Объект.Корзина Цикл
	...
	Оповещение = Новый ОписаниеОповещения("ПослеРедактированияКорзины", ЭтотОбъект);
...
Процедура ДобавитьВКорзину(Команда)
	...
	ДобавитьВКорзинуСервер(МассивСтрок);
...
Процедура Подключаемый_ДобавитьВариантСписка(Команда)
	...
	ЗначенияЗаполнения.Вставить("ИмяСписка", "СписокТорговыеТочки");
...
&НаКлиенте
Процедура Подключаемый_СформироватьЭтапПоМоделиКейса(Команда)
		
	ИдентификаторМоделиКейса = СтрЗаменить(Команда.Имя, "МодельКейса", "");
	ИдентификаторМоделиКейса = СтрЗаменить(ИдентификаторМоделиКейса, "_", "-");
	
	СсылкаЭлемента = CRM_УправлениеПроектамиСервер.СсылкаЭлемента(Новый УникальныйИдентификатор(ИдентификаторМоделиКейса));
	
	ВыделенныеСтроки = Элементы.Список.ВыделенныеСтроки;
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны торговые точки этапа. '"));	
	  	Возврат;
		
	КонецЕсли;
	
	ЗначенияЗаполнения = Новый Структура;
	ЗначенияЗаполнения.Вставить("Тема", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1: %2'"),
		СсылкаЭлемента, ?(ЗначениеЗаполнено(ОтборБизнесРегион), ОтборБизнесРегион, НСтр("ru = 'регион не указан'")))
	);	
	
	ЗначенияЗаполнения.Вставить("БизнесРегион", ОтборБизнесРегион);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗначенияЗаполнения", ЗначенияЗаполнения);
	ПараметрыФормы.Вставить("МассивКлиентов"    , ВыделенныеСтроки);
	ПараметрыФормы.Вставить("МодельКейса"		, СсылкаЭлемента);
	
	ОткрытьФорму("Документ.CRM_ЭтапКалендарногоПлана.Форма.ФормаСозданияЭтапа", 
		ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры // Подключаемый_СформироватьЭтапПоМоделиКейса()
...
Процедура УстановитьБыстрыеОтборы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, 
		"БизнесРегион", ОтборБизнесРегион, ВидСравненияКомпоновкиДанных.ВИерархии,, ЗначениеЗаполнено(ОтборБизнесРегион)
	);
...
Процедура УстановитьРежимПросмотраСписка()
	...
	Если ОтборСписокЗначенийКлассификаторов.Количество() > 0
	  ...
	  ИЛИ ЗначениеЗаполнено(ОтборВидТорговойТочки)
...
Функция ПолучитьПараметрыСписка()
	...
	ПараметрыСписка.Вставить("ИмяСписка"      	 , "СписокТорговыеТочки");
	ПараметрыСписка.Вставить("ОсновнаяТаблица"	 , "Справочник.вогТорговыеТочки");
...
&НаСервере
Процедура ДобавитьВКорзинуСервер(МассивСтрок)

	Для каждого Строка Из МассивСтрок Цикл
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("Ссылка"   , Строка);
		СтруктураПоиска.Вставить("ИмяСписка", "СписокТорговыеТочки");
...
&НаКлиенте
Процедура ПослеРедактированияКорзины(Корзина, Контекст) Экспорт
...
&НаКлиентеНаСервереБезКонтекста
Процедура СформироватьПредставлениеКорзины(Форма)
	
	ЭлементовКорзины = Форма.Объект.Корзина.Количество();
...
Процедура ИнициализироватьФормуПоНастройкеКлиент()
	...
	Настройка = ПолучитьНастройкиПоСсылке(ТекущаяНастройка["СписокТорговыеТочки"]);
...
Процедура СформироватьЗаголовокПоНастройке(Форма)
	...
	Настройка = Форма.ТекущаяНастройка["СписокТорговыеТочки"];
	Форма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Торговые точки (%1)'"), 
