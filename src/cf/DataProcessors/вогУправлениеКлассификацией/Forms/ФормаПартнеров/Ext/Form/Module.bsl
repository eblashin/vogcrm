...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.вогНастройкиУправленияКлассификацией") Тогда
		ТекущаяНастройка["СписокПартнеры"] = ВыбранноеЗначение;	
...
Процедура НастроитьОтборПоКлассификаторам(Команда)
	...
	ПараметрыФормы.Вставить("Принадлежность"		  , "Справочник.Партнеры");
...
Процедура ВыбратьНастройку(Команда)
	...
	Отбор.Вставить("ИмяСписка", "СписокПартнеры");
...
Процедура СохранитьНастройку(Команда)
	...
	//Основные настройки
	ПараметрыНастройки.Вставить("ВариантСписка", ТекущийВариантСписка["СписокПартнеры"]);
	...
	ОткрытьФорму("Справочник.вогНастройкиУправленияКлассификацией.Форма.СохранениеНастройки",
		Новый Структура("ИмяСписка, Настройки", "СписокПартнеры", ПараметрыНастройки), ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Подключаемый_ДобавитьВариантСписка(Команда)
	...
	ЗначенияЗаполнения.Вставить("ИмяСписка", "СписокПартнеры");
...
Процедура УстановитьБыстрыеОтборы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, 
		"Ссылка.БизнесРегион", ОтборБизнесРегион, ВидСравненияКомпоновкиДанных.ВИерархии,, ЗначениеЗаполнено(ОтборБизнесРегион)
	);
...
Процедура УстановитьРежимПросмотраСписка()
	...
	Если ОтборСписокЗначенийКлассификаторов.Количество() > 0
	  ИЛИ ЗначениеЗаполнено(ОтборТипОтношений)
...
Функция ПолучитьПараметрыСписка()
	...
	ПараметрыСписка.Вставить("ИмяСписка"      	 , "СписокПартнеры");
...
Процедура ИнициализироватьФормуПоНастройкеКлиент()
	...
	Настройка = ПолучитьНастройкиПоСсылке(ТекущаяНастройка["СписокПартнеры"]);
...
Процедура СформироватьЗаголовокПоНастройке(Форма)
	...
	Настройка = Форма.ТекущаяНастройка["СписокПартнеры"];
	Форма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Клиенты (%1)'"), 
