...
Процедура ОбновитьНастройкиИИхОписанияЗначениямиПоУмолчанию(ДляОбменаЧерезПрямоеПодключение = Ложь)
	...
	НастройкиПараметровУчетаПоясняющаяНадпись = ПланыОбмена[Объект.ИмяПланаОбмена].ПояснениеДляНастройкиПараметровУчета();
	...
	НастройкиПараметровУчетаКорреспондентаПоясняющаяНадпись = ОбменДаннымиСервер.ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента(
		Объект.ИмяПланаОбмена,
		ВерсияКонфигурацииКорреспондента);
...
Процедура УстановитьВидимостьНаСервере()
	...
	ПодсказкаДляИзменяемогоПрефикса = Метаданные.Константы.ПрефиксУзлаРаспределеннойИнформационнойБазы.Подсказка;
	Если Не Объект.ПрефиксИнформационнойБазыИсточникаУстановлен Тогда
		Для Каждого ПрефиксИсточникаЭлемент Из ПрефиксИсточникаКоллекцияЭлементов Цикл
			ПрефиксИсточникаЭлемент.ТолькоПросмотр = Ложь;
			ПрефиксИсточникаЭлемент.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
			ПрефиксИсточникаЭлемент.РасширеннаяПодсказка.Заголовок = ПодсказкаДляИзменяемогоПрефикса;
		КонецЦикла;
	КонецЕсли;
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		Элементы.ПрефиксИнформационнойБазыПриемника.РасширеннаяПодсказка.Заголовок = ПодсказкаДляИзменяемогоПрефикса;
	КонецЕсли;
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.ПрефиксИнформационнойБазыИсточника) Тогда
