...
// +CRM
	Если Параметры.Свойство("ТолькоВыгрузка") Тогда
		CRM_ТолькоВыгрузка = Параметры.ТолькоВыгрузка;
		Объект.РежимОбмена = "Выгрузка";
	КонецЕсли;
	
	Если Параметры.Свойство("ТолькоЗагрузка") Тогда
		CRM_ТолькоЗагрузка = Параметры.ТолькоЗагрузка;
		Объект.РежимОбмена = "Загрузка";
	КонецЕсли;
	
	CRM_ИмяТекущейКонфигурации		= Метаданные.Имя;
	CRM_ВерсияТекущейКонфигурации	= Лев(Метаданные.Версия,1);
// -CRM
...
// +CRM
	Если CRM_ТолькоВыгрузка Тогда
		Объект.РежимОбмена = "Выгрузка";
	КонецЕсли;
	
	Если CRM_ТолькоЗагрузка Тогда
		Объект.РежимОбмена = "Загрузка";
	КонецЕсли;
// -CRM
...
// +CRM
	Если CRM_ВерсияКонфигурацииБухгалтерии = "" Тогда
		CRM_ВерсияКонфигурацииБухгалтерии = "2";
	КонецЕсли;
	CRM_ПриИзмененииРежимаВыгрузки();
// -CRM
...
// +CRM
	CRM_МетодыФормОбработокКлиент.CRM_УстановитьСвойстваЭлементовФормы(ЭтотОбъект);
	CRM_ПолучитьМакетСформироватьВременныйФайлПрочитатьПравила();
// -CRM
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,,Истина, УникальныйИдентификатор);
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Если ЭтоКлиент Тогда
		...
		// Удаление временного файла происходит не через УдалитьФайлы(АдресНаСервере), а через УдалитьФайлы(Объект.ИмяФайлаОбмена) далее.
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		Если ЭтоАдресВременногоХранилища(АдресФайлаПравилЗагрузкиВХранилище) Тогда
			...
			АдресНаСервере = ПолучитьИмяВременногоФайла("xml");
			// Удаление временного файла происходит не через УдалитьФайлы(АдресНаСервере), а через УдалитьФайлы(Объект.ИмяФайлаПравилОбмена) далее.
			ДвоичныеДанные.Записать(АдресНаСервере);
...
&НаСервере
Функция CRM_ПолучитьАдресФайлаВХранилище()
	
	Возврат ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайлаПравил), УникальныйИдентификатор);
	
КонецФункции

&НаСервере
Процедура CRM_УдалитьВременныйФайлПравил()
	
	Если НЕ CRM_ИмяВременногоФайлаПравил = "" Тогда
		Попытка
...
&НаСервере
Процедура CRM_ПолучитьМакетСформироватьВременныйФайл()
	...
	Если CRM_ИмяТекущейКонфигурации = "БухгалтерияПредприятия" И CRM_ВерсияТекущейКонфигурации = "3" Тогда
		
		ИмяМакета = "БП30_CRM30";
		
		ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
		
		МакетПравилОбмена = ОбработкаОбъект.ПолучитьМакет(ИмяМакета);
		МакетПравилОбмена.Записать(ИмяВременногоФайлаПравилОбмена);
		ИмяФайлаПравил = ИмяВременногоФайлаПравилОбмена;
		CRM_ИмяВременногоФайлаПравил = ИмяВременногоФайлаПравилОбмена;
		Объект.ИмяФайлаПравилОбмена = ИмяВременногоФайлаПравилОбмена;
		
	ИначеЕсли CRM_ИмяТекущейКонфигурации = "CRM" И CRM_ВерсияТекущейКонфигурации = "3" Тогда
		
		Если CRM_ВерсияКонфигурацииБухгалтерии = "3" Тогда
			ИмяМакета = "CRM30_БП30";
		ИначеЕсли CRM_ВерсияКонфигурацииБухгалтерии = "2" Тогда
			ИмяМакета = "CRM30_БП20";
		КонецЕсли;
...
&НаКлиенте
Процедура CRM_ПолучитьМакетСформироватьВременныйФайлПрочитатьПравила()
	
	CRM_ПолучитьМакетСформироватьВременныйФайл();
	
	Если НЕ ИмяФайлаПравил = "" Тогда
		...
		Если ЭтоКлиент Тогда
			...
			АдресФайлаПравилВХранилище	= CRM_ПолучитьАдресФайлаВХранилище();
...
&НаСервере
Процедура CRM_УстановитьЗначениеПараметра()
	
	ТаблицаПараметров = РеквизитФормыВЗначение("Объект.ТаблицаНастройкиПараметров");
	
	СтрокаТаблицы = ТаблицаПараметров.Найти("Организация","Наименование");
	Если СтрокаТаблицы <> Неопределено Тогда
		СтрокаТаблицы.Значение = CRM_Организация;
	КонецЕсли;
...
&НаКлиенте
Процедура CRM_ОрганизацияПриИзменении(Элемент)
	
	CRM_УстановитьЗначениеПараметра();
	
КонецПроцедуры

&НаКлиенте
Процедура CRM_ОрганизацияОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура CRM_ПриИзмененииРежимаВыгрузки(ВыполнитьЗагрузкуПравилОбмена = Ложь)
	
	ГруппаРежима = КоманднаяПанель.ПодчиненныеЭлементы.РежимВыгрузки.ПодчиненныеЭлементы;
	
	ГруппаРежима.ФормаCRM_БП_2.Пометка	= (CRM_ВерсияКонфигурацииБухгалтерии = "2");
	ГруппаРежима.ФормаCRM_БП_3.Пометка	= (CRM_ВерсияКонфигурацииБухгалтерии = "3");
	
	Если CRM_ВерсияКонфигурацииБухгалтерии = "2" Тогда
		КоманднаяПанель.ПодчиненныеЭлементы.РежимВыгрузки.Заголовок = ГруппаРежима.ФормаCRM_БП_2.Заголовок;
	КонецЕсли;
	
	Если CRM_ВерсияКонфигурацииБухгалтерии = "3" Тогда
		КоманднаяПанель.ПодчиненныеЭлементы.РежимВыгрузки.Заголовок = ГруппаРежима.ФормаCRM_БП_3.Заголовок;
	КонецЕсли;
	
	Если ВыполнитьЗагрузкуПравилОбмена Тогда
		CRM_УдалитьВременныйФайлПравил();
		CRM_ПолучитьМакетСформироватьВременныйФайлПрочитатьПравила();
	КонецЕсли;
...
Процедура CRM_БП_2(Команда)
	...
	CRM_ВерсияКонфигурацииБухгалтерии = "2";
	CRM_ПриИзмененииРежимаВыгрузки(Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура CRM_БП_3(Команда)
	
	Если CRM_ВерсияКонфигурацииБухгалтерии = "3" Тогда
		Возврат;
	КонецЕсли;
	
	CRM_ВерсияКонфигурацииБухгалтерии = "3";
	CRM_ПриИзмененииРежимаВыгрузки(Истина);
