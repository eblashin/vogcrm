...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ВидФайла", 				ВидФайла);
	
	Если Не ПустаяСтрока(ИдентификаторГруппы) Тогда
		
		ВозможностьСоздаватьГруппыФайлов = Ложь;
		
		ГруппаФайловСсылка = ПолучитьГруппуФайлов(Ложь);
...
Процедура ПриОткрытии(Отказ)
	...
	// ++ VOG Солодов В.В. 04.08.2020 CRM-572
	Если Не ПустаяСтрока(ИдентификаторГруппы) Тогда
		
		Элементы.Список.ВыделенныеСтроки.Очистить();
		
		Если СсылкаСуществует(ГруппаФайловСсылка) Тогда
			Элементы.Список.ТекущаяСтрока = ГруппаФайловСсылка;
		КонецЕсли;
...
Процедура СписокПриИзменении(Элемент)
	Если Элемент.ВыделенныеСтроки.Количество() > 0 Тогда
		// ++ VOG Солодов В.В. 23.07.2020 CRM-572
		Если Не ПустаяСтрока(ИдентификаторГруппы) Тогда
			
			СтруктураОповещения = Новый Структура;
			СтруктураОповещения.Вставить("Событие", 		"ДанныеФайлаИзменены");
			СтруктураОповещения.Вставить("ВладелецФайла", 	ВладелецФайла);
			
			Оповестить("Запись_Файл", СтруктураОповещения, Элемент.ВыделенныеСтроки);
		...
		Оповестить("Запись_Файл", 
				Новый Структура("Событие", "ДанныеФайлаИзменены"), Элемент.ВыделенныеСтроки);
	КонецЕсли;
КонецПроцедуры
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Копирование Тогда
		...
		КонецЕсли;
	Иначе
		// ++ VOG Солодов В.В. 21.07.2020 CRM-572
		Если Не ПустаяСтрока(ИдентификаторГруппы) Тогда
			ГруппаФайлов = ПолучитьГруппуФайлов(Истина);
		Иначе
		// -- VOG Солодов В.В. 21.07.2020 CRM-572
			ТекущиеДанные = Элементы.Список.ТекущиеДанные;
...
Процедура ЗакончитьРедактирование(Команда)
	...
	ИначеЕсли МассивФайлов.Количество() = 1 Тогда 
		...
		ПараметрыОбновленияФайлов.Вставить("ХранитьВерсии",                    ДанныеСтроки.ХранитьВерсии);
		...
		ПараметрыОбновленияФайлов.Вставить("Редактирует",                        ДанныеСтроки.РедактируетПользователь);
...
Процедура Освободить(Команда)
	...
	Для Каждого ЭлементСписка Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		Если Не РаботаСФайламиСлужебныйКлиент.ВозможностьОсвободитьФайл(
				...
				ДанныеСтроки.РедактируетПользователь) Тогда
			Продолжить;
		КонецЕсли;
		МассивФайлов.Добавить(ДанныеСтроки.Ссылка);
	КонецЦикла;
	
	ОсвободитьФайлы(МассивФайлов);
...
Процедура УстановитьУсловноеОформление()
	...
	Если Не ПустаяСтрока(ИдентификаторГруппы) Тогда
		...
		ОтборЭлемента.ПравоеЗначение 	= ГруппаФайловСсылка;
...
Процедура ОсвободитьФайлы(Знач МассивФайлов)
...
// ++ VOG Солодов В.В. 22.07.2020 CRM-572
&НаСервере
Функция ПолучитьГруппуФайлов(ВозвращатьПараметры)
	
	ГруппаФайлов = ПолучитьПараметрыГруппыФайлов(ВозвращатьПараметры, Ложь);
	
	Если ЗначениеЗаполнено(ВидФайла) Тогда
		
		Если ТипЗнч(ГруппаФайлов) = Тип("Структура") Тогда
			ГруппаФайлов.Вставить("ВидФайла", 		ВидФайла);
			ГруппаФайлов.Вставить("ГруппаСсылка", 	ГруппаФайловСсылка);
		Иначе
			ГруппаФайлов = ПолучитьПараметрыГруппыФайлов(ВозвращатьПараметры, Истина, ГруппаФайлов);
		КонецЕсли;
...
&НаСервере
Функция ПолучитьПараметрыГруппыФайлов(ВозвращатьПараметры, ПоВидуФайла, РодительГруппы = Неопределено)
	
	ИмяСправочника = РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайла);
	
	Если ПоВидуФайла Тогда
		
		НаименованиеВидаФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидФайла, "Наименование", Истина);
		ГруппаСсылка = Справочники[ИмяСправочника].НайтиПоНаименованию(НаименованиеВидаФайла, Истина, РодительГруппы, ВладелецФайла);
		
		Если ВозвращатьПараметры Тогда
			
			ПараметрыГруппы = Новый Структура;
			ПараметрыГруппы.Вставить("ВидФайла", 		ВидФайла);
			ПараметрыГруппы.Вставить("Родитель", 		РодительГруппы);
			ПараметрыГруппы.Вставить("ГруппаСсылка", 	ГруппаФайловСсылка);
			
			Возврат ПараметрыГруппы;
			
		ИначеЕсли Не ЗначениеЗаполнено(ГруппаСсылка) Тогда
			
			ГруппаСсылка 	= Справочники[ИмяСправочника].ПолучитьСсылку();
			
		КонецЕсли;
		
	Иначе
		УИД_Группы 		= Новый УникальныйИдентификатор(СтрЗаменить(ИдентификаторГруппы, "_", "-"));
		ГруппаСсылка 	= Справочники[ИмяСправочника].ПолучитьСсылку(УИД_Группы);
	КонецЕсли;
	...
	СсылкаСуществует 	= ОбщегоНазначения.СсылкаСуществует(ГруппаСсылка);
	
	Если Не СсылкаСуществует И ВозвращатьПараметры Тогда
		
		ГруппаСсылка = Новый Структура;
		ГруппаСсылка.Вставить("УникальныйИдентификатор", 	ИдентификаторГруппы);
		ГруппаСсылка.Вставить("Наименование", 				НаименованиеГруппы);
		
	ИначеЕсли СсылкаСуществует Тогда
		
		НаименованиеГруппыПоСсылке = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаСсылка, "Наименование", Истина);
		
		Если Не НаименованиеГруппы = НаименованиеГруппыПоСсылке Тогда
			
			ГруппаФайлов = ГруппаСсылка.ПолучитьОбъект();
			ГруппаФайлов.Наименование = НаименованиеГруппы;
