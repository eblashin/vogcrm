...
&НаКлиенте
Перем ПараметрыФормыЭлементаСправочникаФайлы;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ПрисоединенныйФайл) = Тип("СправочникСсылка.Файлы") Тогда
		ОткрытьФормуЭлементаСправочникаФайлы = Истина;
		Возврат;
	КонецЕсли;
	...
	ДоступнаПечатьСоШтампом = ДоступнаЭлектроннаяПодпись
	...
	Элементы.ГруппаСертификатыШифрования.Видимость                     = ДоступнаЭлектроннаяПодпись;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОткрытьФормуЭлементаСправочникаФайлы Тогда
		Отказ = Истина;
		ПараметрыФормыЭлементаСправочникаФайлы = Новый Структура(
			"ЗначениеКопирования, Ключ, ТолькоПросмотр, ПрисоединенныйФайл,
			|ВладелецФайла, ПараметрыОтправки, РежимСоздания, ЭтоНовый");
		
		ЗаполнитьЗначенияСвойств(ПараметрыФормыЭлементаСправочникаФайлы, Параметры);
		СтандартныеПодсистемыКлиент.УстановитьХранениеФормы(ЭтотОбъект, Истина);
		ПодключитьОбработчикОжидания("ОткрытьФормуЭлементаСправочникаФайлаОбработчикОжидания", 0.1, Истина);
		Возврат;
	КонецЕсли;
...
Процедура Подписать(Команда)
	...
	МассивФайлов.Добавить(ЭтотОбъект.Объект.Ссылка);
...
&НаКлиенте
Процедура ОткрытьФормуЭлементаСправочникаФайлаОбработчикОжидания()
	
	СтандартныеПодсистемыКлиент.УстановитьХранениеФормы(ЭтотОбъект, Ложь);
	ОткрытьФорму("Справочник.Файлы.Форма.ФормаЭлемента", ПараметрыФормыЭлементаСправочникаФайлы);
...
Функция ПолучитьДоступныеКоманды(ФайлРедактируется,
	...
	Иначе
		УдалитьКомандуИзМассива(ИменаКоманд, "ЗакончитьРедактирование");
		УдалитьКомандуИзМассива(ИменаКоманд, "Освободить");
		УдалитьКомандуИзМассива(ИменаКоманд, "СохранитьИзменения");
	КонецЕсли;
