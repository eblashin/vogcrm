...
&НаСервере
Функция ПолучитьМассивЭлементовПВХОбъектыЖурналаПоОписаниюТипов(ОписаниеТипов, ПолучатьПомеченныеНаУдаление = Истина) 
...
Процедура ЗаполнитьДоступныеДляСозданияОбъекты()
	...
	МассивСсылкаПВХ = ПолучитьМассивЭлементовПВХОбъектыЖурналаПоОписаниюТипов(ОписаниеТипов);
	Для Каждого СсылкаПВХ Из МассивСсылкаПВХ Цикл
		МассивОбъектыЗапрещенныеДляСоздания.Добавить(СсылкаПВХ);
	КонецЦикла;
	
	//Убираем Задачи, События, Сделки и Звонки
	МассивТипов = Новый Массив();
	...
	МассивТипов = Новый Массив();
	МассивТипов.Добавить(Тип("ДокументСсылка.удалитьCRM_Сделка"));
	ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
	МассивСсылкаПВХ = ПолучитьМассивЭлементовПВХОбъектыЖурналаПоОписаниюТипов(ОписаниеТипов);
	Для Каждого СсылкаПВХ Из МассивСсылкаПВХ Цикл
		МассивОбъектыЗапрещенныеДляСоздания.Добавить(СсылкаПВХ);
	...
	МассивТипов = Новый Массив();
	МассивТипов.Добавить(Тип("ДокументСсылка.УдалитьCRM_Событие"));
	ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
	МассивСсылкаПВХ = ПолучитьМассивЭлементовПВХОбъектыЖурналаПоОписаниюТипов(ОписаниеТипов);
	Для Каждого СсылкаПВХ Из МассивСсылкаПВХ Цикл
		МассивОбъектыЗапрещенныеДляСоздания.Добавить(СсылкаПВХ);
...
Процедура КомандаПоказатьСкрытьПолеОтображенияСодержания(Команда)
	...
	ПоказатьПолеОтображенияСодержания = Элементы.КнопкаПоказатьСкрытьПолеОтображенияСодержания.Пометка;
...
&НаКлиенте
// Процедура - обработчик команды "Подключаемый_КомандаВидСписка".
//
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.ПодменюВидСписка.Видимость Тогда
		// Оформление списка
...
Процедура ПриОткрытии(Отказ)
	...
	// -Пользовательские настройки.
	// Устанавливаем видимость поля отображения содержания.
	Элементы.КнопкаПоказатьСкрытьПолеОтображенияСодержания.Пометка = НЕ ПоказатьПолеОтображенияСодержания;
	КомандаПоказатьСкрытьПолеОтображенияСодержания(Неопределено);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	ПервыйВызов = Ложь;
	Если НЕ ЗначениеЗаполнено(ТекущийВидЖурнала) Тогда
		ТекущийВидЖурнала	= Настройки.Получить("ТекущийВидЖурнала");
	КонецЕсли;	
	Если НЕ ЗначениеЗаполнено(ОтборПоТипуОбъекта) Тогда
		ОтборПоТипуОбъекта	= Настройки.Получить("ОтборПоТипуОбъекта");
	КонецЕсли;	
	ЗаполнитьДоступныеДляОтбораОбъекты();
	Если ЗначениеЗаполнено(ТекущийВидЖурнала) Тогда		
		Если ЗначениеЗаполнено(ОтборПоТипуОбъекта) Тогда
	...
	Если НЕ ЗначениеЗаполнено(ОтборОтветственный) Тогда
		...
		Если ТипЗнч(ОтветственныйПоУмолчанию) = Тип("СправочникСсылка.Пользователи") Тогда
			ОтборОтветственный = ОтветственныйПоУмолчанию;
		КонецЕсли;
	КонецЕсли;
	Если ЗначениеЗаполнено(ОтборОтветственный) Тогда
		CRM_ОбщегоНазначенияКлиентСервер.ИзменитьЭлементОтбораСписка(Список, "Ответственный", ОтборОтветственный, Истина);		
