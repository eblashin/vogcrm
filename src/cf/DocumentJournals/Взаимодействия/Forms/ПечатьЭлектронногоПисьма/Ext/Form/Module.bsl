...
Процедура СохранитьВложение(Команда)
	...
	СтруктураНаименованиеИРасширениеФайла = ВзаимодействияКлиентСервер.НаименованиеИРасширениеФайлаПоИмениФайла(ТекущиеДанные.ИмяФайла);
	...
	ДанныеФайла.Вставить("Наименование",                       СтруктураНаименованиеИРасширениеФайла.Наименование);
	ДанныеФайла.Вставить("Расширение",                         СтруктураНаименованиеИРасширениеФайла.Расширение);
...
Процедура ОткрытьВложениеПисьма()
	...
	ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		...
		Если ВзаимодействияКлиентСервер.ЯвляетсяЭлектроннымПисьмом(ТекущиеДанные.Ссылка)
...
Процедура ЗаполнитьВложенияИСФормироватьТекстHTMLНаОснованииПисьмаВложения(ПисьмоВложение)
	...
	Если ПочтовоеСообщение.СтатусРазбора = СтатусРазбораИнтернетПочтовогоСообщения.ОбнаруженыОшибки Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не удалось разобрать почтовое сообщение.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Для Каждого Вложение Из ПочтовоеСообщение.Вложения Цикл
		...
		Если НЕ ПустаяСтрока(Вложение.Идентификатор) 
			...
			НоваяСтрока.Расширение                = ВзаимодействияКлиентСервер.РасширениеФайла(Вложение.ИмяФайла);
		...
		Расширение = ВзаимодействияКлиентСервер.РасширениеФайла(НоваяСтрока.ИмяФайла);
	...
	ДокументHTML = Взаимодействия.СформироватьДокументHTMLНаОснованииПисьма(СтруктураПисьмо,
	                                                                        СтруктураПисьмо.ТипТекста,
	                                                                        СтруктураПисьмо.Текст,
	                                                                        СтруктураПисьмо.ТекстHTML,
	                                                                        СтруктураПисьмо.ТипТекста,
	                                                                        СтруктураПисьмо.Кодировка);
...
Процедура СформироватьЗаголовокФормы(Письмо)
	...
	Заголовок = ВзаимодействияКлиентСервер.ПредставлениеПисьма(ТемаПисьма, ДатаПисьма);
...
Процедура СформироватьСтрокуПисьмаОснования();
	...
	ПредставлениеПисьма = ВзаимодействияКлиентСервер.ПредставлениеПисьма( ТемаПисьмаОснования, ДатаПисьмаОснования);
