...
////////////////////////////////////////////////////////////////////////////////
// Функции для обработки действий пользователя в процессе редактирования
// числа, например ОКПО в документах.

// Открывает форму редактирования числа .
//
// Параметры:
//  ФормаВлалец        - УправляемаяФорма - форма, в поле которой выполняется ввод комментария.
//  ИмяРеквизита       - Строка - имя реквизита формы, в который будет помещен введенное пользователем число.
//  Подсказка          - Строка - текст, который необходимо отобразить в заголовке формы.
//  Длина              - Число - Длина числа;
//  ТочностьЧила       - Число - Точность числа, которое необходимо отредактировать;
//
Процедура ПоказатьФормуРедактированияЧисла(Знач ФормаВладелец,Знач ИмяРеквизита, Знач Подсказка = "Введите число",Знач ДлинаЧисла = 15, Знач ТочностьЧила = 0) Экспорт
	
	ДополнительныеПараметры = Новый Структура("ФормаВладелец,ИмяРеквизита", ФормаВладелец, ИмяРеквизита);
	Оповещение = Новый ОписаниеОповещения("ВводЧислаЗавершениеВвода", 
	ОбщегоНазначенияСлужебныйКлиент, ДополнительныеПараметры);
	ПодсказкаВвода = НСтр("ru = '%1'");
	ПодсказкаВвода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПодсказкаВвода,Подсказка);
	ПоказатьВводЧисла(Оповещение,0,ПодсказкаВвода,ДлинаЧисла,ТочностьЧила);
...
Функция ПредлагатьСоздаватьРезервныеКопии() Экспорт
	...
	Результат = Ложь;
	ИнтеграцияСтандартныхПодсистемКлиент.ПриПроверкеВозможностиРезервногоКопированияВПользовательскомРежиме(Результат);
	Возврат Результат;
...
Процедура ПредложитьПользователюСоздатьРезервнуюКопию() Экспорт
	...
	ИнтеграцияСтандартныхПодсистемКлиент.ПриПредложенииПользователюСоздатьРезервнуюКопию();
