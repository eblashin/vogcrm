...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
	...
	Исключение
		...
		ТекстСообщенияДляПользователя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в правилах регистрации объекта: %1. Подробнее см. в журнале регистрации.'"),
			ИмяПланаОбмена);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияДляПользователя);
...
Процедура ПроверитьКоллизиюИзмененийДанных(ЭлементДанных, ПолучениеЭлемента, Знач Отправитель, Знач ЭтоПолучениеОтГлавного)
	...
	Если ТипЗнч(Отправитель) = Тип("ПланОбменаСсылка.вогОбменСУчетнойСистемой") Тогда 
		Возврат;
	КонецЕсли;
	//вог-
