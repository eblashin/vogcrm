...
Процедура ПрикрепитьОбластьДанных(Знач КодОбластиДанных, Знач СписокПользователей, Знач ПредставлениеПриложения, Знач ЧасовойПоясПриложения) Экспорт
	...
	Попытка
		...
		Для каждого ОписаниеПользователя Из СписокПользователей Цикл
			...
			ПоддержкаАутентификацииОС =  
				ОписаниеПользователя.Тип().URIПространстваИмен = СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_7.Пакет();
			...
			Если ПоддержкаАутентификацииОС Тогда
				ОписаниеПользователяИБ.АутентификацияОС = ОписаниеПользователя.OSAuthentication;
				ОписаниеПользователяИБ.ПользовательОС   = ОписаниеПользователя.OSUser;
			КонецЕсли;
