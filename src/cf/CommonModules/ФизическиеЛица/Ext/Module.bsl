...
//
//START Кайдашов 18/02/20 CRM-367 
Функция Просклонять(Знач ФИО, Падеж, Результат, Пол = Неопределено,НеСклонятьФИО = Ложь) Экспорт
	//+++ Терпогосян Д.Б. [06.08.2021 9:37:02] № DEV-813
	Если Пол = Перечисления.ПолФизическогоЛица.Мужской Тогда лПол = 1; ИначеЕсли Пол = Перечисления.ПолФизическогоЛица.Женский Тогда  лПол = 2; Иначе лПол = 0; КонецЕсли;
	Результат = СклонениеПредставленийОбъектов.ПросклонятьФИО(ФИО, Падеж, ,лПол); 
	Возврат Истина; 
	//--- Терпогосян Д.Б. [06.08.2021 9:37:02] № DEV-813 
	
	Если ТипЗнч(НеСклонятьФИО) = Тип("Булево") тогда
		НеСклонять = НеСклонятьФИО;
	Иначе
		НеСклонять = Ложь;
	КонецЕсли;	
	...
	Иначе
		Начало = 1; 
	КонецЕсли;
	Для Номер = Начало По Мин(МассивСтрок.Количество(), 3) Цикл
	...
	Если НеСклонять тогда
		Результат = СокрЛП(МассивСтрок[0])+" "+СокрЛП(Результат);
	Иначе
