...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Подсистема = Неопределено
		...
		Разделы = МодульТекущиеДелаСервер.РазделыДляОбъекта("Справочник.ДополнительныеОтчетыИОбработки2");
	Иначе
...
Функция ВыгрузитьОтчетыИОбработкиВФайлы(ОтчетыИОбработки)
	...
	Параметры.Вставить("РабочийКаталог", СтандартныеПодсистемыСервер.СоздатьКаталогВременныхФайлов("ARADP"));
	...
	Пока ОбратныйИндекс > 0 Цикл
		...
		//  /LoadExternalDataProcessorOrReportFromFiles <ПутьКФайлуВыгрузки> <ПутьКОбработке>
		//  - создание внешних отчетов и обработок из XML.
		//    <ПутьКФайлуВыгрузки> - путь к файлу в формате XML, из которого требуется создать внешний отчет или обработку.
		//    <ПутьКОбработке> - путь к файлу внешней обработки (.epf) или отчета (.erf), в который будет записан результат
		//                       загрузки из XML-файла.
		//  Дополнительно можно указать адрес строки подключения к информационной базе, например, с помощью параметра /IBName.
		//  При этом, если во внешней обработке есть внешние ссылки, они будут разрешаться относительно указанной
		//  информационной базы. Если информационная база не указана, внешние ссылки разрешаться не будут.
	...
	Если Параметры.КаталогКопии1CD <> Неопределено Тогда
		СтандартныеПодсистемыСервер.ОчиститьКаталогВременныхФайлов(Параметры.КаталогКопии1CD);
	КонецЕсли;
	СтандартныеПодсистемыСервер.ОчиститьКаталогВременныхФайлов(Параметры.РабочийКаталог);
...
Функция ПакетныйЗапускКонфигуратора(Параметры, КлючЗапуска)
	...
	КодВозврата = NULL;
	ЗапуститьПриложение(Кавычка + Параметры.КаталогBIN + "1cv8.exe" + Кавычка + " DESIGNER"
	...
	СтандартныеПодсистемыСервер.ОчиститьКаталогВременныхФайлов(ИмяФайлаСообщений);
	Возврат Результат;
КонецФункции
