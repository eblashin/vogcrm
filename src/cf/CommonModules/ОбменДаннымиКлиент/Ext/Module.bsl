...
Процедура УдалитьНастройкуСинхронизации(Знач УзелИнформационнойБазы) Экспорт
	...
	Иначе
		...
		// В режиме сервиса с помощью этой процедуры выполняется удаление настройки обмена с локальной версией.
		// В этом случае вместо "Удалить настройку" используем оборот "Отключить синхронизацию".
		ПараметрыРаботыКлиента = СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиентаПриЗапуске();
		Если ПараметрыРаботыКлиента.РазделениеВключено Тогда
			ТекстВопроса = НСтр("ru = 'Отключить синхронизацию с %1?'");
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(УзелИнформационнойБазы));
		КонецЕсли;
