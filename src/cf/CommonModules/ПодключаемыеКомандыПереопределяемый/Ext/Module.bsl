...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	//+CRM
	CRM_ПодключаемыеКоманды.ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды);
	//-CRM
	
	// ++ VOG Солодов В.В. 24.01.2020 CRM-106
	ПроверяемыйТип = Тип("ДокументСсылка.вогАнкета");
	
	Для Каждого Команда Из Команды Цикл
		
		Если Не Команда.ТипПараметра = Неопределено
			И Команда.ТипПараметра.СодержитТип(ПроверяемыйТип)
			И Команда.ДополнительныеПараметры.Свойство("ДополнительныеПараметры")
			И Команда.ДополнительныеПараметры.ДополнительныеПараметры.Свойство("Ссылка") Тогда
			
			ВнешняяОбработка 	= Команда.ДополнительныеПараметры.ДополнительныеПараметры.Ссылка;
			ОбъектОбработки 	= ДополнительныеОтчетыИОбработки.ОбъектВнешнейОбработки(ВнешняяОбработка);
			...
			СтруктураОтбора.Вставить("Идентификатор", Команда.Идентификатор);
			
			НайденныеКоманды = РегистрационныеДанные.Команды.НайтиСтроки(СтруктураОтбора);
			
			Для Каждого СтрокаКоманда Из НайденныеКоманды Цикл
				
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаКоманда, "УсловияВидимости")
					И СтрокаКоманда.УсловияВидимости.Количество() > 0 Тогда
					
					Команда.ЕстьУсловияВидимости = Истина;
					
					Для Каждого УсловиеВидимости Из СтрокаКоманда.УсловияВидимости Цикл
