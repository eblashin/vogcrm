		...
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			Если ТипЗнч(Данные) = Тип("УдалениеОбъекта") Тогда
				...
				Если Не ОбщегоНазначения.СсылкаСуществует(Ссылка) Тогда
					Продолжить;
				КонецЕсли;
				Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(Ссылка.Метаданные(), РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных()) Тогда
					ОбластьДанных = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Данные.Ссылка, РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных());
				КонецЕсли;
				
			Иначе
				
				Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(Данные.Метаданные(), РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных()) Тогда
					ОбластьДанных = Данные[РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных()];
				КонецЕсли;
...
Процедура ОбработатьОчередьСообщенийСистемы(Отбор = Неопределено) Экспорт
    ...
    Для Каждого СправочникСообщений Из СправочникиСообщений Цикл
        ...
        ЭтоНеразделенныйСправочник = Не РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация()
			ИЛИ Не РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ПолноеИмяСправочника, РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных());
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Попытка
            ...
            Если ОбрабатыватьСообщенияВНеразделенныхДанных И РаботаВМоделиСервисаПовтИсп.РазделениеВключено() И РаботаВМоделиСервисаПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
...
Функция НаименованиеЭтогоУзлаПоУмолчанию() Экспорт
	...
	Возврат ?(РаботаВМоделиСервисаПовтИсп.РазделениеВключено(), Метаданные.Синоним, ОбменДаннымиПовтИсп.ИмяЭтойИнформационнойБазы());
...
Функция ОбработчикиСообщений()
	...
	ИнтеграцияСтандартныхПодсистем.ПриОпределенииОбработчиковКаналовСообщений(Результат);
