...
Процедура ФорматированиеРезультатовАвтоподбора(ДанныеВыбора, Знач Текст) Экспорт
	...
	Для каждого СтрокаДанных Из ДанныеВыбора Цикл
		...
		Если ТипЗнч(СтрокаДанных.Значение) = Тип("Структура")
			...
			// ++ VOG Солодов В.В. 06.03.2020 CRM-196
			УстаревшийАдрес = (Не СтрокаДанных.Значение.Муниципальный И ДоступноМуниципальноеДеление);
			// До изменения
...
Функция СформироватьАдресПоПредставлению(Представление, РазбиватьНаПоля = Ложь)
	...
	//Кайдашов +++
	Для каждого Строка из ДанныеАнализа цикл
		
		Если Строка.Сокращение = "" тогда
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |ГДЕ
			               |	вогНаселенныеПункты.Наименование = &Наименование
			               |	И вогНаселенныеПункты.ТипАдреса = &ТипАдреса";
			Запрос.УстановитьПараметр("Наименование",Строка.Наименование);
			Запрос.УстановитьПараметр("ТипАдреса","НаселенныйПункт");
			Выборка = Запрос.Выполнить().Выбрать();
			Если Выборка.Следующий() тогда
				Строка.Сокращение = "город";
				Строка.Значение = Строка.Наименование+" город";
			КонецЕсли;
		КонецЕсли;
