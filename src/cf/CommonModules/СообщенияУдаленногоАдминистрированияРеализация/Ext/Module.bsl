...
Процедура ОбновитьПользователя(Знач ДанныеПользователя) Экспорт
	...
	Если ТипЗнч(ДанныеПользователя)<>Тип("ОбъектXDTO") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'СообщенияУдаленногоАдминистрированияРеализация.ОбновитьПользователя: некорректный тип данных пользователя. Ожидается ОбъектXDTO, получено: %1.'"),
			ТипЗнч(ДанныеПользователя));
	КонецЕсли;
	
	ДопустимыеВерсии = Новый Массив();
	ДопустимыеВерсии.Добавить(СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_1.Пакет());
	ДопустимыеВерсии.Добавить(СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_2.Пакет());
	ДопустимыеВерсии.Добавить(СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_3.Пакет());
	ДопустимыеВерсии.Добавить(СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_4.Пакет());
	ДопустимыеВерсии.Добавить(СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_5.Пакет());
	ДопустимыеВерсии.Добавить(СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_6.Пакет());
	ДопустимыеВерсии.Добавить(СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_7.Пакет());
	ВерсияДанных = ДанныеПользователя.Тип().URIПространстваИмен;
	
	Если ДопустимыеВерсии.Найти(ВерсияДанных) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'СообщенияУдаленногоАдминистрированияРеализация.ОбновитьПользователя: некорректный тип XDTO данных пользователя (%1).'"),
			ВерсияДанных);
	КонецЕсли;
	...
	Попытка
		...
		Если ВерсияДанных = СообщенияУдаленногоАдминистрированияОбработчикСообщения_1_0_3_7.Пакет() Тогда
			ОписаниеПользователяИБ.АутентификацияОС = ДанныеПользователя.OSAuthentication;
			ОписаниеПользователяИБ.ПользовательОС   = ДанныеПользователя.OSUser;
		КонецЕсли;
