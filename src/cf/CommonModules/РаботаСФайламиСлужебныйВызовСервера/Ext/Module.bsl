...
//
Функция СправочникиПапкиСоздатьЭлемент(Имя, Родитель, Пользователь = Неопределено, ИмяСправочникаХранилищаФайлов = "", ПапкаРодитель = Неопределено) Экспорт
	...
	Иначе
		Папка = Справочники[ИмяСправочникаХранилищаФайлов].СоздатьГруппу();
		Папка.ВладелецФайла = Родитель;
		Если Пользователь = Неопределено Тогда
			Папка.Автор = Пользователи.ТекущийПользователь();
		Иначе
			Папка.Автор = Пользователь;
		КонецЕсли;
		Папка.Родитель = ПапкаРодитель;
	КонецЕсли;
	...
	Если СведенияОФайле.СсылкаНаВерсиюИсточник <> Неопределено Тогда // создание Файла из шаблона
		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске И ТипХраненияФайлов = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если Не СведенияОФайле.СсылкаНаВерсиюИсточник.Том.Пустая() Тогда
				ПолныйПутьФайлаШаблона = РаботаСФайламиСлужебный.ПолныйПутьТома(СведенияОФайле.СсылкаНаВерсиюИсточник.Том) 
					+ СведенияОФайле.СсылкаНаВерсиюИсточник.ПутьКФайлу; 
				
				Сведения = РаботаСФайламиСлужебный.ДобавитьФайлВТом(ПолныйПутьФайлаШаблона, СведенияОФайле.ВремяИзмененияУниверсальное,
		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе И ТипХраненияФайлов = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Сведения = РаботаСФайламиСлужебный.ДобавитьФайлВТом(СведенияОФайле.АдресВременногоХранилищаФайла.Получить(),
		...
		ИначеЕсли ТипХраненияФайловШаблона = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске И ТипХраненияФайлов = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
			...
			Если Не СведенияОФайле.СсылкаНаВерсиюИсточник.Том.Пустая() Тогда
				ПолныйПутьФайлаШаблона = РаботаСФайламиСлужебный.ПолныйПутьТома(СведенияОФайле.СсылкаНаВерсиюИсточник.Том) + СведенияОФайле.СсылкаНаВерсиюИсточник.ПутьКФайлу; 
...
//
Функция ТекстЗапросаДляИзвлеченияТекста(ПолучитьВсеФайлы = Ложь) Экспорт
...
Функция СкопироватьФайл(ИсходныйФайл, ВладелецНовогоФайла)
	...
	Если ИсходныйФайл.ТекущаяВерсия.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда 
		ХранилищеФайла = ПолучитьХранилищеФайлаИзИнформационнойБазы(ТекущаяВерсия);
	КонецЕсли;
...
Функция СохранитьИзмененияИОсвободитьФайл(ДанныеФайла, СведенияОФайле,
	...
	Если Не ДанныеФайлаТекущие.ФайлРедактируетТекущийПользователь Тогда
...
Функция СохранитьИзмененияФайла(ФайлСсылка, СведенияОФайле, 
	...
	Попытка
		...
		Если ТипЗнч(ФайлСсылка.Ссылка) = Тип("СправочникСсылка.Файлы") Тогда
			НоваяВерсия = ОбновитьВерсиюФайла(ФайлСсылка.Ссылка, СведенияОФайле,, УникальныйИдентификаторФормы);
		Иначе
...
//
Функция ЕстьПапкаСТакимИменем(ИмяФайла, Родитель, ПерваяПапкаСТакимЖеИменем, ИмяСправочникаХранилищаФайлов = "") Экспорт
...
Процедура ПереименоватьФайлВерсииНаДиске(Версия,
	...
	Если НЕ Версия.Том.Пустая() Тогда
		...
		Попытка
			...
			СтарыйПолныйПуть = РаботаСФайламиСлужебный.ПолныйПутьТома(Версия.Том) + Версия.ПутьКФайлу; 
			...
			ПолныйПутьКТому = РаботаСФайламиСлужебный.ПолныйПутьТома(Версия.Том);
...
Функция ПолучитьДанныеДляПереносаВПриложенныеФайлы(ФайлМассив, ВладелецФайла) Экспорт
	...
	Для Каждого ФайлСсылка Из МассивФайлов Цикл
		...
		ИначеЕсли ТаблРезультат.Найти(ФайлСсылка.Наименование, "ПолноеНаименование") <> Неопределено Тогда 
...
Процедура ЗанестиИнформациюОШифровании(ФайлСсылка, Зашифровать, МассивДанныхДляЗанесенияВБазу, УникальныйИдентификатор, 
	...
	Попытка
		...
		Если АдресВременногоХранилищаТекстаТекущейВерсии <> "" Тогда
			Текст = РаботаСФайламиСлужебный.ПолучитьСтрокуИзВременногоХранилища(АдресВременногоХранилищаТекстаТекущейВерсии);
			ФайлОбъект.ТекстХранилище = Новый ХранилищеЗначения(Текст);
		КонецЕсли;
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Если Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(ПрисоединенныйФайл)) Тогда
		ЕстьПраваНаОбъект = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайл, "Ссылка", Истина);
	Иначе
		ЕстьПраваНаОбъект = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайл.Ссылка, "Ссылка", Истина);
	КонецЕсли;
	...
	Попытка
		...
		Иначе
			...
			Попытка
				...
				СведенияОФайле = РаботаСФайламиСлужебный.ДобавитьФайлВТом(ДвоичныеДанные, ФайлОбъект.ДатаМодификацииУниверсальная,
...
Функция ПолучитьИмяФайлаСПутемКДвоичнымДанным(ОбъектСсылка, ПустойПутьДляПустыхДанных = Ложь) Экспорт
	...
	Если ОбъектСсылка.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		ХранилищеФайла = ПолучитьХранилищеФайлаИзИнформационнойБазы(ОбъектСсылка);
		ДвоичныеДанныеФайла = ХранилищеФайла.Получить();
...
Процедура ЗаписатьРезультатИзвлеченияТекстаПриРедактировании(ВерсияСсылка, РезультатИзвлечения, АдресВременногоХранилищаТекста)
	...
	Попытка
		...
		ЭлементБлокировкиДанных.Режим = БлокировкаВерсии.Исключительный;
		...
		Если Файл.ТекущаяВерсия = ВерсияСсылка Тогда
			...
			ЭлементБлокировкиДанных.УстановитьЗначение("Ссылка", Файл);
			ЭлементБлокировкиДанных.Режим = БлокировкаФайла.Исключительный;
...
Процедура ПроверитьПодписи(ИсходныеДанные, ДанныеСтрок) Экспорт
	...
	Для каждого СтрокаПодписи Из ДанныеСтрок Цикл
		...
		РаботаСФайламиСлужебныйКлиентСервер.ЗаполнитьСтатусПодписи(СтрокаПодписи, ОписаниеОшибки);
	КонецЦикла;
...
Функция ПоместитьФайлыВоВременноеХранилище(Параметры) Экспорт
	...
	Для Каждого ФайлВложение Из Параметры.МассивФайлов Цикл
		ОписаниеФайла = ДанныеФайлаДляОтправкиПоПочте(ФайлВложение, Параметры.ИдентификаторФормы);
...
Функция ДанныеФайлаДляОтправкиПоПочте(ФайлВложение, ИдентификаторФормы)
	
	ДанныеФайлаИДвоичныеДанные = ПолучитьДанныеФайла(ФайлВложение, ИдентификаторФормы);
