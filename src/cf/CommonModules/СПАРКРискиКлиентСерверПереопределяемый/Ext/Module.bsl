...
Процедура ПриОпределенииСвойствКонтрагентаВОбъекте(КонтрагентОбъект, Форма, СвойстваКонтрагента) Экспорт
	...
	Если Форма <> Неопределено и Форма.ИмяФормы = "Справочник.Партнеры.Форма.ФормаЭлементаРеквизитыКонтрагента" Тогда
		...
		СвойстваКонтрагента.ПодлежитПроверке = Форма.ЮридическоеФизическоеЛицо =
			ПредопределенноеЗначение("Перечисление.КомпанияЧастноеЛицо.Компания");
	// { VOG RMV 2019-03-25
	ИначеЕсли ТипЗнч(КонтрагентОбъект) = Тип("СправочникОбъект.вогЮридическиеЛица") Тогда
		СвойстваКонтрагента.ИНН = КонтрагентОбъект.CRM_ИНН;
		СвойстваКонтрагента.ПодлежитПроверке = Истина;
	// } VOG RMV 2019-03-25
	Иначе
		СвойстваКонтрагента.ИНН = КонтрагентОбъект.CRM_ИНН;
		СвойстваКонтрагента.ПодлежитПроверке = КонтрагентОбъект.ЮрФизЛицо =
			ПредопределенноеЗначение("Перечисление.КомпанияЧастноеЛицо.Компания");
	КонецЕсли;
	// -CRM не переносить в объединенные решения
