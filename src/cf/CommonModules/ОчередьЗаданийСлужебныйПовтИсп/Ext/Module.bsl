...
Функция СоответствиеИменМетодовПсевдонимам() Экспорт
	...
	Для каждого ИмяМетода Из РазрешенныеМетоды Цикл
		Результат.Вставить(ВРег(ИмяМетода), ИмяМетода);
	КонецЦикла;
	
	МетодыКонфигурации = Новый Соответствие;
	
	ИнтеграцияСтандартныхПодсистем.ПриОпределенииПсевдонимовОбработчиков(МетодыКонфигурации);
...
Функция СоответствиеОбработчиковОшибокПсевдонимам() Экспорт
	...
	ИнтеграцияСтандартныхПодсистем.ПриОпределенииОбработчиковОшибок(ОбработчикиОшибок);
	...
	Для каждого ОписаниеПараметра Из ОчередьЗаданийСлужебныйПовтИсп.ПараметрыЗаданийОчереди() Цикл
		...
		Если СправочникЗаданий <> Неопределено Тогда
			...
			Если ОписаниеПараметра.РазделениеДанных Тогда
				...
				Если Не РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() ИЛИ Не РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(СправочникЗаданий, РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных()) Тогда
