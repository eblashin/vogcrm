...
//
Процедура ЗаполнитьПрисоединенныеФайлыКОбъекту(Знач Объект, Знач МассивФайлов) Экспорт
	...
	Если ТипыВладельцев.Найти(ТипЗнч(Объект)) <> Неопределено Тогда
		...
		МассивФайловЛокальный = РаботаСФайламиСлужебный.ВсеПодчиненныеФайлы(Объект);
...
Процедура ОбновитьФайл(Знач ПрисоединенныйФайл, Знач ИнформацияОФайле) Экспорт
	...
	РаботаСФайламиСлужебный.ОбновитьДвоичныеДанныеФайлаНаСервере(
...
Функция ДобавитьФайл(ПараметрыФайла,
	...
	ВидФайла = Неопределено; // VOG Солодов В.В. 03.08.2020 CRM-572
	Если ПараметрыФайла.Свойство("ГруппаФайлов") 
		И ЗначениеЗаполнено(ПараметрыФайла.ГруппаФайлов)
		И ТипЗнч(ВладелецФайлов) <> Тип("СправочникСсылка.ПапкиФайлов") Тогда
		
		// ++ VOG Солодов В.В. 22.07.2020 CRM-572
		Если ТипЗнч(ПараметрыФайла.ГруппаФайлов) = Тип("Структура") Тогда
			
			ИмяСправочника 	= РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов);
			
			Если Не ПараметрыФайла.ГруппаФайлов.Свойство("Родитель") Тогда
				
				ИдентификаторСтрокой 	= СтрЗаменить(ПараметрыФайла.ГруппаФайлов.УникальныйИдентификатор, "_", "-");
				ИдентификаторГруппы 	= Новый УникальныйИдентификатор(ИдентификаторСтрокой);
				НоваяСсылкаНаГруппу 	= Справочники[ИмяСправочника].ПолучитьСсылку(ИдентификаторГруппы);
				
				ГруппаФайловОбъект 		= Справочники[ИмяСправочника].СоздатьГруппу();
				ГруппаФайловОбъект.УстановитьСсылкуНового(НоваяСсылкаНаГруппу);
				ГруппаФайловОбъект.ВладелецФайла 	= ВладелецФайлов;
				ГруппаФайловОбъект.Наименование 	= ПараметрыФайла.ГруппаФайлов.Наименование;
				
				ГруппаФайловОбъект.Записать();
				
				РодительГруппы = ГруппаФайловОбъект.Ссылка;
				
			Иначе
				
				РодительГруппы = ПараметрыФайла.ГруппаФайлов.Родитель;
				
			КонецЕсли;
			
			Если ПараметрыФайла.ГруппаФайлов.Свойство("ВидФайла") Тогда
				
				ВидФайла = ПараметрыФайла.ГруппаФайлов.ВидФайла;
				
				Если ОбщегоНазначения.СсылкаСуществует(ПараметрыФайла.ГруппаФайлов.ГруппаСсылка) Тогда
					ГруппаФайловОбъект = ПараметрыФайла.ГруппаФайлов.ГруппаСсылка;
				Иначе
					
					НаименованиеВидаФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыФайла.ГруппаФайлов.ВидФайла, "Наименование", Истина);
					
					ГруппаФайловОбъект 					= Справочники[ИмяСправочника].СоздатьГруппу();
					ГруппаФайловОбъект.УстановитьСсылкуНового(ПараметрыФайла.ГруппаФайлов.ГруппаСсылка);
					ГруппаФайловОбъект.ВладелецФайла 	= ВладелецФайлов;
					ГруппаФайловОбъект.Наименование 	= НаименованиеВидаФайла;
					ГруппаФайловОбъект.Родитель 		= РодительГруппы;
			...
			ГруппаФайлов = ГруппаФайловОбъект.Ссылка;
	...
	// ++ VOG Солодов В.В. 03.08.2020 CRM-572
	Если ЗначениеЗаполнено(ВидФайла)
		И ЗначениеЗаполнено(ПрисоединенныйФайл.Ссылка) Тогда
		...
		НаборЗаписей.Отбор.ПрисоединенныйФайл.Установить(ПрисоединенныйФайл.Ссылка);
		
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.ВидФайла 			= ВидФайла;
		НоваяЗапись.ПрисоединенныйФайл 	= ПрисоединенныйФайл.Ссылка;
...
Функция МаксимальныйРазмерФайла() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(МаксимальныйРазмерФайла) Тогда
		...
		Константы[ИмяКонстанты].Установить(МаксимальныйРазмерФайла);
	КонецЕсли;
...
Процедура ИзменитьСправочникХраненияФайлов(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Попытка
		...
		Для Каждого ФайлИсточник Из ФайлыИсточник Цикл
			...
			Если ПрисоединенныйФайл.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
				ХранилищеФайла = РаботаСФайламиСлужебныйВызовСервера.ПолучитьХранилищеФайлаИзИнформационнойБазы(ТекущаяВерсияОбъект.Ссылка);
...
Функция СсылкиНаОбъектыСФайлами(Знач ТаблицаВладельцевФайлов) Экспорт
	...
	Возврат РаботаСФайламиСлужебный.СсылкиНаОбъектыСФайлами(ТаблицаВладельцевФайлов);
