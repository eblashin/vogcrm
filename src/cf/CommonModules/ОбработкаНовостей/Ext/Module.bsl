...
#Область ОчисткаМусора
				...
				Если ЕстьДанныеДляОбработки = Истина Тогда
					...
					ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						...
						Если НЕ Результат.Пустой() Тогда
							ОписаниеРезультатаВложенный = НСтр("ru='Есть необработанные простые категории, не привязанные к новостям:'")
								+ Символы.ПС;
							Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
							Пока Выборка.Следующий() Цикл
							...
							КонецЦикла;
						Иначе
							ОписаниеРезультатаВложенный = НСтр("ru='Нет необработанных простых категорий, не привязанных к новостям.'");
						КонецЕсли;
						...
						Если НЕ Результат.Пустой() Тогда
							ОписаниеРезультатаВложенный = НСтр("ru='Есть необработанные категории интервалов версий, не привязанные к новостям:'")
								+ Символы.ПС;
							Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
							Пока Выборка.Следующий() Цикл
							...
							КонецЦикла;
						Иначе
							ОписаниеРезультатаВложенный = НСтр("ru='Нет необработанных категорий интервалов версий, не привязанных к новостям.'");
						КонецЕсли;
...
Процедура ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если ВыполнениеРазрешено = Истина Тогда
			...
			Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся) = Истина Тогда
				...
				Если (КатегорияВерсияПродукта.Пустая())
					...
					// Возможно, список категорий еще не пришел с сервера.
					ОписаниеРезультата = НСтр("ru='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, не заполнены значения плана видов характеристик КатегорииНовостей:
						...
						+ Символы.ПС;
					ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%КатегорияВерсияПродукта%", КатегорияВерсияПродукта);
					ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%КатегорияВерсияПлатформы%", КатегорияВерсияПлатформы);
		...
		Иначе
			...
			КодРезультата = 1101;
			ОписаниеРезультата = НСтр("ru='Выполнение запрещено.
				|ОбщегоНазначения.РазделениеВключено() = %ОбщегоНазначения.РазделениеВключено()%
				|ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() = %ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей()%
				|ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = %ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()%'");
			ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ОбщегоНазначения.РазделениеВключено()%", ОбщегоНазначения.РазделениеВключено());
			ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей()%", ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей());
			ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных()%", ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных());
...
Процедура КонтекстныеНовости_ПриСозданииНаСервере(
	...
	Форма.Новости = Новый Структура("Новости, ЗаголовокФормыКонтекстныхНовостей, Метаданные, Форма, ТекущаяНовость,
		...
		Новый СписокЗначений, // СписокНовостейИзменениеПризнака_Прочтена - заполняется на клиенте в процессе
							//  работы в случае, если новости хранятся на сервере
							//  и приходят оповещения о прочтении новостей.
		Новый СписокЗначений); // СписокНовостейИзменениеПризнака_ОповещениеВключено - заполняется на клиенте
							//  в процессе работы в случае, если новости хранятся на сервере и приходят оповещения
							//  об изменении оповещения для важных и очень важных новостей.
