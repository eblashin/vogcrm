...
Функция ДополнительныеПараметрыВызоваОперацииСервиса(ФабрикаXDTOСервиса, URIПространстваИмен)
	...
	ДополнительныйПараметрXDTO.value = Метаданные.Имя;
...
КонецФункции // -- VOG Солодов В.В. 05.02.2021 DEV-100
...
Процедура ЗаменитьПолныеНаименованияАдресныхОбъектовСокращенными(Объект, АдресныеСокращения = Неопределено)
	...
	Если АдресныеСокращения = Неопределено Тогда
		АдресныеСокращения = АдресныйКлассификатор.СокращенияАдресныхОбъектов(Новый Массив);
	КонецЕсли;
...
Процедура ПроверитьДополнительныеАдресныеЭлементы(Объект, Фабрика)
	...
	Для каждого ТипОбъектовАдресации Из РаботаСАдресами.ТипыОбъектовАдресацииАдресаРФ() Цикл
	// До изменения
...
Процедура ЗаполнитьИнформациюОПодразделениях(ОбъектXDTO, ИнформацияОСвязях)
	...
	Если ОбъектXDTO.СвПодразд <> Неопределено Тогда
		...
		// ++ VOG Солодов В.В. 03.03.2020 CRM-196
		ПространствоИменКИ = УправлениеКонтактнойИнформациейСлужебный.ПространствоИмен();
		// До изменения
...
Процедура ЗаполнитьЮридическийАдрес(ОбъектXDTO, Реквизиты, Фабрика)
	...
	Если ОбъектXDTO.СвАдрес <> Неопределено 
		...
		Реквизиты.ЮридическийАдрес = АдресКИ(ОбъектXDTO.СвАдрес.Адрес, Фабрика);
...
// ++ VOG Солодов В.В. 05.02.2021 DEV-100
Функция АдресКИ(АдресXDTO, Фабрика)
