...
//
// Параметры:
//    ПараметрыФормы - Структура - Параметры формы установки обновлений.
//
...
Процедура УстановитьОбновление(Форма, Параметры, ПараметрыАдминистрирования) Экспорт
	...
	// Также при проверке возможности установки обновления выполняется
...
Функция ЗапланироватьОбновлениеКонфигурации(Параметры, ПараметрыАдминистрирования)
	...
	ИмяЗапускаемогоСкрипта = СтандартныеПодсистемыКлиент.ПапкаСистемныхПриложений() + "wscript.exe";
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	Возврат СоздатьФайлыСкрипта(КаталогВременныхФайловОбновления, ТекстыМакетов, ИнтерактивныйРежим);
	#КонецЕсли
...
Функция СоздатьФайлыСкрипта(КаталогВременныхФайловОбновления, ТекстыМакетов, ИнтерактивныйРежим)
