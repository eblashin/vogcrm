...
Процедура ОтменитьВыгрузку() Экспорт
	...
	ОбщегоНазначения.УдалитьДанныеИзБезопасногоХранилища(УзелОбмена);
...
Процедура ЗаданиеВыгрузка() Экспорт
	...
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.МиграцияПриложенийСостояниеВыгрузки");
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	...
	Исключение
		ОтменитьТранзакцию();
		Если (ТекущаяУниверсальнаяДата() - СостояниеВыгрузки.ДатаСостоянияЗагрузки) > 86400 Тогда
			ПредставлениеОшибки = НСтр("ru = 'Сервис недоступен больше 24 часов.'");
			ЗавершитьВыгрузкуСОшибкой(ПредставлениеОшибки, ПредставлениеОшибки);
		КонецЕсли;
		Возврат;
	КонецПопытки;
	...
	Если СостояниеЗагрузки.ОчередьСообщений >= МаксимальнаяОчередьЗагрузки() Тогда
	...
	Если СостояниеВыгрузки.Завершение Тогда
	...
	Исключение
		...
		ЗавершитьВыгрузкуСОшибкой(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		ВызватьМетодСервиса(ПараметрыПодключения, "Cancel");
		Возврат;
	...
	СтатусЗагрузки = ВызватьМетодСервиса(ПараметрыПодключения, "Upload", ИмяФайлаАрхива);
	ЗафиксироватьТранзакцию();
	
	ЗаполнитьСостояниеВыгрузки(СостояниеВыгрузки, СтатусЗагрузки, УзелОбмена);
	СостояниеВыгрузки.Записать();
...
Функция ВыгрузитьДанныеВКаталог(КаталогВыгрузки, УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных) Экспорт
	...
	Если ЭтоПоследнееСообщение Тогда
		...
		ВыгрузитьНастройкиПользователей(ХранилищеВариантовОтчетов, "ReportsVariantsStorage.xml", ПараметрыВыгрузки);
		ВыгрузитьНастройкиПользователей(ХранилищеНастроекДанныхФорм, "FormDataSettingsStorage.xml", ПараметрыВыгрузки);
		ВыгрузитьНастройкиПользователей(ХранилищеОбщихНастроек, "CommonSettingsStorage.xml", ПараметрыВыгрузки);
		ВыгрузитьНастройкиПользователей(ХранилищеПользовательскихНастроекДинамическихСписков, "DynamicListsUserSettingsStorage.xml", ПараметрыВыгрузки);
		ВыгрузитьНастройкиПользователей(ХранилищеПользовательскихНастроекОтчетов, "ReportsUserSettingsStorage.xml", ПараметрыВыгрузки);
		ВыгрузитьНастройкиПользователей(ХранилищеСистемныхНастроек, "SystemSettingsStorage.xml", ПараметрыВыгрузки);
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Если Не Выборка.Следующий() Тогда
		...
		Если (ТекущаяУниверсальнаяДата() - ОтсчетВремени) > 86400 Тогда
			ПредставлениеОшибки = НСтр("ru = 'Отсутствуют сообщения в течении больше 24 часов.'");
			ЗавершитьЗагрузкуСОшибкой(ПредставлениеОшибки, ПредставлениеОшибки);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗавершитьЗагрузкуСОшибкой(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		Возврат;
	КонецПопытки;
	...
	Для Каждого КлючИЗначение Из СсылкиПоТипам Цикл
		...
		Для Каждого Ссылка Из КлючИЗначение.Значение Цикл
			...
			Если ПараметрыВыгрузки.ТипыСопоставляемыхОбщихДанныхПоПредопределенномуИмени[КлючИЗначение.Ключ] <> Неопределено
				...
				ВызватьИсключение НСтр("ru = 'Обнаружена ссылка на общие данные без поля поиска.'");
			КонецЕсли;
...
//   ПараметрыВыгрузки - Структура - описание см. ПараметрыВыгрузки().
//
Процедура ВыгрузитьНастройкиПользователей(ХранилищеНастроек, ИмяФайла, ПараметрыВыгрузки)
	...
	Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли Не ТипНастроек = Тип("НастройкиИсторииВыбора") 
				...
				И Не ТипНастроек = Тип("НастройкиФормы") Тогда
...
Процедура ДобавитьВспомогательныеДанные(Значение, ПараметрыВыгрузки)
	...
	ИначеЕсли ПараметрыВыгрузки.ЗапрещенныеТипыОбщихДанных[ТипЗначения] <> Неопределено Тогда 
		...
		Если ЗначениеЗаполнено(Значение) Тогда
			ВызватьИсключение НСтр("ru = 'В объекте обнаружена недопустимая ссылка.'");
		КонецЕсли;
...
Функция ВызватьМетодСервиса(ПараметрыПодключения, МетодСервиса, Тело = Неопределено)
	...
	Соединение = Новый HTTPСоединение(ПараметрыПодключения.ИмяСервера, , ПараметрыПодключения.Логин, ПараметрыПодключения.Пароль, , 10, ЗащищенноеСоединение);
	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		СтатусЗагрузки = СериализаторXDTO.ПрочитатьXML(ЧтениеXML);
		ЧтениеXML.Закрыть();
		
		Возврат СтатусЗагрузки;
...
Процедура ПроверитьСоставПодписки(Подписка, ТекущиеТипы, НужныеТипы, ОбщийРезультат)
	...
	Для Каждого ТекущийТип Из ТекущиеТипы Цикл
		Если НужныеТипы[ТекущийТип] = Неопределено Тогда
			НужноУдалить.Добавить(Строка(ТекущийТип));
		Иначе
	...
	Для Каждого НужныйТип Из НужныеТипы Цикл
		НужноДобавить.Добавить(Строка(НужныйТип.Ключ));
	КонецЦикла;
