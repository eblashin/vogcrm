...
Процедура ПриОпределенииСвойствСправочниковКонтрагентов(СвойстваСправочников) Экспорт
	...
	ОписаниеСправочника.Имя                     = "Партнеры";
	ОписаниеСправочника.РеквизитИНН             = "CRM_ИНН";
...
Процедура ПараметрыНачальногоЗаполненияДанных1СПАРКРиски(ПараметрыЗаполнения) Экспорт
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Контрагенты.Ссылка КАК Контрагент,
	|	Контрагенты.CRM_ИНН КАК ИНН
	|ИЗ
	|	Справочник.Партнеры КАК Контрагенты
	|ГДЕ
	|	Контрагенты.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.КомпанияЧастноеЛицо.Компания) ";
