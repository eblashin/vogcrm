...
Функция РазделительОсновныхДанных() Экспорт
	...
	Возврат РаботаВМоделиСервисаПовтИсп.РазделительОсновныхДанных();
...
Функция РазделительВспомогательныхДанных() Экспорт
	...
	Возврат РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных();
...
Функция ДоступноИспользованиеРазделенныхДанных() Экспорт
	...
	Возврат РаботаВМоделиСервисаПовтИсп.ДоступноИспользованиеРазделенныхДанных();
...
Функция ОбластьДанныхЗаблокирована(Знач ОбластьДанных) Экспорт
	...
	Если НЕ РаботаВМоделиСервисаПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
...
Процедура УстановитьРазделениеСеанса(Знач Использование = Неопределено, Знач ОбластьДанных = Неопределено) Экспорт
	...
	Если НЕ РаботаВМоделиСервисаПовтИсп.СеансЗапущенБезРазделителей() Тогда
...
Функция ПолучитьФайлИзХранилищаМенеджераСервиса(Знач ИДФайла) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПередачаДанных") Тогда
		...
		Если ПоддерживаемыеВерсии.Количество() > 0 Тогда
			...
			ОписаниеФайла = МодульПередачаДанныхСлужебный.Получить(ПараметрыДоступаКХранилищу, ИДФайла);
...
Функция ПоместитьФайлВХранилищеМенеджераСервиса(Знач АдресДанныеФайл, Знач ИмяФайла = "") Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПередачаДанных") Тогда
		...
		Если ПоддерживаемыеВерсии.Количество() > 0 Тогда
			...
			МодульПередачаДанныхСлужебный = ОбщегоНазначения.ОбщийМодуль("ПередачаДанныхСлужебный");
			ИдентификаторФайлаВХранилище = МодульПередачаДанныхСлужебный.Отправить(ПараметрыДоступаКХранилищу, АдресДанныеФайл, ИмяФайла);
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	ИнтеграцияСтандартныхПодсистем.ПриЗаполненииТаблицыПараметровИБ(ТаблицаПараметров);
...
Процедура ПриИзмененииОбластиДанных() Экспорт
	...
	Если РаботаВМоделиСервисаПовтИсп.ДоступноИспользованиеРазделенныхДанных()
...
Функция СеансЗапущенБезРазделителей() Экспорт
	...
	Возврат РаботаВМоделиСервисаПовтИсп.СеансЗапущенБезРазделителей();
...
Процедура ПриПроверкеВключенияБезопасногоРежимаРазделенияДанных() Экспорт
	...
	Если БезопасныйРежим() = Ложь
		И РаботаВМоделиСервисаПовтИсп.РазделениеВключено()
		И РаботаВМоделиСервисаПовтИсп.ДоступноИспользованиеРазделенныхДанных()
		И НЕ РаботаВМоделиСервисаПовтИсп.СеансЗапущенБезРазделителей() Тогда
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	Если НЕ РаботаВМоделиСервисаПовтИсп.РазделениеВключено()
		ИЛИ НЕ РаботаВМоделиСервисаПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		Возврат;
	КонецЕсли;
	...
	Если НЕ Результат.Пустой() Тогда
		Выборка = Результат.Выбрать();
		Выборка.Следующий();
		Если РаботаВМоделиСервисаПовтИсп.СеансЗапущенБезРазделителей() Тогда
...
Процедура ОбработатьВспомогательныеДанные(ОбъектВспомогательныхДанных, Знач Записать, Знач Удалить)
	...
	Попытка
		...
		Если Не МонопольныйРежим() И ЭтоРазделенныйОбъектМетаданных(ПроверяемоеЗначение.Метаданные(), РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных()) Тогда
			
			Если РаботаВМоделиСервисаПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
			...
			Иначе
				...
				Если ПроверяетсяСсылка Тогда
					ЗначениеРазделителя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроверяемоеЗначение, РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных());
				Иначе
					ЗначениеРазделителя = ОбъектВспомогательныхДанных[РаботаВМоделиСервисаПовтИсп.РазделительВспомогательныхДанных()];
				КонецЕсли;
