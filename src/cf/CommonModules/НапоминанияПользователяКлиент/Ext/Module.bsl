...
//  Предмет - ЛюбаяСсылка - предмет напоминания;
//
Процедура Напомнить(Текст, ВремяСобытия = Неопределено, ИнтервалДоСобытия = 0, Предмет = Неопределено) Экспорт
...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	Если ПараметрыРаботыКлиента.НастройкиНапоминаний.ИспользоватьНапоминания Тогда
		ПодключитьОбработчикОжидания("ПроверитьТекущиеНапоминания", 300, Истина); // Через 300 секунд после запуска клиента.
	КонецЕсли;
...
// +++ VOG Кулаков П.Л. 25.12.2020 DEV-46
Процедура вогОткрытьФормуОповещения() Экспорт
	
	ИмяФормыОповещения = "ОбщаяФорма.вогФормаОповещенияПользователя";
	ФормаОповещения = ПолучитьФорму(ИмяФормыОповещения);
	ФормаОповещения.Открыть();
	
КонецПроцедуры // --- VOG Кулаков П.Л.

// +++ VOG Кулаков П.Л. 25.12.2020 DEV-46
// Сбрасывает таймер проверки текущих оповещений и выполняет проверку немедленно.
Процедура вогСброситьТаймерПроверкиТекущихОповещений() Экспорт
	ОтключитьОбработчикОжидания("вогПроверитьТекущиеОповещения");
	вогПроверитьТекущиеОповещения();
КонецПроцедуры // --- VOG Кулаков П.Л.
