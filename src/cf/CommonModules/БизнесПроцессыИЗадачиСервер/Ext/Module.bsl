...
// Помечает на удаление (или снимает отметку) все задачи указанного бизнес-процесса.
//
// Параметры:
...
//
// Параметры:
//  БизнесПроцесс - БизнесПроцессСсылка -Бизнес процесс у которого требуется получить дату отложенного старта.
// 
// Возвращаемое значение:
...
Процедура УведомитьИсполнителейОНовыхЗадачах() Экспорт
	...
	ОписаниеОшибки = "";
	ВидСообщения = НСтр("ru = 'Бизнес-процессы и задачи. Уведомление о новых задачах.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроконтролироватьЗадачи() Экспорт
	...
	Если НЕ СистемнаяУчетнаяЗаписьНастроена(ОписаниеОшибки) Тогда
		ВидСообщения = НСтр("ru = 'Бизнес-процессы и задачи. Мониторинг задач.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ВидСообщения, УровеньЖурналаРегистрации.Ошибка,
...
// См. ИнтеграцияСтандартныхПодсистем.ПриОпределенииГруппИсполнителей.
