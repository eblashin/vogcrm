...
Процедура ВыполнитьКоманду(Знач Форма, Знач ПараметрыВызова, Знач Источник, Результат) Экспорт
	...
	Остаток = "";
	Вид = СтрЛев(ОписаниеКоманды.Обработчик, ".", Остаток);
	Если НРег(Вид) = НРег("ОбщийМодуль") Тогда
		Обработчик = Остаток;
	Иначе
...
Функция НастройкиПрограммногоИнтерфейсаПодключаемыхОбъектов() Экспорт
	...
	ИнтеграцияСтандартныхПодсистем.ПриОпределенииСоставаНастроекПодключаемыхОбъектов(Таблица);
...
Функция КэшФормы(ИмяФормы, ИсточникиЧерезЗапятую, ЭтоФормаОбъекта) Экспорт
	...
	ИнтеграцияСтандартныхПодсистем.ПриОпределенииКомандПодключенныхКОбъекту(Контекст, Источники, ПодключенныеОбъекты, Команды);
...
Функция ВидыКоманд()
	...
	ИнтеграцияСтандартныхПодсистем.ПриОпределенииВидовПодключаемыхКоманд(Виды);
...
Функция ЗарегистрироватьПодменю(Элементы, СведенияОВсехПодменю, ИмяПодменю, ШаблонНового = Неопределено, КоманднаяПанель = Неопределено, ПодменюПоУмолчанию = Неопределено)
	...
	Если ЗначениеЗаполнено(ИмяПодменю) Тогда
		...
		Если Подменю = Неопределено Тогда
			...
			Если ШаблонНового.ВидГруппыФормы = ВидГруппыФормы.Подменю Тогда
...
Функция ИмяСоответствуетТребованиямИменованияСвойств(Имя)
	Если ПустаяСтрока(Имя) Или СтрНайти(Имя, " ") > 0 Или СтрНайти(Имя, ",") > 0 Или СтрНайти(Имя, "(") > 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	Попытка
