...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	Если ИзвлекатьТекстыФайловНаКлиенте Тогда
		АдресВременногоХранилищаТекста = РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекстВоВременноеХранилище(
...
//		ВладелецФайлы - Справочник,Документ ссылка
//		ИдентификаторФормы  - уникальный идентификатор формы из которой вызван метод
//		ГруппаФайлов - группа справочника ...ПрисоединенныеФайлы, в которую нужно добавлять файл
//		Фильтр - фильтр на расширения имени файла
//		ПолноеИмяФайла - полное имя добавляемого файла
//		НеОткрыватьКарточкуПослеСозданияИзФайла - булево, не открывать карточку добавленного файла
//		ИмяСоздаваемогоФайла - имя для нового файла
//		ОбработкаОповещения - Обработка оповещения, после добавления нового файла
//
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Если ВыбранныеФайлы.Количество() > 0  Тогда
			...
			Если ПрисоединенныеФайлыМассив.Количество() > 0 Тогда
				...
				// ++ Харченко Д.И. №  - 20.09.2018 / 
				Если ДополнительныеПараметры.Свойство("ОбработкаОповещения") Тогда
					ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработкаОповещения, ПараметрыОповещения);
				КонецЕсли;
				// -- Харченко Д.И. №  - 20.09.2018
			КонецЕсли;
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если НЕ РасширениеРаботыСФайламиПодключено() Тогда
			ТекстНапоминания = 
				...
				|
				|2. Выберите каталог для сохранения файла и запомните его
				|(каталог понадобится для редактирования и помещения файла обратно в хранилище).
				|
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	Для каждого Элемент Из Коллекция Цикл
		...
		РаботаСФайламиСлужебныйКлиентСервер.ЗаполнитьСтатусПодписи(СтрокаПодписи, ДанныеСтрок[Индекс].ОписаниеОшибки);
		Индекс = Индекс + 1;
	КонецЦикла;
...
Процедура ПоместитьВыбранныеФайлыВХранилище(Знач ВыбранныеФайлы,
                                            ...
                                            Знач ГруппаФайлов = Неопределено) Экспорт
	...
	Для Каждого ПолноеИмяФайла Из ВыбранныеФайлы Цикл
		...
		Иначе
			АдресВременногоХранилищаТекста =
				РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекстВоВременноеХранилище(
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.СоздатьНовуюВерсию = Неопределено Тогда
		...
		Если СоздатьНовуюВерсиюДоступность Тогда
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПроверкиЗашифрован(Неопределен, ПараметрыВыполнения) Экспорт
	...
	Если Не ОбщиеНастройкиРаботыСФайлами.ИзвлекатьТекстыФайловНаСервере Тогда
		Попытка
			СведенияОФайле.АдресВременногоХранилищаТекста = РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекстВоВременноеХранилище(
	...
	Для Индекс = ПараметрыВыполнения.ВыбранныеФайлыИндекс По ПараметрыВыполнения.ВыбранныеФайлыВГраница Цикл
		...
		Если ВыбранКаталог Тогда
			...
			Если РаботаСФайламиСлужебныйВызовСервера.ЕстьПапкаСТакимИменем(
					...
					ПараметрыВыполнения.ПерваяПапкаСТакимЖеИменем,
					ПараметрыВыполнения.ИмяСправочникаХранилищаФайлов) Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ИмпортФайловЦиклПродолжитьИмпорт(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.ПапкаУжеНайдена Тогда
		Если ПустаяСтрока(ПараметрыВыполнения.ИмяСправочникаХранилищаФайлов) Тогда
		...
		Иначе
			...
			ПараметрыВыполнения.ГруппаФайлов = РаботаСФайламиСлужебныйВызовСервера.СправочникиПапкиСоздатьЭлемент(
				ПараметрыВыполнения.ИмяПапки, ПараметрыВыполнения.Владелец, , ПараметрыВыполнения.ИмяСправочникаХранилищаФайлов, ПараметрыВыполнения.ГруппаФайлов);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗакончитьРедактированиеСОповещением(Параметры) Экспорт
	...
	ПараметрыОбработчика.ФайлРедактируетТекущийПользователь = Параметры.ФайлРедактируетТекущийПользователь;
...
Процедура ЗакончитьРедактированиеПоСсылкамСОповещением(Параметры) Экспорт
	...
	ОбработчикРезультата = Новый ОписаниеОповещения("ЗакончитьРедактированиеМассиваФайловСОповещениемЗавершение", ЭтотОбъект, Параметры);
	Параметры.ОбработчикРезультата = ОбработчикРезультата;
...
Процедура ДобавитьНаОсновеШаблонаПослеВыбораШаблона(Результат, ПараметрыВыполнения) Экспорт
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗначениеКопирования", Результат);
	ПараметрыФормы.Вставить("ВладелецФайла",       ПараметрыВыполнения.ВладелецФайла);
	ПараметрыФормы.Вставить("РежимСоздания",       "ИзШаблона");
	ОбработчикРезультата = ПодготовитьОбработчикДляДиалога(ПараметрыВыполнения.ОбработчикРезультата);
	Если ПараметрыВыполнения.Свойство("ИмяСправочникаХранилищаФайлов") Тогда
		ПараметрыФормы.Вставить("ИмяСправочникаХранилищаФайлов", ПараметрыВыполнения.ИмяСправочникаХранилищаФайлов);
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ПрисоединенныйФайл", ПараметрыФормы, ПараметрыВыполнения.ФормаВладелец, , , , ОбработчикРезультата);
	Иначе
		ПараметрыФормы.Вставить("ИмяСправочникаХранилищаФайлов", "Файлы");
...
Функция ПараметрыИмпортаФайлов() Экспорт
	...
	ПараметрыВыполнения.Вставить("ИмяСправочникаХранилищаФайлов", Неопределено);
...
Процедура ИмпортироватьФайл(Знач ВыбранныйФайл, Знач Владелец, Знач ПараметрыВыполнения, Знач АдресВременногоХранилищаФайла, Знач АдресВременногоХранилищаТекста) 
	...
	Иначе
		ПараметрыФайла = Новый Структура;
		ПараметрыФайла.Вставить("ВладелецФайлов",              ПараметрыВыполнения.Владелец);
		ПараметрыФайла.Вставить("Автор",                       Неопределено);
...
#Если Не ВебКлиент Тогда
	...
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		...
		Владелец          = НайденныеСвойства.Файл;
		НомерВерсии       = НайденныеСвойства.НомерВерсии;
		ВРегистреНаЧтение = НайденныеСвойства.ВРегистреНаЧтение;
		ВРегистреКодФайла = НайденныеСвойства.ВРегистреКодФайла;
		ВРегистреПапка    = НайденныеСвойства.ВРегистреПапка;
...
Процедура ВыполнитьСравнениеФайлов(ПутьКФайлу1, ПутьКФайлу2, СпособСравненияВерсийФайлов, ЗаголовокЛевый = "", ЗаголовокПравый = "") Экспорт
	...
	Попытка
		...
		ИначеЕсли СпособСравненияВерсийФайлов = "СравнениеТабличныхДокументов" Тогда
			...
			ФормаСравнения = ОткрытьФорму("ОбщаяФорма.СравнениеТабличныхДокументов", ПараметрыОткрытияФормы, ЭтотОбъект);
		КонецЕсли;
