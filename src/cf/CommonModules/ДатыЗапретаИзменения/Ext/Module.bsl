...
//
//  ОписаниеОшибки      - Строка, Null - возвращаемый параметр: текстовое описание запрета изменения или загрузки.
//                                       Если описание не требуется, то параметр указывать не нужно (Null).
//
...
//  ОписаниеОшибки      - Строка, Null - возвращаемый параметр: текстовое описание причины запрета загрузки.
//                                       Если описание не требуется, то параметр указывать не нужно (Null).
//
...
Функция ОбъектПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	Если ИзменениеЗапрещено(Источник) Тогда
		...
		//+++ Терпогосян Д.Б. [09.08.2021 17:20:16] № DEV-743
		Попытка
			Форма.Элементы.ФормаЗаписать.Доступность = Ложь; 
			Форма.Элементы.ФормаПровестиИЗакрыть.Доступность = Ложь; 
		Исключение
		КонецПопытки; 
		//--- Терпогосян Д.Б. [09.08.2021 17:25:22] № DEV-743 
	КонецЕсли;
КонецФункции
...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки,
	...
	Для Каждого Строка Из ДанныеДляПроверки Цикл
		...
		СвойстваРаздела = ДействующиеДаты.СвойстваРазделов.Разделы.Получить(Строка.Раздел);
	...
	КонецЕсли;
	//+++ Терпогосян Д.Б. [24.09.2021 11:29:48] № CRM-1192
	Если ЗапретыИзменения.Количество() Тогда 
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Действует запрет изменений с " + ЗапретыИзменения[0].Дата + " по " + ЗапретыИзменения[0].Объект + " для " + ЗапретыИзменения[0].Раздел; 
		Сообщение.Сообщить();
	КонецЕсли; 
	//--- Терпогосян Д.Б. [24.09.2021 11:30:00] № CRM-1192 
...
Функция ШаблонДанныхДляПроверки() Экспорт
	...
	Образец = ДатыЗапретаИзмененияСлужебныйПовтИсп.ШаблонДанныхДляПроверки();
	
	Возврат Образец.Скопировать();
