...
Процедура УстановитьПрефикс(Источник, Префикс, УстановитьПрефиксИнформационнойБазы, УстановитьПрефиксОрганизации)
	...
	//--> VOG Солодов
	вогШаблонПрефикса = "[ИБ][Т][Префикс]";
	вогШаблонПрефикса = СтрЗаменить(вогШаблонПрефикса, "[ИБ]", "CRM");
	Если ТипЗнч(Источник) = Тип("СправочникОбъект.вогЮридическиеЛица") Тогда
		ШаблонПрефикса = СтрЗаменить(вогШаблонПрефикса, "[Т]", "Ю");
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникОбъект.вогТорговыеТочки") Тогда
		ШаблонПрефикса = СтрЗаменить(вогШаблонПрефикса, "[Т]", "Т");
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникОбъект.вогРаспределительныеЦентры") Тогда
		ШаблонПрефикса = СтрЗаменить(вогШаблонПрефикса, "[Т]", "Р");
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникОбъект.Партнеры") Тогда
		ШаблонПрефикса = СтрЗаменить(вогШаблонПрефикса, "[Т]", "П");
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникОбъект.ДоговорыКонтрагентов") Тогда
		ШаблонПрефикса = СтрЗаменить(вогШаблонПрефикса, "[Т]", "");
	КонецЕсли;
	//<-- VOG Солодов
