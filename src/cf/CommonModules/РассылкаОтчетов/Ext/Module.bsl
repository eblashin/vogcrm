...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	ЗапускЗафиксирован = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПредварительныеНастройки, "ЗапускЗафиксирован");
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	Если Не ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Результат.Вставить("Подробно", РассылкаОтчетовКлиентСервер.СтрокаСообщенийПользователю(МассивСообщений));
	...
	Пока РассылкиОтчетов.Следующий() Цикл
		Попытка
			Если РассылкиОтчетов.Ссылка <> NULL И РассылкиОтчетов.Ссылка.РегламентноеЗадание <> NULL Тогда
				Задание = РегламентныеЗадания.НайтиПоУникальномуИдентификатору(РассылкиОтчетов.Ссылка.РегламентноеЗадание);
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	// Рассылка
	ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала, Новый Структура("ЗапускЗафиксирован", Истина));
КонецПроцедуры
...
//   Текст - Строка - Краткое описание проблемы, который может содержать параметры %1, %2, %3 и %4.
...
//
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	// Параметры записи в журнал регистрации.
	...
	Если Недоступные.Количество() > 0 Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru = 'В рассылке есть недоступные отчеты (%1):%2'"), ,
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Иначе
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					НСтр("ru = 'Недостаточно прав для создания файлов в папке ""%1"".'"), ,
					ПредставлениеПапки);
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	// Каталог временных файлов (RP = report publishing).
