...
#Иначе
	Возврат РаботаСФайламиСлужебныйКлиентПовтИсп.ПерсональныеНастройкиРаботыСФайлами();
#КонецЕсли
...
#Иначе
	Возврат РаботаСФайламиСлужебныйКлиентПовтИсп.ОбщиеНастройкиРаботыСФайлами();
#КонецЕсли
...
Функция ИзвлечьТекстВоВременноеХранилище(ПолноеИмяФайла, УникальныйИдентификатор = "", Отказ = Ложь,
	...
	#Если Не ВебКлиент Тогда
		...
		#Если Клиент Тогда
			РезультатЗагрузки = РаботаСФайламиСлужебныйКлиент.ПоместитьФайлСДискаВоВременноеХранилище(ИмяВременногоФайла, , УникальныйИдентификатор);
...
Процедура ЗаполнитьСтатусПодписи(СтрокаПодписи, ТекстОшибки = "") Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		СтрокаПодписи.ОписаниеОшибки = ТекстОшибки;
	Иначе
		СтрокаПодписи.Статус = НСтр("ru = 'Неверна'");
	КонецЕсли;
