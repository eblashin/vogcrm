...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.3.3.28";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1c4d3aef-eb91-49d9-aa76-06ee605e9c82");
	Обработчик.Процедура = "Документы.ЭлектронноеПисьмоИсходящее.ЗаполнитьВзаимодействияОснованияУПодчиненныхПисем";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЭлектронноеПисьмоИсходящее.ЗаполнитьВзаимодействияОснованияУПодчиненныхПисемКОбработке";
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ЧитаемыеОбъекты      = "Документ.ЭлектронноеПисьмоВходящее, Документ.ЭлектронноеПисьмоИсходящее";
	...
	Приоритет.Процедура = "Документы.ЭлектронноеПисьмоИсходящее.ЗаполнитьСписокКонтактовИсходящихЭлектронныхПисем";
	Приоритет.Порядок = "До";
	Обработчик.Комментарий = НСтр("ru = 'Выполняет дозаполнение реквизита ВзаимодействиеОснование для исходящее писем, у которых письмо основание в ИБ есть, но оно не указано.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.3.4.118";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("eb7fe7da-ffd1-437f-85c7-8d673c79c4f0");
	Обработчик.Процедура = "Документы.ЭлектронноеПисьмоИсходящее.ЗаполнитьСписокКонтактовИсходящихЭлектронныхПисем";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЭлектронноеПисьмоИсходящее.ОпределитьНезаполненныеКонтактыВИсходящихЭлектронныхПисьмах";
	Обработчик.ОчередьОтложеннойОбработки = 2;
	Обработчик.ЧитаемыеОбъекты      = "Документ.ЭлектронноеПисьмоИсходящее";
	Обработчик.ИзменяемыеОбъекты    = "Документ.ЭлектронноеПисьмоИсходящее";
	Обработчик.Комментарий = НСтр("ru = 'Выполняет определение контакта по электронной почте для исходящих писем, у которых он не установлен.'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	Приоритет = Обработчик.ПриоритетыВыполнения.Добавить();
	Приоритет.Процедура = "Документы.ЭлектронноеПисьмоВходящее.ЗаполнитьВзаимодействияОснованияУПодчиненныхПисем";
	...
	Обработчик.Процедура = "Взаимодействия.ОбновитьПерсональныеУчетныеЗаписи";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("d57f7a36-46ca-4a52-baab-db960e3d376d");
	Обработчик.Комментарий = НСтр("ru = 'Обновляет сведения о персональных учетных записях.'");
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Взаимодействия.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ЧитаемыеОбъекты      = "РегистрСведений.НастройкиУчетныхЗаписейЭлектроннойПочты, Справочник.УчетныеЗаписиЭлектроннойПочты";
...
// Заполняет владельца в учетных записях электронной почты.
Процедура ОбновитьПерсональныеУчетныеЗаписи(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		УчетнаяЗапись.ВладелецУчетнойЗаписи = Выборка.ВладелецУчетнойЗаписи;
...
Процедура УстановитьТекущимКонтакт(Контакт, Форма) Экспорт
	...
	Иначе
		...
		МассивОписанийКонтактов = ВзаимодействияКлиентСервер.МассивОписанияВозможныхКонтактов();
...
Функция ПолучитьВсеКонтактыПоПредметуИлиЦепочке(Предмет, ВключатьEmail)
	...
	Иначе
		...
		ТекстЗапроса = ТекстЗапроса 
		         + ВзаимодействияПереопределяемый.ТекстЗапросаПоискКонтактовПоПредмету(Ложь, ИмяМетаданных, Истина);
	КонецЕсли;
...
Функция СоздатьСписокЗначенийИнтерактивноСоздаваемыхКонтактов() Экспорт
	...
	МассивОписанияВозможныхТиповКонтактов = ВзаимодействияКлиентСервер.МассивОписанияВозможныхКонтактов();
...
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма) Экспорт
	...
	МассивОписанийВозможныхКонтактов = ВзаимодействияКлиентСервер.МассивОписанияВозможныхКонтактов();
...
Функция СформироватьЭлементДанныхШапкиПисьма(ЭлементРодитель,Выборка,ТолькоПоПредставлениюОтправителя = Ложь) Экспорт
	...
	ДобавитьСтрокуВТаблицу(ЭлементТаблица, "Subject: ", Выборка.Тема);
...
Функция ПользовательЯвляетсяОтветственнымЗаВедениеПапок(УчетнаяЗапись) Экспорт
	...
	// +CRM
	Возврат CRM_Взаимодействия.ПользовательЯвляетсяОтветственнымЗаВедениеПапок(УчетнаяЗапись);
	// -CRM
...
Процедура ПриЗаписиДокумента(ДокументОбъект) Экспорт
	...
	ОписаниеТиповКонтактов = Новый ОписаниеТипов(ВзаимодействияКлиентСервер.МассивВозможныхТиповКонтактов());
