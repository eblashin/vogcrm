...
Функция ТекстЗапросаДляОчисткиФайлов(ВладелецФайла, Настройка, МассивИсключений, ЭлементИсключение, ДанныеДляОтчета = Ложь) Экспорт
	...
	Если ЕстьВозможностьХранитьВерсии Тогда
		СправочникВерсийФайлов =
			ОбщегоНазначения.ИдентификаторОбъектаМетаданных(МетаданныеОбъектаФайлов.Реквизиты.ТекущаяВерсия.Тип.Типы()[0]);
		...
		Если Настройка.ПериодОчистки <> Перечисления.ПериодОчисткиФайлов.ПоПравилу Тогда
			Если ДанныеДляОтчета Тогда
				ТекстЗапроса = 
					...
					|	ЕстьNull(ВерсииФайлов.Размер, Файлы.Размер) /1024 /1024 КАК ОбъемНенужныхФайлов";
			Иначе
				ТекстЗапроса = 
					"ВЫБРАТЬ 
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|	И ВЫБОР
				|			КОГДА ЕстьNull(ВерсииФайлов.ТипХраненияФайла, Файлы.ТипХраненияФайла) = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВТомахНаДиске)
				|				ТОГДА ЕстьNull(ВерсииФайлов.Том, Файлы.Том) <> ЗНАЧЕНИЕ(Справочник.ТомаХраненияФайлов.ПустаяСсылка)
				|						ИЛИ (ВЫРАЗИТЬ(ЕстьNull(ВерсииФайлов.ПутьКФайлу, Файлы.ПутьКФайлу) КАК СТРОКА(100))) <> """"
				|			ИНАЧЕ ИСТИНА
				|		КОНЕЦ
				|	И ЕстьNull(ВерсииФайлов.ДатаСоздания, Файлы.ДатаСоздания) <= &ПериодОчистки";
		Иначе
	...
	Иначе
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|		КОНЕЦ
			|	И ТИПЗНАЧЕНИЯ(Файлы.ВладелецФайла) = &ТипВладельца
			|	И Файлы.ДатаСоздания <= &ПериодОчистки";
	Иначе
		МассивРеквизитовСТипомДата = Новый Массив;
...
// 
Процедура УдалитьФайлыНаСервере(ПрежнийПутьНаТоме)
...
Функция ДобавитьФайлВТом(ДвоичныеДанныеИлиПуть, ВремяИзмененияУниверсальное, ИмяБезРасширения, Расширение,
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр("РаботаСФайламиСлужебный.ДобавитьНаДиск", "ДвоичныеДанныеИлиПуть", ДвоичныеДанныеИлиПуть,	
...
Функция ОбъектыИсключенияПриОчисткеФайлов() Экспорт
	...
	НастройкиФайлов = НастройкиФайлов();
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ЧитаемыеОбъекты      = РегистрыСведений.СведенияОФайлах.ЧитаемыеОбъектыПриОбновленииИБ();
	...
	Обработчик.Версия = "2.4.1.1";
	Обработчик.Комментарий =
			НСтр("ru = 'Обновление универсальной даты обновления элементов справочника Файлы.'");
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		Если НЕ ЭлементДанных.ЭтоНовый() Тогда
			...
			Если ВерсияФайла.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
				...
				УдалитьФайлыНаСервере(ПрежнийПутьНаТоме);
...
Функция КоличествоЗанятыхФайлов(Пользователь)
	...
	Запрос.Текст = 
		...
		|	И СведенияОФайлах.Редактирует = &Пользователь";
...
Функция ЗаполнитьДанныеФайлаПоШаблону(Контекст, ОбъектЗначение, Параметры, НастраиватьОбъектФормы)
	...
	ОбъектЗначение.ДатаМодификацииУниверсальная = ДатаСоздания;
...
Процедура НастроитьОбъектФормы(Знач НовыйОбъект, Контекст)
	...
	КонецЕсли;
	МассивИсключений = Новый Массив;
	МассивИсключений.Добавить("РегистрСведений.ПрисоединенныеФайлы");
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			Для Каждого ЭлектроннаяПодписьФайла Из ЭлектронныеПодписи Цикл
				...
				НоваяСтрока.АдресПодписи        = ПоместитьВоВременноеХранилище(
					ЭлектроннаяПодписьФайла.Подпись, Контекст.УникальныйИдентификатор);
...
Функция ВозможностьСозданияПапки(ОбъектВладелец)
	...
	Если АдресЗагруженногоФайла <> Неопределено Тогда
		...
		Иначе
			...
			Если ЭтоФайл Тогда
				...
				РаботаСФайлами.ОбновитьФайл(СсылкаСуществующегоФайла, ИнформацияОФайле);
		...
		Если СтрокаДереваФайлов.ЭтоПапка Тогда
			...
			Если (ТекущаяПапкаФайлов = Неопределено) И (ТаблицаФайлов.Найти(СтрокаДереваФайлов.Href, "Href") = Неопределено) Тогда
				...
				Если ПодсистемаРаботаСФайламиСуществует Тогда
					...
					ВозможностьСозданияПапки = ВозможностьСозданияПапки(ОбъектВладелец);
					...
					Попытка
						...
						ТекущаяПапкаФайлов = РаботаСФайламиСлужебныйВызовСервера.СправочникиПапкиСоздатьЭлемент(СтрокаДереваФайлов.ИмяФайла, ОбъектВладелец, ПараметрыСинхронизации.АвторФайлов);
...
Процедура СинхронизироватьФайлыСОблачнымСервисом(УчетнаяЗапись)
	...
	Попытка
		...
		ТаблицаФайлов.Сортировать("ЭтоПапка УБЫВ, Уровень, РодительСортировка", ОбъектСравнения);
