...
Процедура ВызватьОнлайнПоддержкуПослеПроверкиПутиИзХранилища(ПутьИзХранилищаВерен, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ВызватьОнлайнПоддержкуПослеЗапускаПриложения", ЭтотОбъект);
	Если УчетнаяЗапись.ИспользоватьЛП Тогда
		...
		Если ПутьИзХранилищаВерен Тогда
			НачатьЗапускПриложения(Оповещение, ДополнительныеПараметры.ПутьИзХранилища + ПараметрыЗапускаВызовОнлайнПоддержки);
			Возврат;
		КонецЕсли;
		
		Если ДополнительныеПараметры.ПутьИзРеестраВерен Тогда
			НачатьЗапускПриложения(Оповещение, ДополнительныеПараметры.ПутьИзРеестра + ПараметрыЗапускаВызовОнлайнПоддержки);
			Возврат;
		КонецЕсли;
...
// Продолжение процедуры (см. выше).
Процедура ВызватьОнлайнПоддержкуПослеЗапускаПриложения(КодВозврата, ДополнительныеПараметры) Экспорт
