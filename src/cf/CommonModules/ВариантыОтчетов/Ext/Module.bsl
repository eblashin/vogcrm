...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения, АдресРезультата) Экспорт
	...
	|	ВЫБОР
	|		КОГДА &ЭтоОсновнойЯзык
	// +++ VOG Кулаков П.Л. 10.02.2021 DEV-79
	...
	|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ЕСТЬNULL(ВариантыОтчетов.Описание, ОтчетыРасширений.Описание), ОтчетыКонфигурации.Описание) КАК СТРОКА(1000))
	// --- VOG Кулаков П.Л.
