...
Процедура ПриОпределенииСвойствСправочниковКонтрагентов(СвойстваСправочников) Экспорт
	...
	//START Кайдашов 05/07/19 392
	НоваяСтрока = СвойстваСправочников.Добавить();
	НоваяСтрока.Имя = "Партнеры";
	НоваяСтрока.Иерархический = Истина;
	НоваяСтрока.РеквизитИНН = "CRM_ИНН";
	НоваяСтрока.РеквизитКПП = "CRM_КПП";
	НоваяСтрока.ОтключитьСозданиеИзДосьеКонтрагента = Ложь;
	//END Кайдашов
	// -CRM не переносить в объединенные решения
	//{[+](фрагмент добавлен), Виноградов Дмитрий 07.06.2018 12:36:48 ID
	НоваяСтрока = СвойстваСправочников.Добавить();
	НоваяСтрока.Имя = "вогЮридическиеЛица";
	НоваяСтрока.Иерархический = Ложь;
	НоваяСтрока.РеквизитИНН = "CRM_ИНН";
	НоваяСтрока.РеквизитКПП = "CRM_КПП";
	НоваяСтрока.ОтключитьСозданиеИзДосьеКонтрагента = Ложь;
	//}Администратор 07.06.2018 12:36:48
КонецПроцедуры
...
Процедура ДополнитьРеквизитыФормыКонтрагент(Форма, НовыеРеквизиты) Экспорт
	...
	Если Форма.ИмяФормы = "Справочник.вогЮридическиеЛица.Форма.ФормаЭлемента" Тогда
		
		СтруктураРеквизитов = Новый Структура;
		СтруктураРеквизитов.Вставить("ЮрФизЛицо");
		
		РеквизитыОрганизационноПравоваяФорма = 
			ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Форма.Объект.ОрганизационноПравоваяФорма, СтруктураРеквизитов);
		
		НовыеРеквизиты.ЭтоЮридическоеЛицо = (РеквизитыОрганизационноПравоваяФорма.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо);
