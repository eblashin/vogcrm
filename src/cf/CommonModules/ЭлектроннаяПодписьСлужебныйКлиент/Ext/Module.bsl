...
Процедура СохранитьДанныеСертификатаВместеСПодписьюЦиклВнутреннийЦиклПослеПроверкиСуществованияФайла(Существует, Контекст) Экспорт
	...
	Контекст.ИмяФайлаСертификата = Контекст.ИмяФайлаСертификатаБезРасширения
		+ " (" + Строка(Контекст.Счетчик) + ")" + "." + Контекст.ОписаниеПодписи.РасширениеСертификата;
	
	Контекст.ИмяФайлаСертификата = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(Контекст.ИмяФайлаСертификата);
...
Процедура СохранитьДанныеСертификатаВместеСПодписьюЦиклНачало(Контекст)
	...
	Контекст.ИмяФайлаСертификата = Контекст.СоставИмениФайлаДанных.ИмяБезРасширения + " - "
...
Процедура ВыполнитьНаСторонеЦиклПослеПодписания(СвойстваПодписи, Контекст)
	...
	Если Не ЭлементДанных.Свойство("Объект") Тогда
		ЭлектроннаяПодписьСлужебныйВызовСервера.ЗарегистрироватьПодписаниеДанныхВЖурнале(
			СвойстваТекущегоЭлементаДанных(Контекст, СвойстваПодписи.ДатаПодписи));
		ВыполнитьНаСторонеЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
// Только для внутреннего использования.
Функция СвойстваТекущегоЭлементаДанных(ПараметрыВыполнения, ДатаПодписи = '00010101') Экспорт
