...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(НовыеПользовательскиеНастройкиКД)
	...
	// +++ VOG Кулаков П.Л. 18.06.2021 CRM-665
	...
	ПодразделениеВог	 			= Справочники.СтруктураПредприятия.ПолучитьСсылку(Новый УникальныйИдентификатор("e040cc1f-0c11-11e8-92c3-005056bcf152"));
	...
	ПолеКомпоновкиДанныхКоллекция = Новый ПолеКомпоновкиДанных("Подразделение");
	...
	Если ЗначениеЗаполнено(ОбособленноеПодразделение) И ОбособленноеПодразделение <> ПодразделениеВог Тогда
		Если ПользовательскаяНастройка <> Неопределено Тогда
			ПользовательскаяНастройка.ПравоеЗначение = ОбособленноеПодразделение;
