...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипОтчетаСтрокой = Неопределено ИЛИ НЕ ЗначениеЗаполнено(ТипОтчетаСтрокой) Тогда
	...
	// +++ VOG Кулаков П.Л. 05.08.2020
	Если НЕ Пользователи.РолиДоступны("вогВлючениеВПланКомандировок") Тогда 
		Элементы.ВключитьВПлан.Видимость = Ложь;
	КонецЕсли;
	// --- VOG Кулаков П.Л.
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(НовыеПользовательскиеНастройкиКД)
	...
	// +++ VOG Кулаков П.Л. 28.07.2020 CRM-776
	Если Параметры.Свойство("ПоказыватьСогласование") И Параметры.ПоказыватьСогласование Тогда
		
		Для Каждого ЭлементНастройки Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
			Если ТипЗнч(ЭлементНастройки) = Тип("ГруппировкаКомпоновкиДанных") Тогда
...
Процедура УстановитьПериод(Форма, Знач Путь) Экспорт 
	...
	ИдентификаторЭлемента = Прав(Путь, 32);
	
	// Запись значения в пользовательские настройки компоновки данных.
	ПользовательскаяНастройкаКД = НайтиПользовательскуюНастройкуЭлемента(ИдентификаторЭлемента);
	Если ТипЗнч(ПользовательскаяНастройкаКД) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
		ПользовательскаяНастройкаКД.Значение = ЭтотОбъект[Путь];
	Иначе
		ПользовательскаяНастройкаКД.ПравоеЗначение = ЭтотОбъект[Путь];
	КонецЕсли;
	Если ЗначениеЗаполнено(ЭтотОбъект[Путь]) Тогда
		ПользовательскаяНастройкаКД.Использование = Истина;
	КонецЕсли;
...
Процедура ПослеФормированияНаКлиенте()
	...
	ПолеКомпоновкиНаправлениеДеятельности = Новый ПараметрКомпоновкиДанных("НаправлениеДеятельности");
	ПолеКомпоновкиПериод 				  = Новый ПараметрКомпоновкиДанных("Период");
	
	ИдентификаторНастройки = "";
	ОтборКомпоновкиПодразделение = Новый ПолеКомпоновкиДанных("НаименованиеПодразделенияДляОтбора");
	Для Каждого Настройка Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если ТипЗнч(Настройка) = Тип("ЭлементОтбораКомпоновкиДанных")
				И Настройка.ЛевоеЗначение = ОтборКомпоновкиПодразделение  Тогда
			ИдентификаторНастройки = Настройка.ИдентификаторПользовательскойНастройки;	
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Настройка Из ПользовательскиеНастройки.Элементы Цикл
		Если ТипЗнч(Настройка) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") 
				И Настройка.Параметр = ПолеКомпоновкиНаправлениеДеятельности 
				И Настройка.Использование Тогда
			НаправлениеДеятельности = Настройка.Значение;
		ИначеЕсли ТипЗнч(Настройка) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") 
				И Настройка.Параметр = ПолеКомпоновкиПериод 
				И Настройка.Использование Тогда
			ДатаНачала = Настройка.Значение.ДатаНачала;
		ИначеЕсли ТипЗнч(Настройка) = Тип("ЭлементОтбораКомпоновкиДанных")
				И Настройка.ИдентификаторПользовательскойНастройки = ИдентификаторНастройки 
				И Настройка.Использование Тогда
			Организация = Настройка.ПравоеЗначение;
		КонецЕсли;
	КонецЦикла;
	ДиректорТН = "";
	Если НаправлениеДеятельности <> Неопределено Тогда
		ДиректорТН = ПолучитьДиректораТоварногоНаправления(НаправлениеДеятельности);
	КонецЕсли;
	
	ГенеральныйДиректор = "";
	Если Организация <> "" Тогда
		ГенеральныйДиректор = ПолучитьГенеральногоДиректораПоОрганизации(Организация);
	КонецЕсли;
	
	ОтчетТабличныйДокумент.ПоказатьУровеньГруппировокСтрок(1);
	ОтчетТабличныйДокумент.ПоказатьУровеньГруппировокКолонок(0);
	ТекстСлеваВерхнегоКолонтитула = "Утверждено" + Символы.ПС + "Генеральный директор" + Символы.ПС + "ООО ""ВОГ-ИНВЕСТ""" + Символы.ПС + "Корнеева О.Ю._______________________" + Символы.ПС + Символы.ПС + НаправлениеДеятельности + Символы.Таб + "	период: " + Формат(ДатаНачала,"ДФ='MMMM yyyy'");
	ОтчетТабличныйДокумент.ВерхнийКолонтитул.ТекстСлева = ТекстСлеваВерхнегоКолонтитула;
	ОтчетТабличныйДокумент.ВерхнийКолонтитул.Выводить = Истина;
	ОтчетТабличныйДокумент.ВерхнийКолонтитул.НачальнаяСтраница = 1;
	
	ТекстСправаВерхнегоКолонтитула = """Согласовано""" + Символы.ПС + "Директор ТН " + НаправлениеДеятельности + Символы.ПС + ДиректорТН + "  Согласовано" + Символы.ПС + Символы.ПС + " ";
	ОтчетТабличныйДокумент.ВерхнийКолонтитул.ТекстСправа = ТекстСправаВерхнегоКолонтитула;
	
	ТекстВЦентреВерхнегоКолонтитула = """Согласовано""" + Символы.ПС + "Генеральный директор" + Символы.ПС + ГенеральныйДиректор + "  Согласовано" + Символы.ПС + Символы.ПС + " ";
	//ТекстВЦентреВерхнегоКолонтитула = ТекстВЦентреВерхнегоКолонтитула + Символы.ПС + Символы.ПС + НаправлениеДеятельности + Символы.Таб + "	период: " + Формат(ДатаНачала,"ДФ='MMMM yyyy'");
	ОтчетТабличныйДокумент.ВерхнийКолонтитул.ТекстВЦентре = ТекстВЦентреВерхнегоКолонтитула;
	ОтчетТабличныйДокумент.РазмерКолонтитулаСверху = 30;
	ОтчетТабличныйДокумент.ВерхнийКолонтитул.Шрифт = Новый Шрифт(,8);
	ОтчетТабличныйДокумент.ПолеСверху = 30;
	ОтчетТабличныйДокумент.ВерхнийКолонтитул.ВертикальноеПоложение = ВертикальноеПоложение.Низ;
...
// +++ VOG Кулаков П.Л. 21.07.2020 CRM-796
Функция ПолучитьГенеральногоДиректораПоОрганизации(Организация)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	вогОтветственныеЛицаОрганизаций.ФизическоеЛицо КАК ФизическоеЛицо
		|ИЗ
		|	Справочник.вогОтветственныеЛицаОрганизаций КАК вогОтветственныеЛицаОрганизаций
		|ГДЕ
		|	вогОтветственныеЛицаОрганизаций.Должность = ""Генеральный директор""
		|	И вогОтветственныеЛицаОрганизаций.ДатаОкончания = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
		|	И вогОтветственныеЛицаОрганизаций.Владелец.НазваниеДляОтчетаГенеральномуДиректору = &Наименование";
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Директор = ФизическиеЛица.ФамилияИнициалыФизЛица(ВыборкаДетальныеЗаписи.ФизическоеЛицо);
	Иначе
		Директор = "";
	КонецЕсли;
...
// +++ VOG Кулаков П.Л. 21.07.2020 CRM-796
Функция ПолучитьДиректораТоварногоНаправления(НаправлениеДеятельности)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	Пользователи.CRM_ДолжностнаяПозиция.ЭталоннаяПозиция = &ЭталоннаяПозиция
		|	И Пользователи.CRM_НаправлениеДеятельности = &CRM_НаправлениеДеятельности";
	
	Запрос.УстановитьПараметр("CRM_НаправлениеДеятельности", НаправлениеДеятельности);
	ЭталоннаяПозиция = Справочники.CRM_ДолжностныеПозиции.НайтиПоКоду("000000100"); // Директор по развитию
	Запрос.УстановитьПараметр("ЭталоннаяПозиция", ЭталоннаяПозиция);
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		ДиректорТН = ФизическиеЛица.ФамилияИнициалыФизЛица(ВыборкаДетальныеЗаписи.Наименование);
	Иначе
		ДиректорТН = "";
	КонецЕсли;
...
Процедура ЗаполнитьКомандыВыбораВариантов()
	...
	РезультатПоиска = ВариантыОтчетов.НайтиВариантыОтчетов(ПараметрыПоиска, Истина);
...
Процедура ВывестиЭлементыНастройки(Форма, Элементы, СвойстваНастройки, ГруппаВывода, Прочее)
	...
	Если СвойстваНастройки.ТипЭлементов <> "" Тогда
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СтандартныйПериод" Тогда
			...
			КнопкаВыбораИмя    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонИмениЭлемента, "Значение");
...
Процедура Согласовать(Команда)
	...
	Если ОтчетТабличныйДокумент.ВыделенныеОбласти.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	МассивИдентификаторовРасшифровки = ПолучитьМассивИдентификаторовРасшифровки();
	Если МассивИдентификаторовРасшифровки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	СписокКомандировок = СформироватьСписокЗадачНаСервере(МассивИдентификаторовРасшифровки);
	
	ПараметрыФормы = Новый Структура;
	ОписаниеОповещения = Новый ОписаниеОповещения("СогласоватьЗадачуЗавершение", ЭтотОбъект,Новый Структура("ВидСогласования,СписокКомандировок",ВидСогласования, СписокКомандировок));
...
Процедура СогласоватьЗадачуЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	РезультатСогласования = СогласоватьЗадачи(ДополнительныеПараметры.СписокКомандировок, ДополнительныеПараметры.ВидСогласования,Результат.РезультатВыполнения);
	
	Если РезультатСогласования Тогда
		Сформировать();
	Иначе
...
&НаСервере
Функция СформироватьСписокЗадачНаСервере(МассивИдентификаторовРасшифровки)
	...
	Для Каждого Идентификатор Из МассивИдентификаторовРасшифровки Цикл
		Если ТипЗнч(Идентификатор) <> Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
		ПоляРасшифровки = ДанныеРасшифровки.Элементы[Идентификатор].ПолучитьПоля();
		Если ПоляРасшифровки.Количество() > 0 И ТипЗнч(ПоляРасшифровки[0].Значение) = Тип("ДокументСсылка.вогКомандировка") Тогда
			СписокСсылок.Добавить(ПоляРасшифровки[0].Значение);
		КонецЕсли;
	КонецЦикла;
	
	СписокСсылок = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СписокСсылок);
	Возврат СписокСсылок;
	
КонецФункции

&НаСервере
Функция ПолучитьСписокЗадачПоДокументамКомандировки(СписокДокументов)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ЗадачаИсполнителяЗадачиПоИсполнителю.Исполнитель = ЗНАЧЕНИЕ(Справочник.Пользователи.)
		|				ИЛИ ЗадачаИсполнителяЗадачиПоИсполнителю.Исполнитель ЕСТЬ NULL
		|			ТОГДА ЗадачаИсполнителяЗадачиПоИсполнителю.РольИсполнителя
		|		ИНАЧЕ ЗадачаИсполнителяЗадачиПоИсполнителю.Исполнитель
		|	КОНЕЦ КАК Исполнитель,
		|	ЗадачаИсполнителяЗадачиПоИсполнителю.БизнесПроцесс.Предмет КАК Командировка
		|ИЗ
		|	Задача.ЗадачаИсполнителя.ЗадачиПоИсполнителю(
		|			&Исполнитель,
		|			НЕ БизнесПроцесс.Завершен
		|				И БизнесПроцесс.КартаМаршрута В (&КартаМаршрута)
		|				И БизнесПроцесс.Предмет В (&Документы)
...
&НаСервере
Функция СогласоватьЗадачи(Командировки, ВидСогласования, РезультатВыполнения = "")
	
	РезультатЗапроса = ПолучитьСписокЗадачПоДокументамКомандировки(Командировки);
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Сообщить("Выполнена задача: " + ВыборкаДетальныеЗаписи.Задача);
	КонецЦикла;
...
&НаКлиенте
Функция ПолучитьМассивИдентификаторовРасшифровки()
	...
	Для Каждого ВыделеннаяОбласть Из ВыделенныеОбласти Цикл
		...
		ВыделеннаяОбластьПраво = ОтчетТабличныйДокумент.ШиринаСтраницы;
		...
		Если ВыделеннаяОбластьНиз = 0 Тогда
			ВыделеннаяОбластьНиз = ОтчетТабличныйДокумент.ВысотаТаблицы;
		КонецЕсли;
		...
		Если ВыделеннаяОбластьПраво = 0 Тогда
			ВыделеннаяОбластьПраво = ОтчетТабличныйДокумент.ШиринаТаблицы;
		КонецЕсли;
		...
		Для НомерКолонки = ВыделеннаяОбластьЛево По ВыделеннаяОбластьПраво Цикл
			Для НомерСтроки = ВыделеннаяОбластьВерх По ВыделеннаяОбластьНиз Цикл
				Ячейка = ОтчетТабличныйДокумент.Область(НомерСтроки, НомерКолонки, НомерСтроки, НомерКолонки);
				Результат.Добавить(Ячейка.Расшифровка);
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Процедура ОтправитьНаДоработку(Команда)
	...
	СписокЗадач = СформироватьСписокЗадачНаСервере(МассивИдентификаторовРасшифровки);
	Результат = СогласоватьЗадачи(СписокЗадач, Элементы.Согласовать2.Заголовок);
...
Процедура Отказать(Команда)
	...
	СписокЗадач = СформироватьСписокЗадачНаСервере(МассивИдентификаторовРасшифровки);
	Результат = СогласоватьЗадачи(СписокЗадач, Элементы.Согласовать3.Заголовок);
	Если Результат Тогда
		Сформировать();
	КонецЕсли;
...
&НаКлиенте
Процедура ВключитьВПлан(Команда)
	...
	Если МассивИдентификаторовРасшифровки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ВключитьВПланНаСервере(МассивИдентификаторовРасшифровки);
	
КонецПроцедуры

&НаСервере
Процедура ВключитьВПланНаСервере(МассивИдентификаторовРасшифровки)
	...
	СписокСсылок = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СписокСсылок);
	Для Каждого Командировка Из СписокСсылок Цикл
		ВключатьВПлан = ПроверитьВозможностьВключенияВПлан(Командировка);
		Если ВключатьВПлан Тогда
			ВключитьВПланКомандировку(Командировка);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ПроверитьВозможностьВключенияВПлан(Командировка)
	...
	Запрос.Текст = 
		...
		|		ПО вогКомандировкаКомандированныеСотрудники.Ссылка = CRM_БизнесПроцесс.Предмет
		|			И (CRM_БизнесПроцесс.Стартован)
		|			И (НЕ CRM_БизнесПроцесс.Завершен)
		|			И (НЕ CRM_БизнесПроцесс.ПометкаУдаления)
		|			И (вогКомандировкаКомандированныеСотрудники.Сотрудник <> ЗНАЧЕНИЕ(Справочник.Пользователи.Водитель))
		...
		|	И (НЕ вогЭтапыСогласованияКомандировок.ТребуетсяСогласоватьЗадачи
		|			ИЛИ вогЭтапыСогласованияКомандировок.ТребуетсяСогласоватьЗадачи ЕСТЬ NULL)
		|	И вогКомандировкаКомандированныеСотрудники.Ссылка = &Ссылка
		|	И вогКомандировкаКомандированныеСотрудники.Ссылка.Статус = &Статус";
	
	Запрос.УстановитьПараметр("Ссылка", Командировка);
	Запрос.УстановитьПараметр("Статус", Перечисления.вогСтатусыКомандировок.Создан);
...
&НаСервере
Процедура ВключитьВПланКомандировку(Командировка)
	
	
	Для Каждого Строка Из Командировка.КомандированныеСотрудники Цикл
		
		Если Строка.Сотрудник = Справочники.Пользователи.Водитель Тогда
			Продолжить;
		КонецЕсли;
		
		РегистрыСведений.вогЭтапыСогласованияКомандировок.УстановитьЭтапСогласования(Строка.Сотрудник,Командировка,Перечисления.вогЭтапыСогласованияКомандировок.Согласовано, Истина);
		
		РегистрыСведений.вогФинансированиеКомандировок.УстановитьФинансирование(Строка.Сотрудник, Командировка, Перечисления.вогВидыФинансирования.ВБюджете);
		
	КонецЦикла;
	
	ДокОб = Командировка.ПолучитьОбъект();
	ДокОб.Статус = Перечисления.вогСтатусыКомандировок.Согласован;
	ТаблицаСотрудников = ДокОб.КомандированныеСотрудники.Выгрузить();
	...
	Запрос.УстановитьПараметр("ДатаКомандировки", ДокОб.ВремяНачала);
	
	ТЗ = Запрос.Выполнить().Выгрузить();	
	
	Суточные = ТЗ.Итог("Сумма");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	вогКомандировкаЗадачи.Ссылка КАК Ссылка,
			|	СУММА(CRM_Мероприятие.ПредставительскиеРасходы) КАК ПредставительскиеРасходы
			|ИЗ
			|	Документ.вогКомандировка.Задачи КАК вогКомандировкаЗадачи
			|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.CRM_Мероприятие КАК CRM_Мероприятие
			|		ПО (вогКомандировкаЗадачи.Задача = CRM_Мероприятие.Ссылка)
			|ГДЕ
			|	вогКомандировкаЗадачи.Ссылка = &Ссылка
			|
			|СГРУППИРОВАТЬ ПО
	...
	ПредставительскиеРасходы = РезультатЗапроса.Итог("ПредставительскиеРасходы");
	Проживание = ДокОб.РасходыПроживание.Итог("Стоимость");	
	Проезд = ДокОб.РасходыПроезд.Итог("Стоимость");
	СуммаДополнительныеРасходы = ДокОб.ДополнительныеРасходы.Итог("Сумма");
	Итого = Суточные + ПредставительскиеРасходы + Проживание + Проезд + СуммаДополнительныеРасходы;
	ДокОб.ПредыдущийБюджет = Итого;
	
	ДокОб.Записать();
	
	Сообщить("Командировка №" + ДокОб.Номер + " включена в план");
...
Процедура ОтчетТабличныйДокументПриАктивизации(Элемент)
	...
	Если ОтчетДанныеРасшифровки = "" Тогда
		Возврат;
	КонецЕсли;
	Если ОтчетТабличныйДокумент.ВыделенныеОбласти.Количество() = 0 Тогда
		Возврат;
	...
	КонецЕсли;
	СписокКомандировок = СформироватьСписокЗадачНаСервере(МассивИдентификаторовРасшифровки);
	УстановитьКнопкиСогласования(СписокКомандировок);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьКнопкиСогласования(СписокКомандировок)
	
	Элементы.Согласовать1.Видимость = Ложь;
	Элементы.Согласовать2.Видимость = Ложь;
	Элементы.Согласовать3.Видимость = Ложь;
	Элементы.ОтправитьНаДоработкуВыборочно.Видимость = Ложь;
	Элементы.ОтказатьВСогласованииВыборочно.Видимость = Ложь;
	Элементы.ПустаяКоманда.Видимость = Истина;
	...
	Запрос.Текст = 
		...
		|	КОНЕЦ КАК Исполнитель,
		|	ЗадачаИсполнителяЗадачиПоИсполнителю.БизнесПроцесс.Предмет КАК Командировка,
		|	ЗадачаИсполнителяЗадачиПоИсполнителю.CRM_ТочкаМаршрута КАК ТочкаМаршрута,
		|	ЗадачаИсполнителяЗадачиПоИсполнителю.БизнесПроцесс.КартаМаршрута КАК КартаМаршрута
		|ИЗ
		|	Задача.ЗадачаИсполнителя.ЗадачиПоИсполнителю(
		|			&Исполнитель,
		|			НЕ БизнесПроцесс.Завершен
		|				И БизнесПроцесс.КартаМаршрута В (&КартаМаршрута)
		|				И НЕ Выполнена
		|				И БизнесПроцесс.Предмет В (&Документы)) КАК ЗадачаИсполнителяЗадачиПоИсполнителю
		|ГДЕ
	...
	Запрос.УстановитьПараметр("Документы", СписокКомандировок);
	Запрос.УстановитьПараметр("КартаМаршрута", КартаМаршрута);
	Запрос.УстановитьПараметр("Исполнитель", Пользователи.ТекущийПользователь());
	...
	Если РезультатЗапроса.Пустой() Тогда
		Элементы.ГруппаРешениеПоКомандировкам.Заголовок = "Отсутствуют задачи для согласования";
		Возврат;
	КонецЕсли;
	
	ТаблицаРезультат = РезультатЗапроса.Выгрузить();
	ТаблицаРезультат.Свернуть("КартаМаршрута");
	Если ТаблицаРезультат.Количество() <> 1 Тогда
		Элементы.ГруппаРешениеПоКомандировкам.Заголовок = "Выбранные командировки согласуются по разным картам маршрутов";
		Возврат;
	КонецЕсли;
	
	ТаблицаРезультат = РезультатЗапроса.Выгрузить();
	ТаблицаРезультат.Свернуть("ТочкаМаршрута");
	Если ТаблицаРезультат.Количество() <> 1 Тогда
		Элементы.ГруппаРешениеПоКомандировкам.Заголовок = "Выбранные командировки находятся на разных этапах согласования";
		Возврат;
	КонецЕсли;
	...
	Для Индекс = 1 По МассивВариантов.Количество() Цикл
		Элементы["Согласовать" + Индекс].Заголовок = МассивВариантов[Индекс-1];
		Элементы["Согласовать" + Индекс].Видимость = Истина;
	КонецЦикла;
	
	Если КоличествоЗадач > 0 Тогда
		Элементы.ПустаяКоманда.Видимость = Ложь;
		Элементы.ГруппаРешениеПоКомандировкам.Заголовок = "Решение по командировкам";
	КонецЕсли;
		
	Если КоличествоЗадач > 1 Тогда
		Если МассивВариантов.Найти("Отправить на доработку") <> Неопределено Тогда 
			Элементы.ОтправитьНаДоработкуВыборочно.Видимость = Истина;
		КонецЕсли;
	
		Если МассивВариантов.Найти("Отказать в согласовании") <> Неопределено Тогда 
			Элементы.ОтказатьВСогласованииВыборочно.Видимость = Истина;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтправитьНаДоработкуВыборочно(Команда)
	...
	КонецЕсли;
	СписокКомандировок = СформироватьСписокЗадачНаСервере(МассивИдентификаторовРасшифровки);
	Если Команда.Имя = "ОтправитьНаДоработкуВыборочно" Тогда 
		ВариантВыполнения = "Отправить на доработку";
	ИначеЕсли Команда.Имя = "ОтказатьВСогласованииВыборочно" Тогда 
		ВариантВыполнения = "Отказать в согласовании";
	Иначе
		ВариантВыполнения = "";
	КонецЕсли;
	...
	ПараметрыФормы = ОтправитьНаДоработкуВыборочноНаСервере(СписокКомандировок,ВариантВыполнения);
	Если ПараметрыФормы <> Неопределено Тогда
		ОткрытьФорму("Отчет.вогПланПоКомандировкам.Форма.ФормаВыбораУчастников",ПараметрыФормы,ЭтаФорма,,,, Новый ОписаниеОповещения("ВыборочноеСогласованиеЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ОтправитьНаДоработкуВыборочноНаСервере(СписокКомандировок,ВариантВыполнения)
	
	СписокЗадач = ПолучитьСписокЗадачПоДокументамКомандировки(СписокКомандировок);
	Если СписокЗадач.Пустой() Тогда
		Возврат Неопределено;
	КонецЕсли;
	ТаблицаЗадач = СписокЗадач.Выгрузить();
	МассивЗадач = ТаблицаЗадач.ВыгрузитьКолонку("Задача");
	ПараметрыФормы = Новый Структура("СписокЗадач,ВариантВыполнения",МассивЗадач,ВариантВыполнения);
	Возврат ПараметрыФормы;
	
КонецФункции

&НаКлиенте
Процедура ВыборочноеСогласованиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("ОтменаСогласования")
				И Результат.ОтменаСогласования Тогда
			Возврат;
		ИначеЕсли Результат.Свойство("РезультатСогласования") Тогда
			Если Результат.РезультатСогласования.Успешно Тогда
				Сообщить("Задачи выполнены");
				Сформировать();
				ОтчетТабличныйДокументПриАктивизации(Неопределено);
			Иначе
				Сообщить(Результат.РезультатСогласования.ОписаниеОшибки);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
