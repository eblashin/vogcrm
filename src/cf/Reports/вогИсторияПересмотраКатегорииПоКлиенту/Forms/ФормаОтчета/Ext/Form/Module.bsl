...
Процедура УстановитьСтандартныеПараметрыОтчета()
	...
	ПараметрПериод = ОтчетыПереопределяемый.ПолучитьПараметр(Отчет.КомпоновщикНастроек, "Период");
	
	Если Не ПараметрПериод = Неопределено Тогда
		ЗначениеПоУмолчанию = Новый СтандартныйПериод;
		ЗначениеПоУмолчанию.ДатаНачала = НачалоКвартала(ДобавитьМесяц(ТекущаяДата(),-9));
		ЗначениеПоУмолчанию.ДатаОкончания = КонецКвартала(ТекущаяДата());
		ПараметрПериод.Значение 		= ЗначениеПоУмолчанию;
