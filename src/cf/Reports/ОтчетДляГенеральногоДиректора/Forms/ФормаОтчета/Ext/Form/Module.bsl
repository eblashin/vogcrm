...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТипОтчетаСтрокой = ВариантыОтчетовКлиентСервер.ТипОтчетаСтрокой(Параметры.Отчет, Истина);
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	Иначе
		...
		Если ТипЗнч(СхемаУсловий) = Тип("СхемаКомпоновкиДанных") Тогда
			...
			ПараметрИсключаемаяОрганизация = СхемаУсловий.Параметры.Найти("ИсключаемаяОрганизация");
			
			Если Не ПараметрИсключаемаяОрганизация = Неопределено Тогда
				
				ИсключаемаяОрганизация = ПараметрИсключаемаяОрганизация.Значение;
				
				Если ЗначениеЗаполнено(ИсключаемаяОрганизация) Тогда
					
					ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
						Отчет.КомпоновщикНастроек.Настройки.Отбор,
						"ОсновнаяОрганизация",
						ИсключаемаяОрганизация,
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(НовыеПользовательскиеНастройкиКД)
	...
	УстановитьСтандартныеПараметрыОтчета();
...
&НаСервере
Процедура УстановитьСтандартныеПараметрыОтчета()
	
	ПараметрДатаОтчета = ОтчетыКлиентСервер.ПолучитьПараметр(Отчет.КомпоновщикНастроек, "ДатаФормированияОтчета");
	
	Если Не ПараметрДатаОтчета = Неопределено Тогда
		ПараметрДатаОтчета.Значение 		= ТекущаяДатаСеанса();
		ПараметрДатаОтчета.Использование 	= Ложь;
	КонецЕсли;
	
	ПериодПродаж 			= Отчеты.вогЕженедельныйОтчетПланПродаж.ПолучитьПериодПродаж(ТекущаяДатаСеанса());
	ПараметрПериодПродаж 	= ОтчетыКлиентСервер.ПолучитьПараметр(Отчет.КомпоновщикНастроек, "ПериодПродаж");
	
	Если Не ПараметрПериодПродаж = Неопределено Тогда
		ПараметрПериодПродаж.Значение 		= ПериодПродаж;
		ПараметрПериодПродаж.Использование 	= Ложь;
	КонецЕсли;
	
	ПараметрПериодОтчета 	= ОтчетыКлиентСервер.ПолучитьПараметр(Отчет.КомпоновщикНастроек, "ПериодОтчета");
	
	Если Не ПараметрПериодОтчета = Неопределено Тогда
		ПараметрПериодОтчета.Значение 		= ПериодПродаж;
