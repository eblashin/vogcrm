...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(НовыеПользовательскиеНастройкиКД)
	...
	// +++ VOG Кулаков П.Л. 03.03.2021 CRM-25
	Если Параметры.Свойство("РасшифровкаКоллекции") Тогда
		КомпоновщикНастроекРасшифровки = Отчет.КомпоновщикНастроек;
		ПараметрыКомпоновкиРасшифровки = КомпоновщикНастроекРасшифровки.Настройки.ПараметрыДанных;
		
		ПараметрКомпоновки = Новый ПараметрКомпоновкиДанных("ПериодОтчёта");
		Для Каждого ПараметрСтрока Из ПараметрыКомпоновкиРасшифровки.Элементы Цикл
			Если ПараметрСтрока.Параметр = ПараметрКомпоновки Тогда
				ИдПользователяПериод = ПараметрСтрока.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		ПолеКомпоновкиДанныхКоллекция = Новый ПолеКомпоновкиДанных("Коллекция");
		Для Каждого СтрокаОтбора Из КомпоновщикНастроекРасшифровки.Настройки.Отбор.Элементы Цикл
			Если СтрокаОтбора.ЛевоеЗначение = ПолеКомпоновкиДанныхКоллекция Тогда
				ИДПользователяКоллекция = СтрокаОтбора.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		ПолеКомпоновкиДанныхКоллекция = Новый ПолеКомпоновкиДанных("Компания");
		Для Каждого СтрокаОтбора Из КомпоновщикНастроекРасшифровки.Настройки.Отбор.Элементы Цикл
			Если СтрокаОтбора.ЛевоеЗначение = ПолеКомпоновкиДанныхКоллекция Тогда
				ИДПользователяКомпания = СтрокаОтбора.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		ПолеКомпоновкиДанныхКоллекция = Новый ПолеКомпоновкиДанных("ПодразделениеОтчёт");
		Для Каждого СтрокаОтбора Из КомпоновщикНастроекРасшифровки.Настройки.Отбор.Элементы Цикл
			Если СтрокаОтбора.ЛевоеЗначение = ПолеКомпоновкиДанныхКоллекция Тогда
				ИДПользователяПодразделение = СтрокаОтбора.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		// Коллекция
		ПользовательскаяНастройка = КомпоновщикНастроекРасшифровки.ПользовательскиеНастройки.Элементы.Найти(ИДПользователяКоллекция);
		Если ПользовательскаяНастройка <> Неопределено Тогда
			СписокКоллекций = Новый СписокЗначений;
			СписокКоллекций.Добавить(Параметры.РасшифровкаКоллекции.Коллекция);
			ПользовательскаяНастройка.ПравоеЗначение = СписокКоллекций;
			ПользовательскаяНастройка.Использование = Истина;
		КонецЕсли;
		
		// Подразделение
		Если Параметры.РасшифровкаКоллекции.ПодразделениеОтчёт <> Неопределено Тогда
			ПользовательскаяНастройка = КомпоновщикНастроекРасшифровки.ПользовательскиеНастройки.Элементы.Найти(ИДПользователяПодразделение);
			Если ПользовательскаяНастройка <> Неопределено Тогда
				ПользовательскаяНастройка.ПравоеЗначение = Параметры.РасшифровкаКоллекции.ПодразделениеОтчёт;
				ПользовательскаяНастройка.Использование = Истина;
			КонецЕсли;
		КонецЕсли;
		
		// Компания
		Если Параметры.РасшифровкаКоллекции.Компания <> Неопределено Тогда
			ПользовательскаяНастройка = КомпоновщикНастроекРасшифровки.ПользовательскиеНастройки.Элементы.Найти(ИДПользователяКомпания);
			Если ПользовательскаяНастройка <> Неопределено Тогда
				ПользовательскаяНастройка.ПравоеЗначение = Параметры.РасшифровкаКоллекции.Компания;
				ПользовательскаяНастройка.Использование = Истина;
			КонецЕсли;
		КонецЕсли;
		
		// Период
		ПользовательскаяНастройка = КомпоновщикНастроекРасшифровки.ПользовательскиеНастройки.Элементы.Найти(ИдПользователяПериод);
		Если ПользовательскаяНастройка <> Неопределено Тогда
			ПользовательскаяНастройка.Значение = Параметры.РасшифровкаКоллекции.Период;
			ПользовательскаяНастройка.Использование = Истина;
		КонецЕсли;
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	// +++ VOG Кулаков П.Л. 03.03.2021 DEV-25
	ПоляРасшифровки = ПолучитьПоляРасшифровкиКоллекции(Расшифровка);
	КлючВарианта = ПолучитьКлючВарианта();
	Если (КлючВарианта = "Основной" 
				ИЛИ КлючВарианта = "РасстановкаКоллекция" 
				ИЛИ КлючВарианта = "РасстановкПодразделениеКоллекция")
				И ПоляРасшифровки <> Неопределено Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ПараметрКомпоновки = Новый ПараметрКомпоновкиДанных("ПериодОтчёта");
		Период = Неопределено;
		Для Каждого ПараметрСтрока Из Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы Цикл
			Если ПараметрСтрока.Параметр = ПараметрКомпоновки Тогда
				ИДПериод = ПараметрСтрока.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		ПользовательскаяНастройка = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИДПериод);
		
		Если ПользовательскаяНастройка <> Неопределено Тогда
			Период = ПользовательскаяНастройка.Значение;
		Иначе
		...
		ПараметрыФормы.Вставить("КлючВарианта","ПоТРТ");
		ПараметрыФормы.Вставить("КлючНазначенияИспользования", "Расшифровка");
		ПараметрыФормы.Вставить("РасшифровкаКоллекции",Новый Структура("Период,Коллекция,ПодразделениеОтчёт,Компания",Период,ПоляРасшифровки.Коллекция,ПоляРасшифровки.ПодразделениеОтчёт,ПоляРасшифровки.Компания));
		
		ОткрытьФорму("Отчет.АнализПродажКоллекций.Форма.ФормаОтчета", ПараметрыФормы, , Истина);
...
Функция ПолучитьКлючВарианта()
	
	КлючВарианта = ЭтотОбъект.КлючТекущегоВарианта;
	
	Если СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ЭтотОбъект.КлючТекущегоВарианта) Тогда
		ОсновнойВариант = ЭтотОбъект.НастройкиОтчета.ВариантСсылка.Родитель;
		Если ЗначениеЗаполнено(ОсновнойВариант) И ЗначениеЗаполнено(ОсновнойВариант.ПредопределенныйВариант) Тогда
			КлючВарианта = ОсновнойВариант.ПредопределенныйВариант.КлючВарианта;
		КонецЕсли;
	КонецЕсли;
...
// +++ VOG Кулаков П.Л. 03.03.2021 CRM-25
&НаСервере
Функция ПолучитьПоляРасшифровкиКоллекции(ИдентификаторРасшифровки)
	...
	Для Каждого ПолеРасшифровки Из ПоляРасшифровки Цикл
		Если (ПолеРасшифровки.Поле = "Коллекция" ИЛИ ПолеРасшифровки.Поле = "КоллекцияРасстановка")
				И ТипЗнч(ПолеРасшифровки.Значение) = Тип("СправочникСсылка.КоллекцииНоменклатурыПроизводителей")
				И ЗначениеЗаполнено(ПоляРасшифровки[0].Значение) Тогда
			Коллекция = ПолеРасшифровки.Значение;
		ИначеЕсли (ПолеРасшифровки.Поле = "ПодразделениеОтчёт" ИЛИ ПолеРасшифровки.Поле = "ПодразделениеОтчётРасстановка")
				И ТипЗнч(ПолеРасшифровки.Значение) = Тип("СправочникСсылка.СтруктураПредприятия")
				И ЗначениеЗаполнено(ПолеРасшифровки.Значение) Тогда
			Подразделение = ПолеРасшифровки.Значение;
		ИначеЕсли (ПолеРасшифровки.Поле = "Компания" ИЛИ ПолеРасшифровки.Поле = "КомпанияРасстановка")
				И ТипЗнч(ПолеРасшифровки.Значение) = Тип("Строка")
				И НЕ ПустаяСтрока(ПолеРасшифровки.Значение) Тогда
			Компания = ПолеРасшифровки.Значение;
		КонецЕсли;
	КонецЦикла;	

	// +++ VOG Кулаков П.Л. 11.03.2021 DEV-25
	КлючВарианта = ПолучитьКлючВарианта();
	Если КлючВарианта = "РасстановкаКоллекция" 
				ИЛИ (КлючВарианта = "РасстановкПодразделениеКоллекция") Тогда
		ПолеКомпоновкиДанныхПодразделение = Новый ПолеКомпоновкиДанных("ПодразделениеОтчёт");
		ПолеКомпоновкиДанныхПодразделениеРасстановка = Новый ПолеКомпоновкиДанных("ПодразделениеОтчётРасстановка");
		Для Каждого СтрокаОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			Если СтрокаОтбора.ЛевоеЗначение = ПолеКомпоновкиДанныхПодразделение ИЛИ СтрокаОтбора.ЛевоеЗначение = ПолеКомпоновкиДанныхПодразделениеРасстановка Тогда
				ИДПользователяПодразделение = СтрокаОтбора.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		ПользовательскаяНастройка = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИДПользователяПодразделение);
		Если ПользовательскаяНастройка <> Неопределено И ПользовательскаяНастройка.Использование Тогда
			Подразделение = ПользовательскаяНастройка.ПравоеЗначение;
		КонецЕсли;
		
		ПолеКомпоновкиДанныхКомпания = Новый ПолеКомпоновкиДанных("Компания");
		ПолеКомпоновкиДанныхКомпанияРасстановка = Новый ПолеКомпоновкиДанных("КомпанияРасстановка");
		Для Каждого СтрокаОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			Если СтрокаОтбора.ЛевоеЗначение = ПолеКомпоновкиДанныхКомпания ИЛИ СтрокаОтбора.ЛевоеЗначение = ПолеКомпоновкиДанныхКомпанияРасстановка Тогда
				ИДПользователяКомпания = СтрокаОтбора.ИдентификаторПользовательскойНастройки;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		ПользовательскаяНастройка = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(ИДПользователяКомпания);
		Если ПользовательскаяНастройка <> Неопределено И ПользовательскаяНастройка.Использование Тогда
			Компания = ПользовательскаяНастройка.ПравоеЗначение;
		КонецЕсли;
	...
	Если Коллекция = Неопределено Тогда 
		Возврат Неопределено;
	ИначеЕсли ЭтотОбъект.КлючТекущегоВарианта = "Основной" И Подразделение = Неопределено Тогда
		Возврат Неопределено;
	Иначе
		СтруктураВозврата = Новый Структура("Коллекция,ПодразделениеОтчёт,Компания",Коллекция,Подразделение,Компания);
...
Процедура Сформировать()
	...
	// +++ VOG Кулаков П.Л. 31.03.2021 DEV-262
	ЭтоВариантПоТорговымТочкам = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ЭтоВариантПоТорговымТочкам")).Значение;
	Если ЭтоВариантПоТорговымТочкам Тогда
		ПолеНаборДляОтбора = Новый ПолеКомпоновкиДанных("НаборДляОтбора");
		ПолеКоллеция = Новый ПолеКомпоновкиДанных("Коллекция");
		ОтборПоКоллекции = Ложь;
		ИДНаборДляОтбора = "";
		ИДКоллекция = "";
		Для Каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			Если ЭлементОтбора.ЛевоеЗначение = ПолеНаборДляОтбора Тогда
				ИДНаборДляОтбора = ЭлементОтбора.ИдентификаторПользовательскойНастройки;
			ИначеЕсли ЭлементОтбора.ЛевоеЗначение = ПолеКоллеция Тогда
				ИДКоллекция = ЭлементОтбора.ИдентификаторПользовательскойНастройки;
			КонецЕсли;
		КонецЦикла;
		
		Для Каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
			Если ЭлементОтбора.Использование Тогда
				Если ЭлементОтбора.ИдентификаторПользовательскойНастройки = ИДНаборДляОтбора Тогда
					ОтборПоКоллекции = Истина;
				ИначеЕсли ЭлементОтбора.ИдентификаторПользовательскойНастройки = ИДКоллекция Тогда
					ОтборПоКоллекции = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Если НЕ ОтборПоКоллекции Тогда
			Сообщить("Не установлен отбор по коллекции или набору коллекций. Отчет будет сформирован некорректно.");
		КонецЕсли;
	КонецЕсли;
	// --- VOG Кулаков П.Л.
...
Процедура ПослеФормирования(Результат, ЗагрузитьРезультатФормированияОтчета) Экспорт 
	...
	Если ОтчетСформирован Тогда
		ПараметрОтборПоНабору = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ОтборПоНабору"));
		Если ПараметрОтборПоНабору <> Неопределено И ПараметрОтборПоНабору.Использование И ПараметрОтборПоНабору.Значение = ИСТИНА Тогда
			...
			Для Каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
				Если ЭлементОтбора.ЛевоеЗначение = ПолеНабор Тогда
					...
					Для Каждого ПользовательскаяНастройка Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
						Если ПользовательскаяНастройка.ИдентификаторПользовательскойНастройки = ИДНастройки Тогда
							ПользовательскаяНастройка.Использование = Истина;
						КонецЕсли;
					КонецЦикла;
					ВариантМодифицирован = Ложь;
		...
		//ОтборПоКоллекции = Ложь;
		//Для Каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		//	Если ЭлементОтбора.ЛевоеЗначение = ПолеКоллекция ИЛИ ЭлементОтбора.ЛевоеЗначение = ПолеКоллекцияРасстановка Тогда
		//		ОтборПоКоллекции = ЭлементОтбора.Использование;
		//	КонецЕсли;
		//КонецЦикла;
		//Если ЭтоВариантПоТорговымТочкам 
		//		И ПараметрОтборПоНабору <> Неопределено И ПараметрОтборПоНабору.Значение = ЛОЖЬ
		//		И НЕ ОтборПоКоллекции Тогда
		//	Сообщить("Не установлен отбор по коллекции или набору коллекций. Отчет будет сформирован некорректно.");
		//КонецЕсли;			
