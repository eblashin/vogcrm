...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяОтчета = "Структура задолженности клиента";
	...
	Если СформироватьПриОткрытииФормы Тогда
		Отчет.ГруппаДляХолдинга 	= Параметры.Клиент;
		Отчет.Период				= КонецДня(ТекущаяДата());
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,"ВидВзаиморасчетов"			,"Видимость"	,Ложь);
...
Функция ФоновоеЗаданиеЗапустить()
	...
	ПараметрыОтчета.Вставить("ВидВзаиморасчетов",Отчет.ВидВзаиморасчетов);
	ПараметрыОтчета.Вставить("Организация",Отчет.Организация);
	ПараметрыОтчета.Вставить("ГруппаДляХолдинга",Отчет.ГруппаДляХолдинга);
	...
	РезультатФоновогоЗадания 	= ДлительныеОперации.ВыполнитьВФоне("Отчеты.вогСтруктураЗадолженностиКлиента.СформироватьОтчетWebService",
...
//+++ Терпогосян Д.Б. [01.10.2021 9:33:00] № CRM-1169
&НаКлиенте
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	СтандартнаяОбработка = Ложь; 
КонецПроцедуры
//--- Терпогосян Д.Б. [01.10.2021 9:33:06] № CRM-1169 
#КонецОбласти
// } VOG RMV 2019-03-15