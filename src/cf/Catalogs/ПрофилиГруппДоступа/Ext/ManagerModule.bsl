...
Функция НесовместимыеПрофилиГруппДоступа() Экспорт
	...
	Для Каждого ОписаниеПрофиля Из Выгрузка Цикл
		...
		ОписаниеРолей = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторам(
			ОписаниеПрофиля.Роли.ВыгрузитьКолонку("Роль"), Истина);
...
Процедура ВосстановитьСоставРолейРасширений(ЭлементДанных) Экспорт
	...
	|	И ТИПЗНАЧЕНИЯ(РолиПрофилей.Роль) = ТИП(Справочник.ИдентификаторыОбъектовМетаданных)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	// Регистрация профилей для групп доступа которых нужно обновить регистр сведений ТаблицыГруппДоступа.
	РолиИзменены = Ложь;
	СтарыеРоли = РезультатыЗапроса[0].Выгрузить();
	Если СтарыеРоли.Количество() <> ЭлементДанных.Роли.Количество() Тогда
		РолиИзменены = Истина;
	Иначе
		Для Каждого Роль Из ЭлементДанных.Роли Цикл
			Если СтарыеРоли.Найти(Роль, "Роль") = Неопределено Тогда
				РолиИзменены = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Если РолиИзменены Тогда
		ПрофилиСИзмененнымСоставомРолейПриЗагрузке = ПрофилиСИзмененнымСоставомРолейПриЗагрузке();
		Если ПрофилиСИзмененнымСоставомРолейПриЗагрузке.Найти(ЭлементДанных.Ссылка) = Неопределено Тогда
			ПрофилиСИзмененнымСоставомРолейПриЗагрузке.Добавить(ЭлементДанных.Ссылка);
			УстановитьПривилегированныйРежим(Истина);
			СтандартныеПодсистемыСервер.УстановитьПараметрРаботыПрограммы(
				"СтандартныеПодсистемы.УправлениеДоступом.ПрофилиСИзмененнымСоставомРолейПриЗагрузке",
				ПрофилиСИзмененнымСоставомРолейПриЗагрузке);
...
// Только для внутреннего использования.
Процедура ОбновитьТаблицыГруппДоступаПрофилейСИзмененнымСоставомРолейПриЗагрузке() Экспорт
	...
	ПрофилиСИзмененнымСоставомРолейПриЗагрузке = ПрофилиСИзмененнымСоставомРолейПриЗагрузке();
	Если ПрофилиСИзмененнымСоставомРолейПриЗагрузке.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Профиль", ПрофилиСИзмененнымСоставомРолейПриЗагрузке);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	ГруппыДоступа.Профиль В(&Профиль)";
	
	ГруппыДоступаПрофилей = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	РегистрыСведений.ТаблицыГруппДоступа.ОбновитьДанныеРегистра(ГруппыДоступаПрофилей);
...
Функция ПоставляемыеПрофили()
	...
	ИнтеграцияСтандартныхПодсистем.ПриЗаполненииПоставляемыхПрофилейГруппДоступа(
...
Функция ПрофилиСИзмененнымСоставомРолейПриЗагрузке()
	
	УстановитьПривилегированныйРежим(Истина);
	
	ПрофилиСИзмененнымСоставомРолейПриЗагрузке = СтандартныеПодсистемыСервер.ПараметрРаботыПрограммы(
		"СтандартныеПодсистемы.УправлениеДоступом.ПрофилиСИзмененнымСоставомРолейПриЗагрузке");
	
	Если ТипЗнч(ПрофилиСИзмененнымСоставомРолейПриЗагрузке) <> Тип("Массив") Тогда
		ПрофилиСИзмененнымСоставомРолейПриЗагрузке = Новый Массив;
	КонецЕсли;
	
	Возврат ПрофилиСИзмененнымСоставомРолейПриЗагрузке;
