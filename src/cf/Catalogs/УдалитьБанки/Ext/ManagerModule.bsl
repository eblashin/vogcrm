...
Процедура ДобавитьБанкиИзКлассификатора(Код, КоррСчет, ТаблицаБанков)
	...
	Если МассивБанковИзКлассификатора.Количество() > 0 Тогда
		...
		МассивБанков = УдалитьРаботаСБанкамиПереопределяемый.ПодобратьБанкИзКлассификатора(МассивБанковИзКлассификатора);
	...
	Для каждого НайденныйБанк Из МассивБанков Цикл
		...
		ИначеЕсли ИскатьПоКоду 
			И Найти(НайденныйБанк.Код, Код) > 0 Тогда
			
			БанкНайден = Истина;
			
		ИначеЕсли ИскатьПоКоррСчету 
			И Найти(НайденныйБанк.КоррСчет, КоррСчет) Тогда
...
Процедура ОбновитьБанкиИзКлассификатора(СтруктураПараметров, АдресХранилища) Экспорт
	...
	УспешноОбновлены = УдалитьРаботаСБанкамиПереопределяемый.ОбновитьБанкиИзКлассификатора(,
...
Функция ПолучитьИсточникДанных()
	...
	Запрос = Новый Запрос("ВЫБРАТЬ * ИЗ Справочник.УдалитьБанки");
