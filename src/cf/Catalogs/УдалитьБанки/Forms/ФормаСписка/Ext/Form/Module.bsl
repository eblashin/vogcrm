...
Функция ВыполнитьНаСервере(ИБФайловая)
	...
	Если ИБФайловая Тогда
		...
		Справочники.УдалитьБанки.ОбновитьБанкиИзКлассификатора(СтруктураПараметров, АдресХранилища);
		Результат = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		НаименованиеФоновогоЗадания = НСтр("ru = 'Обновление банков из классификатора'");
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор, 
			"Справочники.УдалитьБанки.ОбновитьБанкиИзКлассификатора", 
			СтруктураПараметров, 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 * ИЗ Справочник.УдалитьБанки КАК Банки ГДЕ Банки.РучноеИзменение <> 2");
...
Процедура ОпределитьНеобходимостьПодбораБанкаИзКлассификатора(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ДополнительныеПараметры.ЭтоГруппа Тогда
			...
			ОткрытьФорму("Справочник.УдалитьБанки.ФормаГруппы", Новый Структура("ЭтоГруппа",Истина), ЭтотОбъект);
			
		Иначе
			
			ОткрытьФорму("Справочник.УдалитьБанки.ФормаОбъекта");
