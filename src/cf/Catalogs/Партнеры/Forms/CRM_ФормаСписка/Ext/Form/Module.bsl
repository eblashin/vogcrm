...
Процедура УстановитьПунктМеню()
	...
	Если Элементы.МенюHTML.Документ <> Неопределено Тогда
		Если Элементы.МенюHTML.Документ.readyState = "complete" Тогда
			ПуктМеню = "";
			Если ТипОтношений = ПредопределенноеЗначение("Справочник.CRM_ТипыОтношений.Поставщик") Тогда
				ПуктМеню = "поставщики";
			ИначеЕсли ТипОтношений = ПредопределенноеЗначение("Справочник.CRM_ТипыОтношений.КлиентДистрибьютора") Тогда
				ПуктМеню = "клиент дистрибьют.";
			КонецЕсли;
			вогНачальнаяСтраницаПользователяКлиетСервер.НачальнаяСтраницаПользователяПриОткрытии(ЭтаФорма,ПуктМеню);
			//ОтключитьОбработчикОжидания("УстановитьПунктМеню");
		КонецЕсли;
	КонецЕсли;
...
Процедура Подключаемый_МенюHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ДанныеСобытия.Element <> Неопределено Тогда
		...
		ИначеЕсли ТипОтношений = ПредопределенноеЗначение("Справочник.CRM_ТипыОтношений.КлиентДистрибьютора") Тогда
			ПуктМеню = "клиент дистрибьют.";
		КонецЕсли;
		вогНачальнаяСтраницаПользователяКлиетСервер.МенюHTMLПриНажатии(ЭтаФорма,ПуктМеню);
	КонецЕсли;
...
Процедура ОбработатьАктивизациюСтрокиСписка()
	...
	// ++ Харченко Д.И. №  - 19.09.2018 / 
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Контакты, "Владелец", Элементы.Список.ТекущаяСтрока); 
	...
	Если Интересы.Отбор.Элементы.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Интересы, "Партнер", Элементы.Список.ТекущаяСтрока); 
		//ИнтересыОтборПоКлиенту					= Интересы.Отбор.Элементы[0];
		//ИнтересыОтборПоКлиенту.ПравоеЗначение	= Элементы.Список.ТекущаяСтрока;
		// +++ VOG Кулаков П.Л. 05.02.2021 DEV-79
		Попытка
	...
	Если Элементы.Список.ТекущиеДанные <> Неопределено И Элементы.Найти("ПринятьОбращениеПоКлиенту") <> Неопределено Тогда
		Партнер = Элементы.Список.ТекущиеДанные.Ссылка;
		Если ЭтоГруппаПартнеров(Партнер) Тогда
			Элементы.ПринятьОбращениеПоКлиенту.Доступность = Ложь;
		Иначе	
			Элементы.ПринятьОбращениеПоКлиенту.Доступность = Истина;
		КонецЕсли;	
	КонецЕсли;	
...
Процедура УстановитьОтборПоСостоянию()
	...
	Если Интересы.Отбор.Элементы.Количество() > 0 Тогда
		// ++ Харченко Д.И. №  - 19.09.2018 / 
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Интересы, "Завершен", ТолькоАктивные); 
...
//&НаКлиенте
//// Процедура определяет доступность контактной информации.
////
//// Параметры:
////	Клиент	- СправочникСсылка	- Клиент
////	Контакт	- СправочникСсылка	- Контакт
////
//Функция УстановитьДоступностьКонтакта(Клиент = Неопределено, Контакт = Неопределено)
//	Если ЗначениеЗаполнено(Клиент) Тогда
//		ЕстьДоступУровняАккаунтМенеджера	= CRM_УправлениеДоступомВызовСервера.ЕстьДоступУровняАккаунтМенеджера(Клиент);
//	Иначе
//		ЕстьДоступУровняАккаунтМенеджера	= Ложь;
//	КонецЕсли;	
//	Если ЕстьДоступУровняАккаунтМенеджера Тогда
//		ЕстьДоступККонтакту	= Истина;
//	ИначеЕсли ЗначениеЗаполнено(Клиент) Тогда	
//		ЕстьДоступККонтакту	= CRM_УправлениеДоступомВызовСервера.ЕстьПраваДоступаККонтакту(Клиент, Контакт);
//	Иначе
//		ЕстьДоступККонтакту	= Ложь;
//	КонецЕсли;	
//	Возврат ЕстьДоступККонтакту;
//КонецФункции // УстановитьДоступностьКонтакта()
