...
//+++ Терпогосян Д.Б. [10.08.2021 17:16:29] № DEV-743
//Добавляет всех остальных пользователей в ограниченную группу кроме пользователей группы Полные Права
&НаСервере
Процедура ЗаполнитьНаСервере()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НЕ Пользователи.Ссылка В (&мПолныеПрава)";
	...
	Запрос.УстановитьПараметр("мПолныеПрава", мПолныеПрава);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		нСтр.Пользователь = ВыборкаДетальныеЗаписи.Ссылка; 
	КонецЦикла;
