...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры = Неопределено) Экспорт
	...
	ВидКонтактнойИнформацииСсылка = Запрос.Выполнить().Выбрать();
	// До изменения
	...
	Пока ВидКонтактнойИнформацииСсылка.Следующий() Цикл
		Попытка
			...
			// ++ VOG Солодов В.В. 02.03.2020 CRM-196
			Если Не ВидКонтактнойИнформации.ЭтоГруппа И Не ЗначениеЗаполнено(ВидКонтактнойИнформации.ВидРедактирования) Тогда
			// До изменения
			//Если Не ВидКонтактнойИнформации.ЭтоГруппа Тогда
			// -- VOG Солодов В.В. 02.03.2020
				Если ВидКонтактнойИнформации.РедактированиеТолькоВДиалоге Тогда
