...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Работа с печатными формами не поддерживается.'"));
		Возврат;
	КонецЕсли;
...
Функция ЗапросыОбновленияРазрешений()
	...
	Возврат ДополнительныеОтчетыИОбработкиВБезопасномРежимеСлужебный.ЗапросыРазрешенийДополнительнойОбработки(
		Объект, РазрешенияПрофиляБезопасности());
...
Процедура ОбновлениеИзФайлаМеханикаНаКлиенте(ПараметрыРегистрации)
	...
	Иначе
		...
		Иначе
			...
			ПараметрыПредупреждения.Вставить("Текст", ПараметрыРегистрации.ТекстОшибки);
			ПараметрыПредупреждения.Вставить("Подробно", ПараметрыРегистрации.КраткоеПредставлениеОшибки);
			ПараметрыПредупреждения.Вставить("Заголовок", ЗаголовокОшибки);
			СтандартныеПодсистемыКлиент.ВывестиПредупреждение(ЭтотОбъект, ПараметрыПредупреждения, ОбработчикРезультата);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеОкончанияРедактированияРасписания(Расписание, Контекст) Экспорт
	...
	Иначе
		...
		Если ДополнительныеОтчетыИОбработкиКлиентСервер.РасписаниеЗадано(Расписание) Тогда
			...
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтандартныеПодсистемыКлиентСервер.ПредставлениеРасписания(Расписание);
		Иначе
...
Процедура ПослеЗавершенияВыполненияСервернойКомандыВФоне(Задание, ВыполняемаяКоманда) Экспорт
	Если Задание.Статус = "Ошибка" Тогда
		...
		Предупреждение.Вставить("Текст",    ВводныйТекст + Символы.ПС + Задание.КраткоеПредставлениеОшибки);
		Предупреждение.Вставить("Подробно", Задание.ПодробноеПредставлениеОшибки);
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если ЭтоГлобальнаяОбработка Тогда
		Для Каждого СтрокаРаздел Из Объект.Разделы Цикл
			...
			Иначе
				ПредставлениеНазначения = ПредставлениеНазначения + ", " + ПредставлениеРаздела;
			КонецЕсли;
		КонецЦикла;
	Иначе
		Для Каждого СтрокаНазначение Из Объект.Назначение Цикл
			ПредставлениеОМ = ДополнительныеОтчетыИОбработки.ПредставлениеОбъектаМетаданных(СтрокаНазначение.ОбъектНазначения);
			Если ПредставлениеНазначения = "" Тогда
				ПредставлениеНазначения = ПредставлениеОМ;
			Иначе
				ПредставлениеНазначения = ПредставлениеНазначения + ", " + ПредставлениеОМ;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	Если ПредставлениеНазначения = "" Тогда
...
Процедура СформироватьПредставленияРазрешений(Знач ВидРазрешения)
	...
	ТаблицаРазрешений = РазрешенияПрофиляБезопасности();
		...
		Если Объект.Вид = ВидДополнительнаяОбработка
			...
			Если ЗначениеЗаполнено(РегламентноеЗаданиеGUID) Тогда
				...
				Если РегламентноеЗадание <> Неопределено Тогда
					...
					ЭлементКоманда.РегламентноеЗаданиеПредставление = СтандартныеПодсистемыКлиентСервер.ПредставлениеРасписания(РегламентноеЗадание.Расписание);
