...
Процедура ПриЗаписиГлобальнойОбработки(Отказ)
	...
	ПубликацияВключена = (Публикация <> Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.Отключена);
	ТеснаяИнтеграцияСПодсистемойВМоделиСервиса = ДополнительныеОтчетыИОбработки.ИспользуетсяТеснаяИнтеграцияСПодсистемойВМоделиСервиса();
	...
	Для Каждого КлючИЗначение Из ДополнительныеСвойства.ЗаданияДляОбновления Цикл
		...
		Если ТеснаяИнтеграцияСПодсистемойВМоделиСервиса Тогда
			ИнтеграцияСТехнологиейСервиса.ПередОбновлениемЗадания(ЭтотОбъект, Команда, Задание, Изменения);
		КонецЕсли;
