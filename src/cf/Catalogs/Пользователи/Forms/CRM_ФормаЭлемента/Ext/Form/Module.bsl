...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() И Параметры.Свойство("ПараметрыНового") Тогда
		...
		СтрокаКонтакт = Объект.КонтактнаяИнформация.Добавить();
		СтрокаКонтакт.АдресЭП = СокрЛП(Параметры.ПараметрыНового.АдресПочты);
		СтрокаКонтакт.Вид = Справочники.ВидыКонтактнойИнформации.EmailПользователя;
		СтрокаКонтакт.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
		СтрокаКонтакт.Представление = СокрЛП(Параметры.ПараметрыНового.АдресПочты);
		Поз = Найти(Параметры.ПараметрыНового.АдресПочты, "@");
		Если Поз <> 0 Тогда
			СтрокаКонтакт.ДоменноеИмяСервера = Сред(Параметры.ПараметрыНового.АдресПочты, Поз+1);	
		КонецЕсли;
	КонецЕсли;
	// -CRM
	...
	// +CRM
	// Не используется командная панель формы, перенесем кнопку редактирования состава свойств.
	Если НЕ (Элементы.Найти("РедактироватьСоставСвойств") = Неопределено) Тогда
		Элементы.Переместить(Элементы["РедактироватьСоставСвойств"], Элементы.КоманднаяПанель); 
	КонецЕсли;
	// -CRM
	...
	// + Тищенко В.В.
	// Добавлен список Роли исполнителя
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(РолиИсполнителя,"Исполнитель",Объект.Ссылка);
	// - Тищенко В.В.
	
	// ++ VOG Солодов В.В. 02.08.2019 task 463
	Если ЗначениеЗаполнено(Объект.вогРуководитель) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "вогРуководитель", "Видимость", Истина);
		ЯвляетсяАссистентом 				= Истина;
	КонецЕсли;
...
//START Кайдашов 23/08/19 590
	Если РольДоступна("ПолныеПрава") тогда
		Элементы.ГруппаПраваДляПоручений.ТолькоПросмотр = Ложь;	
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.CRM_УчаствуетВАнкетировании Тогда
		Попытка
			Элементы.ФормаСправочникВнешниеПользователиВнешнийДоступ.Картинка		= БиблиотекаКартинок.Пользователь;
			Элементы.ФормаСправочникВнешниеПользователиВнешнийДоступ.Отображение	= ОтображениеКнопки.КартинкаИТекст;
		Исключение
		КонецПопытки;	
	КонецЕсли;
	// -CRM
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Добавлен список Роли исполнителя
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(РолиИсполнителя,"Исполнитель",Объект.Ссылка);
	Элементы.РолиИсполнителя.Обновить();
	// - Тищенко В.В.
...
// ++ VOG Солодов В.В. 02.08.2019 task 463
&НаКлиенте
Процедура ЯвляетсяАссистентомПриИзменении(Элемент)
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "вогРуководитель", "Видимость", ЯвляетсяАссистентом);
	
	Если Не ЯвляетсяАссистентом Тогда
		Объект.вогРуководитель = ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка");
	КонецЕсли;
...
// ++ Тищенко В.В.
&НаКлиенте
Процедура вогЗапуститьПодПользователем(Команда)
	...
	// +++ VOG Кулаков П.Л. 25.03.2020
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ТекущийТипПлатформы = СистемнаяИнформация.ТипПлатформы;
	Расположение1С = "C:\Program Files (x86)";
	Если ТекущийТипПлатформы = ТипПлатформы.Windows_x86_64 Тогда
		Расположение1С = "C:\Program Files";
	КонецЕсли;
	Если Найти(СтрокаСоединенияИнформационнойБазы(),"Srvr=") = 0 Тогда
		СтрокаЗапуска = Расположение1С + "\1cv8\common\1cestart.exe ENTERPRISE /RunModeManagedApplication"+СтрЗаменить(СтрЗаменить(СтрокаСоединенияИнформационнойБазы(),"File="," /F "),";","")+" /n "+"""" + ТранзитныеПараметры.ИмяПользователя+""""+" /p "+"123"+"";
	Иначе
		СтрокаЗапуска = Расположение1С + "\1cv8\common\1cestart.exe ENTERPRISE /RunModeManagedApplication"+СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрокаСоединенияИнформационнойБазы(),"Srvr="," /S "),";Ref=""","\"),"""",""),";","")+" /n "+""""+ТранзитныеПараметры.ИмяПользователя+""""+" /p "+"123"+"";
	КонецЕсли;	
	// --- VOG Кулаков П.Л.
...
Процедура ОбщаяНастройкаФормы(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Если CRM_ОбщегоНазначенияПовтИсп.ЭтоCRM() Тогда
		Элементы.CRM_УровеньДоступа.ТолькоПросмотр	 = Элементы.Наименование.ТолькоПросмотр;
	КонецЕсли;
	Элементы.CRM_Планируемый.ТолькоПросмотр 	 = Элементы.Наименование.ТолькоПросмотр;
	// -CRM
...
Функция ПодсказкаПотребоватьСменуПароляПриВходе()
	...
	Если ЭтоПолноправныйПользователь
		И Метаданные.Подсистемы.Найти("Администрирование") <> Неопределено Тогда
		Подсказка.Добавить(" ");
...
#Область ЗапускПодПользователем
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРолиИсполнителя

// + Тищенко В.В.

&НаКлиенте
Процедура РолиИсполнителяПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	Отказ 			= Истина;
	ПараметрыФормы 	= Новый Структура("Исполнитель",Объект.Ссылка);
	ОткрытьФорму("РегистрСведений.ИсполнителиЗадач.ФормаЗаписи",ПараметрыФормы,Элемент,УникальныйИдентификатор);
...
Процедура Подключаемый_КонтактнаяИнформацияПриИзмененииАдресаЭлектроннойПочты(Элемент)
	...
	Если РезультатПроверки.КодОшибки <> 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(РезультатПроверки.Сообщение, , Элемент.Имя);
	КонецЕсли;
КонецПроцедуры // Подключаемый_КонтактнаяИнформацияПриИзмененииАдресаЭлектроннойПочты()
...
Процедура Подключаемый_ОтправитьСМСНаНомер(Команда)
	...
	ИначеЕсли CRM_ОбщегоНазначенияКлиент.смсПроверитьДоступностьSMS4B() Тогда
		Реквизит = СтрЗаменить(ТекущийЭлемент.Имя,"Команда","");
		Реквизит = СтрЗаменить(Реквизит,"ОтправитьСМСНаНомер","");
		Телефон = ЭтотОбъект[Реквизит];
...
&НаКлиенте
// Процедура - динамически подключаемый обработчик команды "ПозвонитьНаНомер".
//
Процедура Подключаемый_ПозвонитьНаНомер(Элемент)
	...
	// -СофтФон
КонецПроцедуры // Подключаемый_ПозвонитьНаНомер()
...
Процедура CRM_ОткрытьПерсональныеНастройки(Команда)
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Перед открытием формы настроек необходимо записать данные!'"));
	Иначе
