...
&НаСервере
Процедура ЗаполнитьДеревоПодразделенийФилиала(ДеревоПодразделений)
	...
	вогУправлениеДоступом.ЗаполнитьСтрокиДереваПодразделенийРекурсивно(
		ДеревоПодразделений.Строки,
		ОбособленноеПодразделение,
		6);
	
КонецПроцедуры // -- VOG Солодов В.В. 16.11.2020 CRM-992

// ++ VOG Солодов В.В. 18.02.2021 TRG-277
&НаСервере
Процедура ЗаполнитьДеревоПодразделенийПоСписку(ДеревоПодразделений, СписокПодразделений)
	
	Если Не СписокПодразделений.Найти(Справочники.СтруктураПредприятия.ОсновноеПодразделение) = Неопределено Тогда
		
		вогУправлениеДоступом.ЗаполнитьСтрокиДереваПодразделенийРекурсивно(
			ДеревоПодразделений.Строки,
			Справочники.СтруктураПредприятия.ОсновноеПодразделение,
			6);
		
	Иначе
		
		Для Каждого ОбособленноеПодразделение Из СписокПодразделений Цикл
...
//START Кайдашов 27/08/19 595
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			...
			"Недействителен",
			Ложь);
		НовыйЭлементОтбора = ПользователиСписок.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));	
		НовыйЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПометкаУдаления");
		НовыйЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		НовыйЭлементОтбора.ПравоеЗначение = Ложь;
		НовыйЭлементОтбора.Использование = Истина;
		НовыйЭлементОтбора = ПользователиСписок.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));	
		НовыйЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("CRM_ДолжностнаяПозиция");
...
//END Кайдашов
	...
	//+вог
	Элементы.СтраницыПодразделений.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;
//START Кайдашов 22/08/19 595	
	Если Параметры.ВсеПользователи или (Параметры.Свойство("РежимАдминистратора")
	  И Параметры.РежимАдминистратора) Тогда
		Элементы.СтраницыПодразделений.ТекущаяСтраница = Элементы.СтраницаСписок;
//END Кайдашов
	Иначе
		Элементы.СтраницыПодразделений.ТекущаяСтраница = Элементы.СтраницаДерево;
	КонецЕсли;
	...
	Если Параметры.Свойство("РуководительПользователя") И ЗначениеЗаполнено(Параметры.РуководительПользователя) Тогда
		вогУправлениеДоступом.ЗаполнитьДеревоПодразделенийПоДаннымПользователя(
			ЭтотОбъект,
			ПодразделенияДеревоСервер,
			Параметры.РуководительПользователя);
			
	ИначеЕсли Параметры.Свойство("ПодразделенияФилиала") // ++ VOG Солодов В.В. 16.11.2020 CRM-992
		И Параметры.ПодразделенияФилиала = Истина Тогда
		ЗаполнитьДеревоПодразделенийФилиала(ПодразделенияДеревоСервер);
	// -- VOG Солодов В.В. 16.11.2020 CRM-992
	// ++ VOG Солодов В.В. 18.02.2021 TRG-277
	ИначеЕсли Параметры.Свойство("СписокПодразделений")
		И Параметры.СписокПодразделений.Количество() > 0 Тогда
		ЗаполнитьДеревоПодразделенийПоСписку(ПодразделенияДеревоСервер, Параметры.СписокПодразделений);
	// -- VOG Солодов В.В. 18.02.2021 TRG-277
	Иначе
		вогУправлениеДоступом.ЗаполнитьДеревоПодразделенийПоДаннымПользователя(ЭтотОбъект, ПодразделенияДеревоСервер);
	КонецЕсли;
	// До изменения
	//вогУправлениеДоступом.ЗаполнитьДеревоПодразделенийПоДаннымПользователя(ЭтотОбъект, ПодразделенияДеревоСервер);
...
Процедура ОбновитьСодержимоеФормыПриИзмененииГруппы()
	...
	Иначе
		...
		ОбновитьЗначениеПараметраКомпоновкиДанных(ПользователиСписок, "ГруппаПользователей", ГруппаПользователейВсеПользователи);
		//+вог       
		Если Элементы.СтраницыПодразделений.ТекущаяСтраница = Элементы.СтраницаДерево Тогда
			ТекущиеДанные = Элементы.ПодразделенияДерево.ТекущиеДанные;
			ОбновитьЗначениеПараметраКомпоновкиДанных(ПользователиСписок, "Подразделение", ?(ТекущиеДанные = Неопределено, Неопределено, ТекущиеДанные.Ссылка));
		Иначе	
...
Процедура ВыбратьПользователя(Команда)
	...
	ОповеститьОВыборе(Элементы.ПользователиСписок.ТекущиеДанные.Ссылка)	
	// -- Тищенко В.В.
КонецПроцедуры
