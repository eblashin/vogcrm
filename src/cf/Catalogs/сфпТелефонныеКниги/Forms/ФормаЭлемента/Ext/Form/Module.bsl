...
Процедура ЗаполнитьТаблицуВидовТелефонов()
	...
	НоваяСтрока.Вид				= ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.CRM_ТелефонПользователяРабочий");
	НоваяСтрока.Тип				= 0;
	...
	НоваяСтрока.Вид				= ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.CRM_МобильныйТелефонПользователя");
	НоваяСтрока.Тип				= 0;
	// ТелефонПользователя
	НоваяСтрока = Объект.ВидыТелефонов.Добавить();
	НоваяСтрока.Пометка			= Истина;
...
Процедура ПроверитьЗаполнениеТаблицыВидовТелефонов()
	Для Каждого СтрокаВида Из Объект.ВидыТелефонов Цикл
		...
		ИначеЕсли НЕ СсылкаСуществует(СтрокаВида.Вид) Тогда
...
Процедура ЗаполнитьДеревоПользователей()
	...
	Для Каждого СтрокаВида Из Объект.ВидыТелефонов Цикл
		...
		Если СтрокаВида.Пометка И ЗначениеЗаполнено(СтрокаВида.Вид) Тогда
	...
	ИначеЕсли ВариантОтображенияПользователей = 1 Тогда
		...
		Для Каждого ЭлементМассиваГрупп Из МассивВсехГрупп Цикл
			...
			КонецЕсли;	
			Если ЭлементМассиваГрупп.Группа = ПредопределенноеЗначение("Справочник.ГруппыПользователей.ВсеПользователи") Тогда
		...
		НайденныйЭлемент = СписокГрупп.НайтиПоЗначению(ПредопределенноеЗначение("Справочник.ГруппыПользователей.ВсеПользователи"));
		...
		КонецЕсли;	
		//Элементы.Пользователи.НачальноеОтображениеДерева = НачальноеОтображениеДерева.РаскрыватьВсеУровни;
...
Процедура ДобавитьСтрокуСтруктурыВТелефоннуюКнигу(ЭлементМассива, СтрокиДерева, Родитель)
	Если НЕ (ЭлементМассива.Объект = Неопределено) И НЕ (ЭлементМассива.ЭтоГруппа) Тогда
		Если НЕ СсылкаСуществует(ЭлементМассива.Объект) Тогда Возврат; КонецЕсли;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЭлементМассива.Объект) Тогда	
		Если ЭлементМассива.Картинка = 2 Тогда
			Если ТипЗнч(ЭлементМассива.Объект) = Тип("СправочникСсылка.Пользователи") Тогда
				...
				Если ПустаяСтрока(ТекущийВнутреннийНомер) Тогда Возврат; КонецЕсли;	
				СтрокаГруппы = СтрокиДерева.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаГруппы, ЭлементМассива);
				СтрокаГруппы.Наименование		= НСтр("ru='Внутренний номер: '") + ТекущийВнутреннийНомер;
			Иначе
...
Процедура ВидыТелефоновПометкаПриИзменении(Элемент)
	...
	//	ЗаполнитьДеревоПользователей();
	//Иначе
	Если ЗначениеЗаполнено(ТД.Вид) Тогда
		ЗаполнитьДеревоПользователей();
	Иначе
		ТД.Пометка = Ложь;	
	КонецЕсли;	
...
Процедура ПользователиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе
		...
		КонецЕсли;	
		ОписаниеПользователя	= Новый ОписаниеОповещения("ОбновитьОписаниеПользователя", ЭтотОбъект, ДополнительныеПараметры);
		ПараметрыФормы			= Новый Структура;
		ПараметрыФормы.Вставить("Ключ", СтрокаКонтакта.Объект);
		ОткрытьФорму("Справочник.Пользователи.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект, , , , ОписаниеПользователя);
	КонецЕсли;	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Константы.сфпИспользоватьРегламентноеЗаданиеТелефонныхКниг.Получить() = Истина Тогда
			ЗначениеОбъекта.УчаствуетВРегламентномЗадании = Истина;
		Иначе	
			ЗначениеОбъекта.УчаствуетВРегламентномЗадании = Ложь;
		КонецЕсли;	
