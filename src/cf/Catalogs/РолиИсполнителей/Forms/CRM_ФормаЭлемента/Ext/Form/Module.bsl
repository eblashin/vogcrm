...
&НаКлиенте
Процедура ИспользуетсяСОбъектамиАдресацииПриИзменении(Элемент)
	
	// + Тищенко В.В.
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,"вогИспользоватьНижеСтоящиеПодразделение","Доступность",Объект.ИспользуетсяСОбъектамиАдресации);
	// - Тищенко В.В.
	
	Если НЕ Объект.ИспользуетсяСОбъектамиАдресации Тогда
		Объект.вогИспользоватьНижеСтоящиеПодразделение = Ложь;
	КонецЕсли;
