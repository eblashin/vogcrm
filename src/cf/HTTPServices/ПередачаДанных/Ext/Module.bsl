...
Функция ПолучитьGETОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		Иначе
			...
			Буфер = Новый БуферДвоичныхДанных(Диапазон.Конец - Диапазон.Начало + 1);
			ПотокДанных.Перейти(Диапазон.Начало, ПозицияВПотоке.Начало);
			Прочитано = ПотокДанных.Прочитать(Буфер, 0, Буфер.Размер);
			Ответ.УстановитьТелоИзДвоичныхДанных(ПолучитьДвоичныеДанныеИзБуфераДвоичныхДанных(Буфер));
			
			Ответ.Заголовки.Вставить("Content-Range", СтрШаблон("bytes %1-%2/%3", Формат(Диапазон.Начало, "ЧН=0; ЧГ=0"), Формат(Диапазон.Начало + Прочитано - 1, "ЧН=0; ЧГ=0"), Формат(ОписаниеДанных.Размер, "ЧН=0; ЧГ=0")));
...
Функция ОтправитьPUTОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		Буфер = Новый БуферДвоичныхДанных(ПолученныйДиапазон.Конец - ПолученныйДиапазон.Начало + 1);
		...
		Иначе
			...
			Идентификатор = МенеджерХранилища.Загрузить(ОписаниеДанных);
			
			ДанныеОтвета = Новый Структура("id", Строка(Идентификатор));
...
Функция МенеджерЛогическогоХранилища(Знач Storage, Ответ)
	...
	МенеджерХранилища = ПередачаДанных.ВсеМенеджерыЛогическихХранилищ()[Storage];
...
Функция МенеджерФизическогоХранилища(Знач VolumeID, Ответ)
	...
	МенеджерХранилища = ПередачаДанных.ВсеМенеджерыФизическихХранилищ()[VolumeID];
