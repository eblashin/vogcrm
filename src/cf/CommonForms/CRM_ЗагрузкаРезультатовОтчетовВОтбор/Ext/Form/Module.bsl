...
Процедура НастроитьВидимостьИДоступностьФормы(Форма)
	Если Форма.Режим = "ВыборПоляОтбора" Тогда
		...
		Форма.Заголовок = НСтр("ru = 'Выбор поля отбора'");
	Иначе
		...
		Форма.Заголовок = НСтр("ru = 'Загрузка результата отчета в отбор'");
	КонецЕсли;
КонецПроцедуры
...
Функция ПроверитьНаКорректность()
	...
	Если Не ЗначениеЗаполнено(ПолеОтбора) Тогда
		СообщитьПользователю(НСтр("ru = 'Поле отбора должно быть заполнено!'"), "ПредставлениеПоляОтбора");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Типы.Количество() > 0 Тогда
		...
		Если ДоступноеПолеОтбора <> Неопределено Тогда
			...
			Если Не бСодержитТип Тогда
				СообщитьПользователю(НСтр("ru = 'Тип загружаемых значений не соответствует типу выбранного поля!'"), "ПредставлениеПоляОтбора");
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПредставлениеПоляОтбораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Форма = ПолучитьФорму("ОбщаяФорма.CRM_ЗагрузкаРезультатовОтчетовВОтбор", СтруктураПараметры, ЭтотОбъект, Новый УникальныйИдентификатор());
