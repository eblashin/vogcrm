...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ГрафикРаботыИсполнителя = Неопределено;
	...
	ПараметрыДлительности  = Новый Структура;
	ПараметрыДлительности .Вставить("Дни", 0);
	ПараметрыДлительности .Вставить("Часы", 0);
	ПараметрыДлительности .Вставить("Минуты", 0);
	ПараметрыДлительности .Вставить("ДатаОтсчета", Дата(1,1,1));
	
	ПараметрыДлительности.ДатаОтсчета = ДатаОтсчета;
	
	ПараметрыДлительности.Дни = 1;
	СрокИсполнения_1День = СрокИсполненияПоДлительности(ПараметрыДлительности);
	
	ПараметрыДлительности.Дни = 2;
	СрокИсполнения_2Дня = СрокИсполненияПоДлительности(ПараметрыДлительности);
	
	ПараметрыДлительности.Дни = 3;
	СрокИсполнения_3Дня = СрокИсполненияПоДлительности(ПараметрыДлительности);
	
	ПараметрыДлительности.Дни = 7;
	СрокИсполнения_Неделя = СрокИсполненияПоДлительности(ПараметрыДлительности);
	
	ПараметрыДлительности.Дни = 30;
	СрокИсполнения_Месяц = СрокИсполненияПоДлительности(ПараметрыДлительности);
	...
	Если Элементы.БыстрыйСрокСегодня.Видимость Тогда
		Элементы.БыстрыйСрок1День.Заголовок = НСтр("ru = 'Завтра'");
		
	ИначеЕсли НачалоДня(СрокИсполнения_1День) = НачалоТекущегоДня Тогда
		
		Элементы.БыстрыйСрок1День.Заголовок = НСтр("ru = 'Сегодня'");
		Элементы.БыстрыйСрок2Дня.Заголовок = НСтр("ru = 'Завтра'");
	КонецЕсли;
...
Процедура СрокДатаПриИзменении(Элемент)
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Невозможно установить срок по следующей причине:
			|%1'");
...
Процедура ЗаполнитьВаринатыРабочегоВремени(Форма)
	...
	// ++ VOG Ульянов И.В. 15.01.2020 CRM-139
