...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПользовательИБПолноправный Тогда
		ВывестиУровниДоступа();
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			СписокРолиИсполнителей, "Исполнитель", Параметры.Пользователь); 	
	Иначе
		// Обычному пользователю запрещено просматривать любые настройки чужого доступа.
		Элементы.УровниДоступаПользователей.Видимость  = Ложь;
		Элементы.РолиИсполнителей.Видимость  		   = Ложь;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	//+вог
	Если ВРег(ИмяСобытия) = ВРег("Запись_УровеньДоступа") Тогда
		ВывестиУровниДоступа();
...
&НаКлиенте
Процедура ВключитьИсключитьИзУровня(УровеньДоступа, ВключитьВУровеньДоступа) Экспорт
	
	Если ТипЗнч(УровеньДоступа) <> Тип("СправочникСсылка.CRM_УровниДоступа")
	  ИЛИ НЕ ЗначениеЗаполнено(УровеньДоступа) Тогда
	...
	ИзменитьСоставУровня(
		УровеньДоступа,
		ВключитьВУровеньДоступа,
		ОписаниеОшибки);
	
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		ПоказатьПредупреждение(, ОписаниеОшибки);
	Иначе
		ОповеститьОбИзменении(УровеньДоступа);
		Оповестить("Запись_УровеньДоступа", Новый Структура, УровеньДоступа);
	КонецЕсли;
...
&НаСервере
Процедура ВывестиУровниДоступа()
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	CRM_УровниДоступаСостав.Ссылка.Наименование КАК Наименование,
	|	CRM_УровниДоступаСостав.Ссылка КАК УровеньДоступа
	|ИЗ
	...
	ЗначениеВРеквизитФормы(РезультатЗапроса.Выгрузить(), "УровниДоступа");
	
КонецПроцедуры // ВывестиУровниДоступа()

&НаСервере
Процедура ИзменитьСоставУровня(Знач УровеньДоступа, Знач Добавить, ОписаниеОшибки = "")
				
	УстановитьПривилегированныйРежим(Истина);
	
	УровеньДоступаОбъект = УровеньДоступа.ПолучитьОбъект();
	ЗаблокироватьДанныеДляРедактирования(УровеньДоступаОбъект.Ссылка, УровеньДоступаОбъект.ВерсияДанных);
	Если Добавить Тогда
		Если УровеньДоступаОбъект.Состав.Найти(Параметры.Пользователь, "Пользователь") = Неопределено Тогда
			УровеньДоступаОбъект.Состав.Добавить().Пользователь = Параметры.Пользователь;
		КонецЕсли;
	Иначе
		СтрокаТЧ = УровеньДоступаОбъект.Состав.Найти(Параметры.Пользователь, "Пользователь");
		Если СтрокаТЧ <> Неопределено Тогда
			УровеньДоступаОбъект.Состав.Удалить(СтрокаТЧ);
		КонецЕсли;
	КонецЕсли;
	
	УровеньДоступаОбъект.ДополнительныеСвойства.Вставить("НеОчищатьОбъектыУровня");	
	...
	РазблокироватьДанныеДляРедактирования(УровеньДоступаОбъект.Ссылка);
	
КонецПроцедуры

&НаКлиенте
Процедура ВключитьВУровень(Команда)
		
	ОткрытьФорму("Справочник.CRM_УровниДоступа.ФормаВыбора",, ЭтотОбъект,
		,,, Новый ОписаниеОповещения("ВключитьИсключитьИзУровня", ЭтотОбъект, Истина), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура ИсключитьИзУровня(Команда)
		
	ТекущиеДанные = Элементы.УровниДоступа.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Уровень доступа не выбран.'"));
		Возврат;
		
	КонецЕсли;
		
	ВключитьИсключитьИзУровня(ТекущиеДанные.УровеньДоступа, Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьУровень(Команда)
	...
	ИначеЕсли ПользовательИБПолноправный Тогда
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Ключ", ТекущиеДанные.УровеньДоступа);
		ОткрытьФорму("Справочник.CRM_УровниДоступа.ФормаОбъекта", ПараметрыФормы);
	Иначе
		ПоказатьПредупреждение(,
			НСтр("ru = 'Недостаточно прав для редактирования уровня доступа.
			           |Редактировать уровень доступа может только администратор.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтчетПоУровнямДоступа(Команда)
	...
	Отбор.Вставить("Пользователь", Параметры.Пользователь);
