...
Функция ПараметрыОтправкиПисьма(ВыбранныеПараметры)                    	
	...
	КонецЕсли;
	Если ЭтаФорма.Параметры.ИмяМенеджераПечати = "Справочник.ДоговорыКонтрагентов" Тогда
		Получатели = СформироватьСписокАдресов(Новый Структура("Партнер, КонтактноеЛицо", ПредопределенноеЗначение("Справочник.Партнеры.ПустаяСсылка"), вогОбщегоНазначенияВызовСервера.ЗначениеРеквизитаОбъекта(ЭтаФорма.Параметры.ПараметрыПечати.ОбъектыПечати[0], "вогПодписантПартнера"))); //+ VOG Иванов С.А. 01.11.2019 ID заявки: 000000776
	КонецЕсли;	
...
Процедура НастроитьВидимостьЭлементовФормы(Знач ЕстьРазрешенныйВывод)
	...
	//++ VOG Иванов С.А. 01.11.2019 ID заявки: 000000776
	// ++ VOG Солодов В.В. 12.05.2020 CRM-631
	ТолькоПечать = Параметры.ПараметрыПечати.Свойство("ДополнительныеПараметры")
		И ТипЗнч(Параметры.ПараметрыПечати.ДополнительныеПараметры) = Тип("Структура")
		И Параметры.ПараметрыПечати.ДополнительныеПараметры.Свойство("ТолькоПечать")
		И Параметры.ПараметрыПечати.ДополнительныеПараметры.ТолькоПечать = Истина;
		
	ТолькоОтправка = Параметры.ПараметрыПечати.Свойство("ДополнительныеПараметры")
		И ТипЗнч(Параметры.ПараметрыПечати.ДополнительныеПараметры) = Тип("Структура")
		И Параметры.ПараметрыПечати.ДополнительныеПараметры.Свойство("ТолькоОтправка")
		И Параметры.ПараметрыПечати.ДополнительныеПараметры.ТолькоОтправка = Истина;
	// До изменения
	//ТолькоПечать = НастройкиПечатныхФорм[0].ИмяМакета = "ДоговорПоставки" 
	//							ИЛИ НастройкиПечатныхФорм[0].ИмяМакета = "КоммерческоеСоглашение";
	//ТолькоОтправка = НастройкиПечатныхФорм[0].ИмяМакета = "ДоговорПоставкиСПодписями" 
	//							ИЛИ  НастройкиПечатныхФорм[0].ИмяМакета = "КоммерческоеСоглашениеСПодписямиПлитка" 
	//							ИЛИ  НастройкиПечатныхФорм[0].ИмяМакета = "КоммерческоеСоглашениеСПодписямиОбои"
	//							// ++ VOG Солодов В.В. 07.05.2020 CRM-630
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение1СПодписямиПлитка"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение2СПодписямиПлитка"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение4СПодписямиПлитка"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение5СПодписямиПлитка"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение6СПодписямиПлитка"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение1СПодписямиОбои"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение2СПодписямиОбои"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение4СПодписямиОбои"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение5СПодписямиОбои"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "Приложение6СПодписямиОбои"
	//							Или НастройкиПечатныхФорм[0].ИмяМакета = "ДопСоглашениеНормаБояСПодписями";
								// -- VOG Солодов В.В. 07.05.2020
	// -- VOG Солодов В.В. 12.05.2020
	
	Если ТолькоОтправка ИЛИ ТолькоПечать Тогда
		Элементы.КнопкаОтправить.Видимость = ТолькоОтправка;
		Элементы.КнопкаСохранить.Видимость = Ложь; 
		Элементы.ВычислитьСумму.Видимость = ТолькоПечать;
		Элементы.КнопкаРедактирование.Видимость = ТолькоПечать;
		Элементы.КнопкаПечатьКоманднаяПанель.Видимость = ТолькоПечать;
		Элементы.КнопкаПредварительныйПросмотр.Видимость = ТолькоПечать;
		Элементы.Копий.Видимость = ТолькоПечать;
	КонецЕсли;
	//-- VOG Иванов С.А. 01.11.2019 ID заявки: 000000776
...
Процедура ОтправитьПечатныеФормыПоПочтеНастройкаУчетнойЗаписиПредложена(УчетнаяЗаписьНастроена, ДополнительныеПараметры) Экспорт
	...
	Если Параметры.ИмяМенеджераПечати = "Справочник.ДоговорыКонтрагентов" Тогда
		
		ФорматыСохранения = Новый Массив;
		ФорматыСохранения.Добавить(ТипФайлаТабличногоДокумента.PDF);
		
		ФорматВложения = Новый Структура;
		ФорматВложения.Вставить("ПереводитьИменаФайловВТранслит", 	Ложь);
		ФорматВложения.Вставить("УпаковатьВАрхив", 					Ложь);
		ФорматВложения.Вставить("ФорматыСохранения", 				ФорматыСохранения);
		
		ПараметрыОтправки = ПараметрыОтправкиПисьма(ФорматВложения);
		...
		Для Каждого СтрокаТаб Из МассивСтрок Цикл
			...
			Если НЕ ТекСсылка = СтрокаТаб.Ссылка Тогда
				...
				Если ПерваяСтрока = Неопределено Тогда						
					...
					СтруктураАдреса.Вставить("ИсточникКонтактнойИнформации"		,СтрокаТаб.Ссылка);
					СписокАдресов.Добавить(СтруктураАдреса);
					ТекСсылка = СтрокаТаб.Ссылка;						
				Иначе						
					СтруктураАдреса.Вставить("Адрес"		,СокрЛП(ПерваяСтрока.АдресЭП));
					СтруктураАдреса.Вставить("Представление",СокрЛП(ПерваяСтрока.Ссылка));
					СтруктураАдреса.Вставить("ИсточникКонтактнойИнформации"		,ПерваяСтрока.Ссылка);
					СписокАдресов.Добавить(СтруктураАдреса);
