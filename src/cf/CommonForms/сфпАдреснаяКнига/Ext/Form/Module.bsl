...
Процедура ЗаполнитьТаблицуТелефонов(Ссылка)
	...
	Для Каждого ЭлементМассива Из МассивТелефонов Цикл
		...
		НоваяСтрока.Ссылка 				= Ссылка;
		НоваяСтрока.ПоказыватьКолонку	= Истина;
	КонецЦикла;
...
Процедура ОбработчикКонтактыПриАктивизацииСтроки()
	...
	Если ТД = Неопределено Тогда
		Возврат;
	ИначеЕсли ТекущийКонтакт = ТД.Ссылка И НЕ ПоискЗапущен Тогда
		Возврат;
	КонецЕсли;
	ТекущийКонтакт = ТД.Ссылка;
	Если НЕ ПоискЗапущен Тогда
		ЗаполнитьТаблицуТелефонов(ТД.Ссылка);
	Иначе
		Для Каждого СтрокаТаблицы Из СписокТелефонов Цикл
			СтрокаТаблицы.ПоказыватьКолонку = Ложь;
		КонецЦикла;
		Отбор = Новый Структура();
		Отбор.Вставить("Ссылка", ТД.Ссылка);
		МассивСтрок = СписокТелефонов.НайтиСтроки(Отбор);
		Для каждого СтрокаМассива Из МассивСтрок Цикл
			СтрокаМассива.ПоказыватьКолонку 	= Истина;
			ПозвонитьПоНомеру 					= СтрокаМассива.Представление;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВыполнитьПоискНаСервере()
	...
	|	сфпНомераТелефоновДляПоиска.Объект.Наименование КАК Наименование,
	|	сфпНомераТелефоновДляПоиска.Вид КАК ВидКИ,
	...
	|ГДЕ
	|	сфпНомераТелефоновДляПоиска.Объект ССЫЛКА Справочник." + ТекущийСправочник + "
	| //ДополнитьТекст";
	
	Запрос.УстановитьПараметр("Представление", "%" + ПолеПоиска + "%");
	Если ПереключательВыборПоляПоиска = "ПоТелефону" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, 
		" //ДополнитьТекст", 
		" И (сфпНомераТелефоновДляПоиска.НомерТелефона ПОДОБНО &Представление
		|			ИЛИ сфпНомераТелефоновДляПоиска.Представление ПОДОБНО &Представление)");
	ИначеЕсли ПереключательВыборПоляПоиска = "ПоНаименованию" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, 
		"//ДополнитьТекст", 
		"И сфпНомераТелефоновДляПоиска.Объект.Наименование ПОДОБНО &Представление");
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не выбран режим поиска!'"),,"ПереключательВыборПоляПоиска");
		Возврат;
	КонецЕсли;
	Если ПереключательВыборПоляПоиска = "ПоТелефону" Тогда
		Выборка = Запрос.Выполнить().Выбрать();
		...
		Пока Выборка.Следующий() Цикл
			Если МассивДобавленных.Найти(Выборка.Объект) = Неопределено Тогда
				НовыйКонтакт = Контакты.Добавить();
				НовыйКонтакт.Ссылка = Выборка.Объект;
				НовыйКонтакт.Картинка = 2;
				Если ТекущийСправочник = "Пользователи" Тогда
					НовыйКонтакт.ВнутреннийНомер = сфпСофтФонПроСервер.сфпТекущийВнутреннийНомер(Выборка.Объект);
				КонецЕсли;
				МассивДобавленных.Добавить(Выборка.Объект);
			КонецЕсли;
			НовыйТелефон 						= СписокТелефонов.Добавить();
			НовыйТелефон.Вид 					= Выборка.ВидКИ;
			НовыйТелефон.Представление 			= Выборка.Представление;
			НовыйТелефон.Ссылка 				= Выборка.Объект;
			НовыйТелефон.ПоказыватьКолонку		= Ложь;
		КонецЦикла;
	ИначеЕсли ПереключательВыборПоляПоиска = "ПоНаименованию" Тогда
		 ТаблицаРезультата = Запрос.Выполнить().Выгрузить();
		 ТаблицаРезультата.Свернуть("Объект");
		 МассивРезультата = ТаблицаРезультата.ВыгрузитьКолонку("Объект");
		 Для Каждого СтрокаМассива Из МассивРезультата Цикл
			НовыйКонтакт = Контакты.Добавить();
			НовыйКонтакт.Ссылка 	= СтрокаМассива;
			НовыйКонтакт.Картинка 	= 2;
			Если ТекущийСправочник = "Пользователи" Тогда
				НовыйКонтакт.ВнутреннийНомер = сфпСофтФонПроСервер.сфпТекущийВнутреннийНомер(СтрокаМассива);
			КонецЕсли;
			МассивТелефонов = сфпСофтФонПроСервер.сфпПолучитьМассивТелефоновИФаксов(СтрокаМассива);
			Для Каждого ЭлементМассива Из МассивТелефонов Цикл
				НоваяСтрока						= СписокТелефонов.Добавить();
				НоваяСтрока.Вид 				= ЭлементМассива.Вид;
				НоваяСтрока.Представление		= ЭлементМассива.Представление;
				НоваяСтрока.Ссылка 				= СтрокаМассива;
				НоваяСтрока.ПоказыватьКолонку	= Ложь;
			КонецЦикла;
		 КонецЦикла;
	КонецЕсли;
	
	Элементы.ПоискОтмена.Доступность = Истина;
	ПоискЗапущен = Истина;
...
Процедура ТекущийСправочникОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ПоискЗапущен 						= Ложь;
	Элементы.ПоискОтмена.Доступность 	= Ложь;
...
&НаКлиенте
// Процедура - обработчики события "ОкончаниеВводаТекста" элемента формы "ПолеТелефон"
//
Процедура ПолеТелефонОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	ПолеПоиска = Элемент.ТекстРедактирования;
	Поиск(Неопределено);
КонецПроцедуры
...
&НаКлиенте
// Процедура - обработчик команды формы "Поиск"
//
Процедура Поиск(Команда)
	Если ПустаяСтрока(ТекущийСправочник) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не выбран источник поиска!'"),,"ТекущийСправочник");
		Возврат;
	КонецЕсли;
	Если ПустаяСтрока(ПолеПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не заполнено значение поиска!'"),,"ПолеПоиска");
		Возврат;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("ТекущийСправочник") Тогда
		ЭтотОбъект.ТекущийСправочник = Параметры.ТекущийСправочник;
	КонецЕсли;	
