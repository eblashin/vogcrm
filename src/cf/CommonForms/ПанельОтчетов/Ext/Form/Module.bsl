...
Процедура Подключаемый_ВариантНажатие(Элемент)
	...
	//++ VOG Ульянов И.В. CRM-1116
    ПользовательскиеНастройки = ПолучитьНастройкуПоПодразделению(Вариант.Отчет);
	
	Если ПользовательскиеНастройки <> Неопределено тогда
		ПараметрыФормыОтчета.Вставить("ПользовательскиеНастройки",ПользовательскиеНастройки);		
	КонецЕсли;	
	//-- VOG Ульянов И.В. CRM-1116
...
&НаСервере
Функция ПолучитьНастройкуПоПодразделению(Отчет)    //VOG Ульянов И.В. CRM-1116
	
	ОтчетОбъект = Отчеты[Отчет.Имя].Создать();
	КомпоновщикНастроек = ОтчетОбъект.КомпоновщикНастроек;
	
	Настройки=КомпоновщикНастроек.ПолучитьНастройки();
	
	ЕстьПроизвольныеНастройки = Ложь;
	
	Если Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных("НаправлениеДеятельности")) <> Неопределено тогда
		Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("НаправлениеДеятельности",вогОбщегоНазначенияВызовСервера.ПолучитьНаправлениеДеятельностиТекущегоПользователя());
		ЕстьПроизвольныеНастройки = Истина;
	КонецЕсли;	
	
	Если Настройки.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных("ПодразделениеКлассификаторов")) <> Неопределено тогда
		ПодразделениеПользователя = вогОбщегоНазначения.ПолучитьПодразделениеТекущегоПользователя();
		Если ПодразделениеПользователя <> Справочники.СтруктураПредприятия.ОсновноеПодразделение тогда
			Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ПодразделениеКлассификаторов",ПодразделениеПользователя);
			ЕстьПроизвольныеНастройки = Истина;
		КонецЕсли;	
	КонецЕсли;	
		
	Если ЕстьПроизвольныеНастройки = Истина тогда
