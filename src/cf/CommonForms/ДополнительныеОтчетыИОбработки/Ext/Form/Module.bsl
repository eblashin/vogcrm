...
Процедура ВыполнитьСерверныйМетодОбработкиЗавершение(Задание, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Обработчик = Новый ОписаниеОповещения("ПослеПоказаТекстаОшибкиФоновогоЗадания", ЭтотОбъект);
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Команда ""%1"" не выполнена:'"),
			ВыполняемаяКоманда.Представление);
		Предупреждение = Новый Структура;
		Предупреждение.Вставить("Текст",    Текст + Символы.ПС + Задание.КраткоеПредставлениеОшибки);
...
&НаКлиенте
Процедура ПослеПоказаТекстаОшибкиФоновогоЗадания(ПустойПараметр, ДополнительныеПараметры) Экспорт
	Если Открыта() Тогда
		Закрыть();
	КонецЕсли;
КонецПроцедуры
