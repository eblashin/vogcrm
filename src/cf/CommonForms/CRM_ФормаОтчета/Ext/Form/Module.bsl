...
Процедура ПриОткрытии(Отказ)
	...
	// +CRM
	// ++ VOG Солодов В.В. 30.04.2020 CRM-618
	// До изменения
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если Найти(СтруктураРасшифровки.ИмяРасшифровки, "НаКонтроле") > 0 Тогда
		Если НЕ ЗначениеЗаполнено(СтруктураРасшифровки.ЗначениеРасшифровки) Тогда
			СтандартнаяОбработка = Ложь;
			Возврат;
		КонецЕсли;
		ПараметрыФормы = Новый Структура("Интерес, БезОткрытияДокумента", СтруктураРасшифровки.ЗначениеРасшифровки, Истина);
		ОткрытьФорму("Документ.CRM_Интерес.Форма.ФормаКонтроля", ПараметрыФормы);
	Иначе
		// -CRM
...
#Область ДоработкиCRM

&НаСервере
Функция CRM_СформироватьРезультатДляСохраненияСервер()
...
&НаКлиенте
Процедура CRM_СохранитьРезультат(Команда)
	СтруктураПараметры = CRM_СформироватьРезультатДляСохраненияСервер();
...
&НаКлиенте
Процедура CRM_ЗагрузитьРезультатВОтбор(Команда)
	...
	КонецЕсли;
	ОписаниеОповещения = Новый ОписаниеОповещения("CRM_ЗагрузитьРезультатВОтборЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура CRM_ЗагрузитьРезультатВОтборЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура CRM_ОбновитьЗаголовокОтчета()
	// Заголовок.
	ОтчетНаименованиеТекущегоВарианта = СокрЛП(ОтчетНаименованиеТекущегоВарианта);
	Если ЗначениеЗаполнено(ОтчетНаименованиеТекущегоВарианта) Тогда
		Заголовок = ОтчетНаименованиеТекущегоВарианта;
	...
	ЕстьОтборы = Ложь;
	Для Каждого тОтбор Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		Если ТипЗнч(тОтбор) = Тип("ЭлементОтбораКомпоновкиДанных") И тОтбор.Использование = Истина Тогда
			Если НЕ ЕстьОтборы Тогда
				Заголовок = Заголовок + " /Отборы:";
			КонецЕсли;
			Заголовок = Заголовок + ?(НЕ ЕстьОтборы, " ", ", ") + тОтбор.ПравоеЗначение;
			ЕстьОтборы = Истина;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ОчиститьОтборОтветственныйРекурсия(ЭлементыОтбора)
	Для Каждого тОтбор Из ЭлементыОтбора Цикл
		Если ТипЗнч(тОтбор) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			ОчиститьОтборОтветственныйРекурсия(тОтбор.Элементы)
		Иначе	
			Если тОтбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ответственный") Тогда
				ЭлементыОтбора.Удалить(тОтбор);
				Прервать;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьРасшифровкуНаСервере(Расшифровка)
	...
	Если Поля.Количество() > 0 Тогда
		...
		СтруктураВозврата.Вставить("ИмяРасшифровки", Поля[0].Поле);
		СтруктураВозврата.Вставить("ЗначениеРасшифровки", Поля[0].Значение);
	Иначе
		СтруктураВозврата = Новый Структура;
		СтруктураВозврата.Вставить("ИмяРасшифровки", "");
		СтруктураВозврата.Вставить("ЗначениеРасшифровки", "");	
	КонецЕсли;	
   Возврат СтруктураВозврата;
КонецФункции

&НаСервере
Функция ПолучитьСписокПодразделенийПользователя()
	Возврат CRM_УправлениеДоступомКПользователям.ПолучитьСписокДоступныхПодразделений();
КонецФункции

&НаСервере
Функция ПолучитьСписокМенеджеровПользователя()
	Возврат CRM_УправлениеДоступомКПользователям.ПолучитьСписокДоступныхПользователей();
КонецФункции
