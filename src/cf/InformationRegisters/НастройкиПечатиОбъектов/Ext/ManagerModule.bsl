...
//
Функция КомплектПечатныхФорм(ТипОбъекта, Знач МассивСсылок = Неопределено, ВариантИспользования = 0, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ЕстьНаправлениеДеятельности = ТекущийОбъект.ЕстьНаправлениеДеятельности И МассивСсылок <> Неопределено;
	
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ОбъектыПечати.Ссылка КАК Ссылка"
		+?(ЕстьОрганизация,", ОбъектыПечати.Организация КАК Организация","")
		+ ?(ЕстьНаправлениеДеятельности,", ОбъектыПечати.НаправлениеДеятельности","") + "
	|ПОМЕСТИТЬ ВТ_Ссылки
	|ИЗ
	...
	|	И НастройкиПечатиОбъектов.НаправлениеДеятельности = ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)";
	...
	Если ЕстьОрганизация Тогда
		...
		|	NULL,
		|	НастройкиПечатиОбъектов.НаправлениеДеятельности
		|ИЗ
		|	РегистрСведений.НастройкиПечатиОбъектов КАК НастройкиПечатиОбъектов
	...
	Если ЕстьНаправлениеДеятельности Тогда
		...
		|	РегистрСведений.НастройкиПечатиОбъектов КАК НастройкиПечатиОбъектов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Ссылки КАК ВТ_Ссылки
		|		ПО (ВТ_Ссылки.НаправлениеДеятельности = НастройкиПечатиОбъектов.НаправлениеДеятельности)
		|ГДЕ
		|	НастройкиПечатиОбъектов.ТипОбъекта = &ТипОбъекта
		|	И НастройкиПечатиОбъектов.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)";
	КонецЕсли;
	
	Если ТекущийОбъект.ЕстьНаправлениеДеятельности И ТекущийОбъект.ЕстьОрганизация И МассивСсылок <> Неопределено Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
	...
	КомплектПоУмолчанию = МенеджерОбъекта.КомплектПечатныхФорм(ДополнительныеПараметры);
	...
	Для Каждого СтрокаТЧ Из КоллекцияПечатныхФорм Цикл
		...
		Если КомплектПоУмолчанию.Найти(СтрокаТЧ.Имя) = Неопределено Тогда
			КоллекцияПечатныхФорм.Удалить(СтрокаТЧ);
		КонецЕсли;
...
//		ЗаписыватьНаправлениеДеятельности - Булево - признак, указывающий на необходимость записи настройки в разрезе направления деятельности
//		Ссылка - ЛюбаяСсылка, ссылка из которой необходимо извлечь организацию и/или направление деятельности.
//
Процедура ЗаписатьНастройкиПечатиОбъекта(ТипОбъекта, КомплектПечатныхФорм, ЗаписыватьОрганизацию = Ложь, ЗаписыватьНаправлениеДеятельности = Ложь, Ссылка = Неопределено) Экспорт
	...
	Если ЗаписыватьОрганизацию И ЗаписыватьНаправлениеДеятельности Тогда
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "НаправлениеДеятельности,Организация");
		ЗаписьНастройкиПечатиОбъектов.Организация = Реквизиты.Организация;
		ЗаписьНастройкиПечатиОбъектов.НаправлениеДеятельности = Реквизиты.НаправлениеДеятельности;
	ИначеЕсли ЗаписыватьОрганизацию Тогда
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Организация");
		ЗаписьНастройкиПечатиОбъектов.Организация = Реквизиты.Организация;
	ИначеЕсли ЗаписыватьНаправлениеДеятельности Тогда
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "НаправлениеДеятельности");
		ЗаписьНастройкиПечатиОбъектов.НаправлениеДеятельности = Реквизиты.НаправлениеДеятельности;
	КонецЕсли;
...
//		НаправлениеДеятельности - СправочникСсылка.НаправленияДеятельности - направление деятельности, для которого необходимо удалить настройки.
//
Процедура УдалитьНастройкиПечатиОбъекта(ТипОбъекта, Организация, НаправлениеДеятельности) Экспорт
	
	ЗаписьНастройкиПечатиОбъектов = РегистрыСведений.НастройкиПечатиОбъектов.СоздатьМенеджерЗаписи();
	ЗаписьНастройкиПечатиОбъектов.ТипОбъекта  = ТипОбъекта;
	ЗаписьНастройкиПечатиОбъектов.Организация = Организация;
	ЗаписьНастройкиПечатиОбъектов.НаправлениеДеятельности = НаправлениеДеятельности;
...
//		* ЕстьНаправлениеДеятельности 	- Булево - признак наличия направления деятельности в объекте.
//
Функция ПараметрыОбъектаДляПечатиКомплектно(ТипОбъекта) Экспорт
	...
	ПараметрыОбъектаДляПечатиКомплектно.Вставить("ЕстьНаправлениеДеятельности", Ложь);
...
Функция ТаблицаОбъектовДляПечатиКомплектно() Экспорт
	...
	ТаблицаОбъектов.Колонки.Добавить("ЕстьНаправлениеДеятельности");

	// Договоры контрагентов
	НоваяСтрока = ТаблицаОбъектов.Добавить();
	НоваяСтрока.ТипОбъекта 					= Метаданные.Справочники.ДоговорыКонтрагентов.ПолноеИмя();
	НоваяСтрока.Представление 				= Метаданные.Справочники.ДоговорыКонтрагентов.Представление();
	НоваяСтрока.ЕстьОрганизация 			= Истина;
	НоваяСтрока.ЕстьНаправлениеДеятельности = Ложь;
...
Процедура СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	СоответствиеТипов = вогОбщегоНазначения.СоответствиеМассивовПоТипамОбъектов(МассивОбъектов);
