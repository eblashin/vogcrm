		...
		Для ТекСчетчикУникальныйИд = 1 По СчетчикУникальныйИд[ИмяТабличнойЧасти] Цикл
			...
			Пока ИндексПоВерсиям >= 1 Цикл
				...
				Если НайденнаяСтрока <> Неопределено Тогда
					...
					ОписаниеТабличнойЧасти = Неопределено;
...
Функция ВывестиРезультатыКомпоновкиВОтчет(Параметры)
...
Функция ВывестиШапку(ТЧОтчета, СписокВерсий, ЧислоВерсий, ОбщийШаблон, СсылкаНаОбъект)
