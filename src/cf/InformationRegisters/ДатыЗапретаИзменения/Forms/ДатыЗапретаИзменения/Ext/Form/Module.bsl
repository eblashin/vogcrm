...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	КонецЕсли;
	//+++ Терпогосян Д.Б. [09.08.2021 17:40:41] № DEV-743
		ТекстВопроса = ?(ТекстВопроса = Неопределено, "", ТекстВопроса) + " Все существующие настройки будут потеряны безвозвратно!!!"; 
	//--- Терпогосян Д.Б. [09.08.2021 17:40:43] № DEV-743 
...
Процедура ПрочитатьДанныеПользователя(Форма, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
	
	Если Форма.УстановкаДатыЗапрета = "НетЗапрета" Тогда
		
		РазблокироватьВсеЗаписиНаСервере(Форма.АдресУстановленныхБлокировок);
		Возврат;
		
	ИначеЕсли Форма.УстановкаДатыЗапрета = "ПоПользователям" Тогда
	...
	ПередаваемыеПараметры.Вставить("НачалоДня",                    Форма.НачалоДня);
	ПередаваемыеПараметры.Вставить("Пользователь",                 Форма.ТекущийПользователь);
	ПередаваемыеПараметры.Вставить("ЕдинственныйРаздел",           Форма.ЕдинственныйРаздел);
	ПередаваемыеПараметры.Вставить("ПоказыватьРазделы",            Форма.ПоказыватьРазделы);
	ПередаваемыеПараметры.Вставить("ВсеРазделыБезОбъектов",        Форма.ВсеРазделыБезОбъектов);
	ПередаваемыеПараметры.Вставить("РазделыБезОбъектов",           Форма.РазделыБезОбъектов);
	ПередаваемыеПараметры.Вставить("АдресТаблицыРазделов",         Форма.АдресТаблицыРазделов);
	ПередаваемыеПараметры.Вставить("ИдентификаторФормы",           Форма.УникальныйИдентификатор);
	ПередаваемыеПараметры.Вставить("СпособУказанияДатыЗапрета",    Форма.СпособУказанияДатыЗапрета);
	ПередаваемыеПараметры.Вставить("ЗначениеДляВсехПользователей", Форма.ЗначениеДляВсехПользователей);
	ПередаваемыеПараметры.Вставить("ДатыЗапретаЗагрузкиДанных",    Форма.Параметры.ДатыЗапретаЗагрузкиДанных);
	ПередаваемыеПараметры.Вставить("АдресУстановленныхБлокировок", Форма.АдресУстановленныхБлокировок);
	
	ДанныеПользователя = Неопределено;
	ПрочитатьДанныеПользователяНаСервере(ПередаваемыеПараметры,
		ДанныеПользователя, Форма.ПоказыватьРазделыТекущегоПользователя);
...
Процедура ПрочитатьДанныеПользователяНаСервере(Знач Форма, ДанныеПользователя, ПоказыватьРазделыТекущегоПользователя)
	...
	Если УдалитьЛишнее Тогда
		...
		Попытка
			...
			Иначе
				...
				//+++ Терпогосян Д.Б. [27.08.2021 15:33:18] № DEV-743
				Запрос.УстановитьПараметр("ОставитьДляВсехПользователей", Истина);
				//--- Терпогосян Д.Б. [27.08.2021 15:33:18] № DEV-743 
			КонецЕсли;
...
Функция КомментарийТекущегоПользователя(Форма)
	...
	Если Форма.УстановкаДатыЗапрета = "ПоПользователям" Тогда
		Комментарий = Форма.Элементы.Пользователи.ТекущиеДанные.Комментарий;
	Иначе
...
//+++ Терпогосян Д.Б. [09.08.2021 18:10:35] № DEV-743
&НаСервере
Процедура ПриОткрытииНаСервере(Отказ)
	гПолныеПрава = Справочники.ГруппыПользователей.НайтиПоРеквизиту("Идентификатор", "70908c47-f9e7-11eb-80fe-005056bc3fe8"); 
	мПолныеПрава = гПолныеПрава.Состав.ВыгрузитьКолонку("Пользователь"); 

	Если Не РольДоступна("ПолныеПрава") И НЕ мПолныеПрава.Найти(ПараметрыСеанса.ТекущийПользователь) Тогда
		Отказ = Истина; 
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Недостаточно прав доступа для открытия команды!";
		Сообщение.Сообщить(); 
	КонецЕсли;
КонецПроцедуры
...
КонецПроцедуры
//--- Терпогосян Д.Б. [09.08.2021 18:10:40] № DEV-743 
